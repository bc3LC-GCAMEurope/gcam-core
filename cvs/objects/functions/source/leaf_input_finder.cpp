...
/*! 
* \file leaf_input_finder.cpp
* \ingroup Objects
* \brief The LeafInputFinder class source file.
* \author Pralit Patel
*/
...
//! Default Constructor
LeafInputFinder::LeafInputFinder() {
}
...
/*!
 * \brief Gets the leaf inputs.
 * \return A vector of the leaf inputs.
 * \todo We should check to make sure an attempt to start visiting
 *       occured before a call to this method otherwise these results
 *       could be incorrect.  Also the way this class is set up it
 *       could only be used as a one time deal since there is no way
 *       to reset the found inputs.
 */
vector<IInput*> LeafInputFinder::getInputs() const {
    return mInputs;
}
