...
#' the generated outputs: \code{L1327.in_EJ_R_paper_Yh}, \code{L1327.in_EJ_R_paper_Yh_tech}, \code{L1327.IO_GJkg_R_paper_F_Yh_EUR},\code{L1327.out_Mt_R_paper_Yh_EUR}, \code{L1327.in_EJ_R_indenergy_F_Yh_EUR}, \code{L127.in_EJ_R_indchp_F_Yh_EUR}. The corresponding file in the
...
module_gcameurope_L1327.paper <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L1327.in_EJ_R_paper_F_Yh_EUR",
    ...
    L123.in_EJ_R_indchp_F_Yh_EUR <- get_data(all_data, "L123.in_EJ_R_indchp_F_Yh_EUR", strip_attributes = TRUE)
    paper_food_TFE <- get_data(all_data, "energy/paper_food_TFE", strip_attributes = TRUE) %>% filter_regions_europe()
    ...
    # Add biomass cogen to paper energy use (L1327.in_EJ_R_paper_F_Yh_EUR)
    L1327.in_EJ_R_paper_F_Yh_nochp %>%
    ...
    L1327.IO_woodpulp_energy_EUR <- L110.For_ALL_bm3_R_Y %>%
      ...
      left_join(L1327.in_EJ_R_paper_F_Yh_EUR %>%
    ...
    DEFAULT_PULP_ENERGY_IO <- L1327.IO_woodpulp_energy_EUR %>%
    ...
    L1326.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      rename(raw = value) %>%
      left_join(L1327.in_EJ_R_paper_F_Yh_EUR %>% select(-sector), by = c("GCAM_region_ID", "year", "fuel")) %>%
      ungroup() %>%
      mutate(value = replace_na(value, 0),
    ...
    L1327.in_EJ_R_paper_F_Yh_EUR %>%
      ...
      bind_rows(L1327.total_process_heat_calculated,
                L1327.total_woodpulp_energy_calculated) %>%
      rename(input = value) %>%
      left_join(L1327.out_Mt_R_paper_Yh_EUR %>% rename(output = value), by = c("GCAM_region_ID", "year", "sector")) %>%
    ...
    L1327.out_Mt_R_paper_Yh_EUR %>%
      ...
      add_legacy_name("L1327.out_Mt_R_paper_Yh_EUR") %>%
    ...
    L1327.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      add_units("Exajoules") %>%
      add_comments("Subtracted pulp and paper energy use from industrial energy use values in L1326.in_EJ_R_indenergy_F_Yh_EUR") %>%
    ...
    L127.in_EJ_R_indchp_F_Yh_EUR %>%
      ...
      add_units("None") %>%
      add_comments("Assumed biomass cogen in paper industry subtracted from L123.in_EJ_R_indchp_F_Yh_EUR") %>%
      add_legacy_name("L127.in_EJ_R_indchp_F_Yh_EUR") %>%
      add_precursors("L123.in_EJ_R_indchp_F_Yh_EUR", "energy/paper_food_TFE",
    ...
    return_data(L1327.in_EJ_R_paper_F_Yh_EUR, L1327.out_Mt_R_paper_Yh_EUR,
                L1327.IO_GJkg_R_paper_F_Yh_EUR, L1327.in_EJ_R_indenergy_F_Yh_EUR,
                L1327.elec_noheat_adj_shwt_R_EUR, L127.in_EJ_R_indchp_F_Yh_EUR,
                L1327.IO_woodpulp_energy_EUR)
