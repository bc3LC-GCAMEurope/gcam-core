...
module_socio_L100.NationalAccounts <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    assertthat::assert_that(L100.National_Accounts_Employment_Share_POP_R_Yh %>% filter(year == Socioeconomic.PWT.LastYear) %>%
                              proxy_socioecon_eu(., 'lt', 0.25) %>%          # NOTE 13/03/2026: Bosnia&Hzg has 0.209 employment rate in 2019. The value of Macedonia is used as proxy.
    ...
    assertthat::assert_that(Capital_GDP_Ratio_R_Yh %>%
                              proxy_socioecon_eu(., 'lt', 0.8) %>%          # NOTE 13/03/2026: use some proxies for Bulgaria from 1971-1976, which has values ranging from 0.6-0.78
    ...
    assertthat::assert_that(L100.National_Accounts_En_capital_inv_share_R_Yh %>%
                              proxy_socioecon_eu(., 'lt', 0.02) %>%           # NOTE 13/03/2026: use proxies for some 0.018 values
                              pull(value) %>% min > 0.02, msg = "check min value in data")
    assertthat::assert_that(L100.National_Accounts_En_capital_inv_share_R_Yh %>%
