# Copyright 2019 Battelle Memorial Institute; see the LICENSE file.

#' module_policy_MAC_rename_xml
#'
#' Construct XML data structure for \code{aeei.xml}.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{policy_MAC_rename.xml}.
module_policy_MAC_rename_xml <- function(command, ...) {
  all_xml_names <- get_xml_names("policy/A_MAC_rename.csv", "policy_MAC_rename.xml")

  MODULE_INPUTS <- c("L306.ResMAC_fos",
                     "L306.MAC_higwp",
                     "L306.AgMAC",
                     "L306.MAC_an",
                     "L306.ResMAC_fos_tc_average",
                     "L306.ResMAC_fos_phaseInTime",
                     "L306.MAC_higwp_tc_average",
                     "L306.MAC_higwp_phaseInTime")

  if(command == driver.DECLARE_INPUTS) {
    return(MODULE_INPUTS)
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(all_xml_names)
  } else if(command == driver.MAKE) {

    all_data <- list(...)[[1]]

    # Load required inputs
    get_data_list(all_data, MODULE_INPUTS)

    # ===================================================

    # Produce outputs
    for (xml_name in all_xml_names){
      filter_for_xml <- function(df) filter_xml(df, xml_name)  # Wrapper function

        assign(xml_name,
               create_xml(xml_name) %>%
                 add_xml_data(filter_for_xml(L306.ResMAC_fos), "ResMAC") %>%
                 add_xml_data(filter_for_xml(L306.MAC_higwp), "MAC") %>%
                 add_xml_data(filter_for_xml(L306.AgMAC), "AgMAC") %>%
                 add_xml_data(filter_for_xml(L306.MAC_an), "MAC") %>%
                 add_xml_data(filter_for_xml(L306.ResMAC_fos_tc_average), "ResMACTC") %>%
                 add_xml_data(filter_for_xml(L306.ResMAC_fos_phaseInTime), "ResMACPhaseIn") %>%
                 add_xml_data(filter_for_xml(L306.MAC_higwp_tc_average), "MACTC") %>%
                 add_xml_data(filter_for_xml(L306.MAC_higwp_phaseInTime), "MACPhaseIn") %>%
                 add_precursors(MODULE_INPUTS)
        )
      }



    # Need this for loop because having issues with lapply(all_xml_names, get)
    list_of_xmls <- list()
    for(xml_name in all_xml_names){
      list_of_xmls[[xml_name]] <- get(xml_name)
    }
    return_multiple_xmls(list_of_xmls, all_xml_names)

  } else {
    stop("Unknown command")
  }
}
