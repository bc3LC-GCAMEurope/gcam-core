...
#' module_gcameurope_L132.water_demand_manufacturing
#'
...
module_gcameurope_L132.water_demand_manufacturing <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L132.water_km3_ctry_ind_Yh_EUR",
    ...
    L101.en_bal_EJ_R_Si_Fi_Yh_EUR <- get_data(all_data, "L101.en_bal_EJ_R_Si_Fi_Yh_EUR")
    # want only the isos of the main GCAM-Europe regions (no islands, etc)
    main_europe_isos <- GCAM_region_names %>%
      semi_join(L101.en_bal_EJ_R_Si_Fi_Yh_EUR, by = "GCAM_region_ID") %>%
      left_join_error_no_match(iso_GCAM_regID %>%
                  filter(iso != "rou") %>%
                  mutate(country_name = gsub("Macedonia, the former Yugoslav Republic of", "Macedonia", country_name),
                         country_name = gsub("Moldova, Republic of", "Moldova", country_name),
                         country_name = gsub("United Kingdom", "UK", country_name)), by = c("GCAM_region_ID", "region" = "country_name")) %>%
      distinct(GCAM_region_ID, iso)

    # combine all energy balance with isos
    L101.en_bal_EJ_ctry_Si_Fi_Yh_EUR <- L101.en_bal_EJ_R_Si_Fi_Yh_EUR %>%
      left_join_error_no_match(main_europe_isos, by = "GCAM_region_ID") %>%
      bind_rows(L101.en_bal_EJ_ctry_Si_Fi_Yh_full)

    water.GCAM_MFG_SECTORS_VASSOLO_EUR <- c(water.GCAM_MFG_SECTORS_VASSOLO, "net_CALCULATED_industry_energy transformation")
    # ===================================================
    ...
    L132.in_EJ_cntnt_ind_1995 <- filter(L101.en_bal_EJ_ctry_Si_Fi_Yh_EUR,
                                        sector %in% water.GCAM_MFG_SECTORS_VASSOLO_EUR,
    ...
    L132.water_km3_ctry_ind_Yh_EUR <- filter(L101.en_bal_EJ_ctry_Si_Fi_Yh_EUR,
                                         sector %in% water.GCAM_MFG_SECTORS_VASSOLO_EUR,
                                         fuel %in% water.GCAM_MFG_FUELS_EFW) %>%
    ...
    L132.scalers_ctry_ind_Yh <-
      ...
      ungroup() %>%
      inner_join(subset(L132.water_km3_ctry_ind_Yh_EUR, water_type == "water withdrawals"), by = c("iso", "year")) %>%
    ...
    L132.water_km3_ctry_ind_Yh_EUR <-
      left_join(L132.water_km3_ctry_ind_Yh_EUR, L132.scalers_ctry_ind_Yh,
                by = c("iso", "year")) %>%
    ...
    L132.water_km3_R_ind_Yh_EUR <-
      left_join_error_no_match(L132.water_km3_ctry_ind_Yh_EUR, iso_GCAM_regID[c("iso", "GCAM_region_ID")],
                               by = "iso") %>%
    ...
    L132.water_km3_ctry_ind_Yh_EUR %>%
      ...
      add_legacy_name("L132.water_km3_ctry_ind_Yh_EUR") %>%
    ...
    L132.water_km3_R_ind_Yh_EUR %>%
      ...
      add_comments("to determine water withdrawal and consumption coefficients") %>%
      add_comments("Estimated withdrawal volumes are capped by Aquastat data") %>%
      add_legacy_name("L132.water_km3_R_ind_Yh_EUR") %>%
    ...
    return_data(L132.water_km3_ctry_ind_Yh_EUR, L132.water_km3_R_ind_Yh_EUR)
  } else {
