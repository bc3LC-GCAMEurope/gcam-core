...
#' module_gcameurope_L173.EFW_manufacturing
#'
...
#'   \code{command} is "MAKE") all the generated outputs: \code{L173.in_EJ_R_indEFWtot_F_Yh_EUR}
#' @details Generate estimates of energy consumption and energy input-output coefficients  for
...
module_gcameurope_L173.EFW_manufacturing <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L173.in_EJavail_ind <- subset(L101.en_bal_EJ_R_Si_Fi_Yh_EUR,
    ...
    L173.in_EJ_R_indScaler_Yh_EUR <- select(L173.in_ALL_ctry_indEFW_F_Yh,
    ...
    L173.in_ALL_ctry_R_indEFW_F_Yh <- L173.in_ALL_ctry_indEFW_F_Yh %>%
      # Switch to region ID and filter to GCAM-Europe
      left_join_error_no_match(iso_GCAM_regID, by = "iso") %>%
      semi_join(L101.en_bal_EJ_R_Si_Fi_Yh_EUR, by = "GCAM_region_ID") %>%
      left_join_error_no_match(select(L173.in_EJ_R_indScaler_Yh_EUR, GCAM_region_ID, year, fuel, scaler),
                               by = c("GCAM_region_ID", "year", "fuel")) %>%
    ...
    L173.in_EJ_R_indEFWtot_F_Yh_EUR <- L173.in_ALL_ctry_R_indEFW_F_Yh %>%
    ...
    L173.IO_GJkm3_R_indEFW_F_Yh_EUR <- L173.in_ALL_ctry_indEFW_F_Yh %>%
