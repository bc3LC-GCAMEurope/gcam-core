...
#' module_gcameurope_L211.ag_nonco2
#'
...
#' the generated outputs: \code{L211.AWBEmissions_EUR}, \code{L211.AGREmissions_EUR}, \code{L211.AnEmissions_EUR}, \code{L211.AnNH3Emissions_EUR}, \code{L211.AGRBio_EUR}, \code{L211.AWB_BCOC_EmissCoeff_EUR}, \code{L211.nonghg_max_reduction_EUR}, \code{L211.nonghg_steepness_EUR}. The corresponding file in the
...
#' @author RH July 2017
module_gcameurope_L211.ag_nonco2 <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L211.AWBEmissions_EUR",
             "L211.AGREmissions_EUR",
             "L211.AnEmissions_EUR",
             "L211.AnNH3Emissions_EUR",
             "L211.AGRBio_EUR",
             "L211.AWB_BCOC_EmissCoeff_EUR",
             "L211.nonghg_max_reduction_EUR",
             "L211.nonghg_steepness_EUR"))
    ...
    basin_to_country_mapping <- get_data(all_data, "water/basin_to_country_mapping", strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    A_regions <- get_data(all_data, "emissions/A_regions") %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L2052.AgCost_bio_irr_mgmt <- get_data(all_data, "L2052.AgCost_bio_irr_mgmt", strip_attributes = TRUE) %>% filter_regions_europe()
    L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR <- get_data(all_data, "L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR", strip_attributes = TRUE)
    L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR <- get_data(all_data, "L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR", strip_attributes = TRUE)
    L121.nonco2_tg_R_awb_C_Y_GLU_EUR <- get_data(all_data, "L121.nonco2_tg_R_awb_C_Y_GLU_EUR", strip_attributes = TRUE) %>%
      replace_GLU(basin_to_country_mapping)
    L122.ghg_tg_R_agr_C_Y_GLU_EUR <- get_data(all_data, "L122.ghg_tg_R_agr_C_Y_GLU_EUR", strip_attributes = TRUE) %>%
      replace_GLU(basin_to_country_mapping)
    L123.bcoc_tgmt_R_awb_2000 <- get_data(all_data, "L123.bcoc_tgmt_R_awb_2000", strip_attributes = TRUE) %>%
      replace_GLU(basin_to_country_mapping) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    ...
    # ===================================================
    # L211.AWBEmissions_EUR: Agricultural Waste Burning emissions in all regions
    L211.AWBEmissions_EUR <- L121.nonco2_tg_R_awb_C_Y_GLU_EUR %>%
    ...
    # L211.AGREmissions_EUR: Agricultural emissions in all regions
    L211.AGREmissions_EUR <- L122.ghg_tg_R_agr_C_Y_GLU_EUR %>%
      filter(year %in% emissions.MODEL_BASE_YEARS) %>%
      # Add region, supplysector, subsector and tech names
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      rename(input.emissions = value,
    ...
    L211.AGRBio_EUR <- L2052.AgCost_bio_irr_mgmt %>%
    ...
    # L211.AnEmissions_EUR: Animal emissions in all regions
    L211.AnEmissions_EUR <- L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR %>%
      filter(year %in% emissions.MODEL_BASE_YEARS) %>%
      rename(input.emissions = value) %>%
      # Add region
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    # L211.AnNH3Emissions_EUR: Animal NH3 emissions in all regions
    L211.AnNH3Emissions_EUR <- L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR %>%
      filter(year %in% emissions.MODEL_BASE_YEARS) %>%
      rename(input.emissions = value) %>%
      # Add region
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    # L211.AWB_BCOC_EmissCoeff_EUR: BC / OC AWB emissions coefficients in all regions
    # Add region name & replicate for all commodities & base years
    L211.AWB_BCOC_EmissCoeff_EUR <- L123.bcoc_tgmt_R_awb_2000 %>%
      # Add region, supplysector, subsector and tech names
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    # L211.nonghg_max_reduction_EUR: maximum emissions coefficient reduction for ag technologies in all regions
    L211.nonghg_max_reduction_EUR <- bind_rows(L211.AWB_BCOC_EmissCoeff_EUR %>%
                                             select(-emiss.coef),
                                           L211.AWBEmissions_EUR %>%
                                             select(-input.emissions)) %>%
      filter(year == emissions.CTRL_BASE_YEAR) %>%
      mutate(ctrl.name = "GDP_control") %>%
      left_join_error_no_match(A11.max_reduction, by = "AgSupplySector")

    # L211.nonghg_steepness_EUR: steepness of reduction, as function of per-capita GDP, for agricultural technologies in all regions
    L211.nonghg_steepness_EUR <- L211.nonghg_max_reduction_EUR %>%
    ...
    L211.AWBEmissions_EUR %>%
      ...
      add_legacy_name("L211.AWBEmissions_EUR") %>%
      add_precursors("common/GCAM_region_names", "water/basin_to_country_mapping",
                     "emissions/A_regions", "L121.nonco2_tg_R_awb_C_Y_GLU_EUR") ->
      L211.AWBEmissions_EUR
    L211.AGREmissions_EUR %>%
      ...
      add_legacy_name("L211.AGREmissions_EUR") %>%
      add_precursors("common/GCAM_region_names", "water/basin_to_country_mapping",
                     "L122.ghg_tg_R_agr_C_Y_GLU_EUR") ->
      L211.AGREmissions_EUR
    L211.AnEmissions_EUR %>%
      ...
      add_units("Tg") %>%
      add_comments("Filtered L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR by year, added region, and rounded value") %>%
      add_legacy_name("L211.AnEmissions_EUR") %>%
      add_precursors("common/GCAM_region_names", "L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR") ->
      L211.AnEmissions_EUR
    L211.AnNH3Emissions_EUR %>%
      add_title("Animal NH3 emissions by region, supplysector, subsector, stub.technology and historical year") %>%
      add_units("Tg") %>%
      add_comments("Filtered L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR by year, added region, and rounded value") %>%
      add_legacy_name("L211.AnNH3Emissions_EUR") %>%
      add_precursors("common/GCAM_region_names", "L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR") ->
      L211.AnNH3Emissions_EUR
    L211.AGRBio_EUR %>%
      ...
      add_legacy_name("L211.AGRBio_EUR") %>%
    ...
    L211.AWB_BCOC_EmissCoeff_EUR %>%
      ...
      add_legacy_name("L211.AWB_BCOC_EmissCoeff_EUR") %>%
      add_precursors("common/GCAM_region_names", "water/basin_to_country_mapping",
                     "L123.bcoc_tgmt_R_awb_2000") ->
      L211.AWB_BCOC_EmissCoeff_EUR
    L211.nonghg_max_reduction_EUR %>%
      ...
      add_legacy_name("L211.nonghg_max_reduction_EUR") %>%
      add_precursors("common/GCAM_region_names", "water/basin_to_country_mapping",
    ...
    L211.nonghg_steepness_EUR %>%
      ...
      add_legacy_name("L211.nonghg_steepness_EUR") %>%
      add_precursors("common/GCAM_region_names", "water/basin_to_country_mapping",
                     "L123.bcoc_tgmt_R_awb_2000", "emissions/A11.max_reduction",
                     "emissions/A_regions", "L121.nonco2_tg_R_awb_C_Y_GLU_EUR",
    ...
    return_data(L211.AWBEmissions_EUR, L211.AGREmissions_EUR, L211.AnEmissions_EUR, L211.AnNH3Emissions_EUR,
                L211.AGRBio_EUR, L211.AWB_BCOC_EmissCoeff_EUR, L211.nonghg_max_reduction_EUR, L211.nonghg_steepness_EUR)
  } else {
