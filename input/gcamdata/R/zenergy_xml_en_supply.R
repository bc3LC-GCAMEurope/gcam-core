...
module_energy_en_supply_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L239.PrimaryConsKeyword_en <- get_data(all_data, "L239.PrimaryConsKeyword_en")   %>% filter(region != "Switzerland")
    ...
    L239.Supplysector_reg <- get_data(all_data, "L239.Supplysector_reg")  %>% filter(region != "Switzerland")
    L239.SubsectorAll_reg <- get_data(all_data, "L239.SubsectorAll_reg")  %>% filter(region != "Switzerland")
    L239.TechShrwt_reg <- get_data(all_data, "L239.TechShrwt_reg")  %>% filter(region != "Switzerland")
    L239.TechCoef_reg <- get_data(all_data, "L239.TechCoef_reg")  %>% filter(region != "Switzerland")
    L239.Production_reg_imp <- get_data(all_data, "L239.Production_reg_imp") %>% filter(region != "Switzerland")
    L239.Production_reg_dom <- get_data(all_data, "L239.Production_reg_dom")  %>% filter(region != "Switzerland")
    L239.Consumption_intraregional <- get_data(all_data, "L239.Consumption_intraregional")  %>% filter(region != "Switzerland")
    L239.CarbonCoef <- get_data(all_data, "L239.CarbonCoef")
    L281.TechAccountInput_entrade <- get_data(all_data, "L281.TechAccountInput_entrade")  %>% filter(region != "Switzerland")
    ...
    L281.TechAccountOutput_entrade <- get_data(all_data, "L281.TechAccountOutput_entrade") %>%
        filter(supplysector %in% unique(L239.TechCoef_tra$supplysector))  %>% filter(region != "Switzerland")
    # ===================================================
