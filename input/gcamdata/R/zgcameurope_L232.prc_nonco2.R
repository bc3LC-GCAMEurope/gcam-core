...
#' module_gcameurope_L232.prc_nonco2
#'
...
#' the generated outputs: \code{L232.nonco2_prc_EUR}, \code{L232.nonco2_max_reduction_EUR}, \code{L232.nonco2_steepness_EUR}. The corresponding file in the
...
#' @author RH July 2017
module_gcameurope_L232.prc_nonco2 <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L131.nonco2_tg_R_prc_S_S_Yh_EUR <- get_data(all_data, "L131.nonco2_tg_R_prc_S_S_Yh_EUR", strip_attributes = TRUE)
    A32.max_reduction <- get_data(all_data, "emissions/A32.max_reduction")
    A32.steepness <- get_data(all_data, "emissions/A32.steepness")

    # ===================================================
    # L232.nonco2_prc_EUR: Pollutant emissions for energy technologies in all regions
    # Formatting and rounding L131.nonco2_tg_R_prc_S_S_Yh_EUR
    L232.nonco2_prc_EUR <- L131.nonco2_tg_R_prc_S_S_Yh_EUR %>%
      ungroup() %>%
    ...
    # L232.nonco2_max_reduction_EUR: maximum reduction for energy technologies in all regions
    L232.nonco2_max_reduction_EUR <- L232.nonco2_prc_EUR %>%
    ...
    # L232.nonco2_steepness_EUR: steepness of reduction for energy technologies in all regions
    L232.nonco2_steepness_EUR <- L232.nonco2_max_reduction_EUR %>%
      select(region, supplysector, subsector, stub.technology, Non.CO2) %>%
      repeat_add_columns(tibble(year = as.integer(MODEL_BASE_YEARS))) %>%
      mutate(ctrl.name = "GDP_control") %>%
    ...
    L232.nonco2_prc_EUR %>%
      ...
      add_units("Tg") %>%
      add_comments("Filtered and rounded L131.nonco2_tg_R_prc_S_S_Yh_EUR") %>%
      add_legacy_name("L232.nonco2_prc_EUR") %>%
      add_precursors("common/GCAM_region_names", "emissions/A_regions", "L131.nonco2_tg_R_prc_S_S_Yh_EUR") ->
      L232.nonco2_prc_EUR
    L232.nonco2_max_reduction_EUR %>%
      ...
      add_legacy_name("L232.nonco2_max_reduction_EUR") %>%
      add_precursors("common/GCAM_region_names", "emissions/A_regions",
                     "L131.nonco2_tg_R_prc_S_S_Yh_EUR", "emissions/A32.max_reduction") ->
      L232.nonco2_max_reduction_EUR
    L232.nonco2_steepness_EUR %>%
      ...
      add_legacy_name("L232.nonco2_steepness_EUR") %>%
      add_precursors("common/GCAM_region_names", "emissions/A_regions",
                     "L131.nonco2_tg_R_prc_S_S_Yh_EUR", "emissions/A32.steepness") ->
      L232.nonco2_steepness_EUR

    return_data(L232.nonco2_prc_EUR, L232.nonco2_max_reduction_EUR, L232.nonco2_steepness_EUR)
  } else {
