...
module_energy_L210.resources <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L210.RsrcEnvironCost_SSP4 %>%
      ...
      mutate(environCost = dplyr::if_else(resource == "coal" & region %in% L210.low_reg, 0, input.cost),
             environCost = dplyr::if_else(resource == "coal" & region %in% L210.high_reg, 10 * input.cost, input.cost)) %>%
