...
#' module_gcameurope_L232.water_demand_manufacturing
#'
...
#' the generated outputs: \code{L232.TechCoef_EUR}. The corresponding file in the
...
module_gcameurope_L232.water_demand_manufacturing <- function(command, ...) {
  ...
  MODULE_INPUTS <-
    c(FILE = "common/GCAM_region_names",
      ...
      # Global datasets to scale from USA values
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    GCAM_region_names_USA <- GCAM_region_names %>% filter_regions_europe() %>%
      bind_rows(GCAM_region_names %>% filter(region == gcam.USA_REGION))
    ...
    L1327.out_Mt_R_paper_Yh_EUR %>%
      ...
      filter(year %in% unique(L132.water_km3_R_ind_Yh_EUR$year)) %>%
    ...
    L1328.out_Pcal_R_food_Yh_EUR %>%
      ...
      left_join_error_no_match(L1328.in_EJ_R_food_F_Yh_EUR %>%
    ...
    L1328.out_Pcal_R_food_Yh %>%
      ...
      left_join_error_no_match(L1328.in_EJ_R_food_F_Yh %>%
                               ...
                               by = c("GCAM_region_ID", "year")) %>%
      mutate(energy_coef = energy / prod) ->
      L1328.IO_inputEJPcal_R_food_Yh_USA

    # Calculate ratio of coefficients in latest base year relative to USA's coefficient
    L1328.IO_inputEJPcal_R_food_Yh %>%
      left_join_error_no_match(L1328.IO_inputEJPcal_R_food_Yh_USA %>%
                                 filter(GCAM_region_ID == gcam.USA_CODE) %>%
                                 left_join_error_no_match(GCAM_region_names_USA, by = "GCAM_region_ID") %>%
      ...
      # add USA data
      bind_rows(L1328.IO_inputEJPcal_R_food_Yh_USA %>%
                  filter(GCAM_region_ID == gcam.USA_CODE) %>%
    ...
    food_mfg_intensity %>%
      ...
      complete(nesting(sector, intensity), region = unique(GCAM_region_names_USA$region)) %>%
      left_join_error_no_match(GCAM_region_names_USA, by = "region") %>%
    ...
    L232.water_km3_R_ind_Yh %>%
      ...
      add_legacy_name("L232.TechCoef_EUR") %>%
    ...
    L1327.water_km3_R_ind_Yh_paper %>%
      ...
      repeat_add_columns(distinct(L2327.StubTechProd_paper_EUR, supplysector, subsector, technology = stub.technology)) %>%
      mutate(water_sector = "Manufacturing",
             minicam.energy.input = set_water_input_name(water_sector, water_type, water_td_sectors),
             market.name = region) %>%
      select(LEVEL2_DATA_NAMES$TechCoef) %>%
    ...
    L2327.TechCoef_paper_EUR %>%
      ...
      add_legacy_name("L2327.TechCoef_paper_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "water/water_td_sectors",
                     "water/paper_mfg_intensity",
                     "water/mfg_water_ratios",
    ...
    L1328.water_km3_R_ind_Yh_food_recal %>%
      ...
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      select(region, water_type, year, coefficient) %>%
      repeat_add_columns(distinct(L2328.StubTechProd_food_EUR, supplysector, subsector, technology = stub.technology)) %>%
      mutate(water_sector = "Manufacturing",
             minicam.energy.input = set_water_input_name(water_sector, water_type, water_td_sectors),
             market.name = region) %>%
      select(LEVEL2_DATA_NAMES$TechCoef) %>%
