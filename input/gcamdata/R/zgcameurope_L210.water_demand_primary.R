...
#' module_gcameurope_L210.water_demand_primary
#'
...
#' the generated outputs: \code{L210.TechCoef_EUR}. The corresponding file in the
...
#' @author YL May 2017
module_gcameurope_L210.water_demand_primary <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    bind_rows(A21.tmp1, A22.tmp,A_ff_RegionalTechnology.tmp,A_ff_TradedTechnology.tmp) %>%
      unique() %>%
      left_join(L110.water_demand_primary_R_S_W_m3_GJ_EUR, ., by = "supplysector") %>%
    ...
    tibble(year = MODEL_YEARS, MERGEFIELD = 1) %>%
      left_join(mutate(L210.TechCoef_EUR, MERGEFIELD = 1), by = "MERGEFIELD") %>%
      ...
      add_legacy_name("L210.TechCoef_EUR") %>%
