...
#' module_gcameurope_L102.en_emiss_CDIAC
#'
...
#' the generated outputs: \code{L102.CO2_Mt_R_F_Yh_EUR}, \code{L102.Ccoef_kgCGJ_R_F_Yh_EUR}, \code{L102.Ccoef_kgCGJ_F_Yh_EUR}.
#' The corresponding file in the original data system was \code{LA102.en_emiss_CDIAC.R} (energy level1).
#' @details This chunk maps the CDIAC country emissions database (L100.CDIAC_CO2_ctry_hist) onto GCAM
#' regions (iso_GCAM_regID) and fuels (CDIAC_fuel) to determine regional emissions by fuel (L102.CO2_Mt_R_F_Yh_EUR),
#' extracts non-energy emissions and uses the remainder to calculate regional (L102.Ccoef_kgCGJ_R_F_Yh_EUR) and
#' global (L102.Ccoef_kgCGJ_F_Yh_EUR) emissions coefficients
#' @importFrom assertthat assert_that
#' @importFrom dplyr filter if_else group_by mutate select summarise
#' @author CWR July 2017

module_gcameurope_L102.en_emiss_CDIAC <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "emissions/mappings/CDIAC_fuel",
             FILE = "energy/A32.nonenergy_Cseq",
             "L100.CDIAC_CO2_ctry_hist",
             "L101.GCAM_EUR_regions",
             "L1012.en_bal_EJ_R_Si_Fi_Yh_EUR"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L102.CO2_Mt_R_F_Yh_EUR",
             "L102.Ccoef_kgCGJ_R_F_Yh_EUR",
             "L102.Ccoef_kgCGJ_F_Yh_EUR"))
    ...
    L100.CDIAC_CO2_ctry_hist %>%
      ...
      left_join(iso_GCAM_regID_reg, by = "iso") %>% # expected NAs because we only subset EUR
    ...
    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR %>%
      filter(sector == "in_industry_feedstocks",
             fuel %in% L102.CO2_Mt_R_F_Yh_EUR$fuel) %>%
    ...
    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR %>%
      filter(sector == "TPES", fuel %in% L102.CO2_Mt_R_F_Yh_EUR$fuel) %>%
      filter(year <= max(L102.en_sequestered_EJ_R_Fi_Yh$year)) %>% # avoid NAs
    ...
    L102.CO2_Mt_R_F_Yh_EUR %>%
      filter(year %in% HISTORICAL_YEARS) -> L102.CO2_Mt_R_F_Yh_EUR
    ...
    L102.CO2_Mt_R_F_Yh_EUR %>%
      ...
      left_join_error_no_match(L102.en_emitted_EJ_R_Fi_Yh %>%
                                 filter(year <= max(L102.CO2_Mt_R_F_Yh_EUR$year)), # avoid NAs
    ...
    L102.CO2_Mt_R_F_Yh_EUR %>%
      ...
      add_legacy_name("L102.CO2_Mt_R_F_Yh_EUR") %>%
    ...
    L102.Ccoef_kgCGJ_R_F_Yh_EUR %>%
      ...
      add_legacy_name("L102.Ccoef_kgCGJ_R_F_Yh_EUR") %>%
      add_precursors("L100.CDIAC_CO2_ctry_hist", "common/iso_GCAM_regID", "emissions/mappings/CDIAC_fuel", "L1012.en_bal_EJ_R_Si_Fi_Yh_EUR", "energy/A32.nonenergy_Cseq")  ->
      L102.Ccoef_kgCGJ_R_F_Yh_EUR
    ...
    L102.Ccoef_kgCGJ_F_Yh_EUR %>%
      ...
      add_legacy_name("L102.Ccoef_kgCGJ_F_Yh_EUR") %>%
      add_precursors("L100.CDIAC_CO2_ctry_hist", "common/iso_GCAM_regID", "emissions/mappings/CDIAC_fuel", "L1012.en_bal_EJ_R_Si_Fi_Yh_EUR", "energy/A32.nonenergy_Cseq") ->
      L102.Ccoef_kgCGJ_F_Yh_EUR

    return_data(L102.CO2_Mt_R_F_Yh_EUR, L102.Ccoef_kgCGJ_R_F_Yh_EUR, L102.Ccoef_kgCGJ_F_Yh_EUR)
  } else {
