...
#' module_gcameurope_L1325.chemical
#'
...
#' the generated outputs: \code{L1325.in_EJ_R_chemical_F_Y_EUR}, \code{L1325.in_EJ_R_indenergy_F_Yh_EUR}, \code{L1324.in_EJ_R_indfeed_F_Yh_EUR}. The corresponding file in the
...
module_gcameurope_L1325.chemical <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1325.in_EJ_R_chemical_F_Y_EUR_raw %>%
      ...
      left_join_error_no_match(L1322.in_EJ_R_indenergy_F_Yh_EUR%>%
                                 group_by(GCAM_region_ID, year, fuel) %>%
      ...
      L1325.in_EJ_R_chemical_F_Y_EUR_CHEMICAL

    L1325.in_EJ_R_chemical_F_Y_EUR_raw %>%
      filter(grepl("feedstock", sector)) %>%
      left_join_error_no_match(L132.in_EJ_R_indfeed_F_Yh_EUR%>%
                                 group_by(GCAM_region_ID, year, fuel) %>%
                                 summarise(value_ori = sum(value)),
                               by = c("GCAM_region_ID",  "fuel", "year")) %>%
      left_join_error_no_match(L1322.in_EJ_R_indfeed_F_Yh_EUR%>%
                                 group_by(GCAM_region_ID, year, fuel) %>%
                                 summarise(value_new = sum(value)),
                               by = c("GCAM_region_ID",  "fuel", "year")) %>%
      mutate(value = value - value_ori + value_new,
             value =if_else(value > 0 , value, 0)) %>%
      select(-value_ori, -value_new) ->
      L1325.in_EJ_R_chemical_F_Y_EUR_NECHEM
    ...
    L1324.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      rename(raw = value) %>%
      left_join(L1325.in_EJ_R_chemical_F_Y_EUR %>%
      ...
      mutate(value = replace_na(value, 0),
             value = raw - value ) ->
      L1325.in_EJ_R_indenergy_F_Yh_EUR_tmp
    ...
    L1325.in_EJ_R_chemical_F_Y_EUR %>%
      ...
      L1325.in_EJ_R_chemical_F_Y_EUR_recal
    ...
    L1324.in_EJ_R_indenergy_F_Yh_EUR %>%
      rename(raw = value) %>%
      left_join(L1325.in_EJ_R_chemical_F_Y_EUR_recal %>%
                  group_by(GCAM_region_ID, year, fuel) %>%
                  summarise(value = sum(value)), by = c("GCAM_region_ID", "year", "fuel")) %>%
      ungroup() %>%
      mutate(value = replace_na(value, 0),
    ...
    L1325.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      add_units("Exajoules") %>%
      add_comments("Subtracted chemical energy use from industrial energy use values in L1324.in_EJ_R_indenergy_F_Yh_EUR") %>%
    ...
    L1324.in_EJ_R_indfeed_F_Yh_EUR %>%
      ...
      add_units("Exajoules") %>%
      add_comments("Subtracted chemical feedstock use from industrial feedstock use values in L1324.in_EJ_R_indfeed_F_Yh_EUR") %>%
