...
#' the generated outputs: \code{L144.end_use_eff_EUR}, \code{L144.shell_eff_R_Y_EUR}, \code{L144.in_EJ_R_bld_serv_F_Yh_EUR}, \code{L144.in_EJ_R_bld_serv_tech_F_Yh_EUR},
#' \code{L144.NEcost_75USDGJ_EUR}, \code{L144.internal_gains_EUR}, \code{L144.base_service_EJ_serv_EUR}, \code{L144.prices_bld_EUR}, \code{L144.base_service_EJ_serv_hh_EUR},
#' \code{L144.base_service_EJ_serv_fuel_hh_EUR}. The corresponding file in the
...
#' @importFrom dplyr bind_rows filter group_by left_join lag mutate pull select summarise rowwise coalesce
...
module_gcameurope_L144.building_det_en <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/GCAM_region_names",
             ...
             FILE = "gcam-europe/A44.shares_hp_EUR",
    ...
    A44.shares_hp_EUR <- get_data(all_data, "gcam-europe/A44.shares_hp_EUR", strip_attributes = TRUE)
    ...
    # Reshape hp shares' data
    A44.shares_hp_EUR <- A44.shares_hp_EUR %>%
      pivot_longer(cols = c(-decile, -technology),
                   names_to = 'country', values_to = 'D10share') %>%
      mutate(decile = paste0('d',decile)) %>%
      left_join(GCAM32_to_EU %>%
                  select(country = GCAMEU_region, GCAM_region_ID),
                by = 'country') %>%
      select(-country) %>%
      distinct()
    ...
    # Note that this produces a final output table
    # We compute it by hh group (_hh_) and aggregated (default)
    ...
    # considering HH groups
    L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR_comm <- L144.in_EJ_R_bld_serv_tech_F_Yh_EUR %>%
      filter(grepl("comm", sector)) %>%
    ...
    L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR_resid <- rbind(
      ...
      L144.in_EJ_R_bld_serv_tech_F_Yh_EUR %>%
        ...
        mutate(value_to_add_byHH = sum(value_to_add),                                                                  # quantity to be added by the decile
               total_value       = sum(value, na.rm = TRUE)) %>%                                                       # count total value by group to allocate value_to_add proportionally
        ungroup() %>%
        mutate(value_to_add_byHH = if_else(value_to_add == 0 & value != 0, value_to_add_byHH * value / total_value, 0),# quantity to add by item
               final_share       = if_else(value != 0, final_share + value_to_add_byHH / value, final_share)) %>%      # update final share
        select(-value_to_add_byHH, -total_value, -value_to_add, -share_to_add) %>%
        ...
        mutate(gap_value           = sum(value*fuel_share) - sum(value*final_share),                                   # quantity to be added by the country
               total_value         = sum(value, na.rm = TRUE)) %>%                                                     # count nº items that will modify their share
        ungroup() %>%
        mutate(value_to_add_byCTRY = if_else((is.na(D10share) | D10share != 0) & value != 0,
                                             gap_value * value / total_value, 0),                                      # quantity to add by item
        ...
        select(-D10share, -gap_value, -total_value, -value_to_add_byCTRY) %>%
        # apply share and clean data
        mutate(gcam.consumer = paste('resid EUR', decile, sep = '_'),
               value = if_else(!GCAM_region_ID %in% c(47,50) | fuel != 'elec_solar CSP',
                               value * final_share, value * fuel_share)) %>% # skip adjustment for solar fuel
    ...
    L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR <- rbind(
      ...
      L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR_resid
    )


    L144.base_service_EJ_serv_hh_EUR <- L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR %>%
      # Join efficiency data
      left_join(L144.end_use_eff_EUR_2f, by = c("GCAM_region_ID", "sector", "fuel", "service",
                                                "subsector", "technology", "year")) %>%
      # Energy output is the product of energy consumption and efficiency
      mutate(value = value * value_eff) %>%
      # Aggregate across fuel types (by region, sector, service)
      group_by(GCAM_region_ID, sector, service, year, gcam.consumer) %>%
      summarise(value = sum(value)) %>%
      ungroup() %>%
      # explicitly set zeros for any services without values
    ...
    # Check hh files match the totals ###############################################################################

    # a) fuel consumption by HH
    data_check <- L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR %>%
      group_by(gcam.consumer, fuel, GCAM_region_ID, year) %>%
      summarise(value_hh = sum(value)) %>%
      ungroup() %>%
      left_join_error_no_match(
        L144.in_EJ_R_bld_serv_tech_F_Yh_EUR %>%
          group_by(fuel, GCAM_region_ID, year) %>%
          summarise(value = sum(value)) %>%
          ungroup(),
        by = c('fuel', 'GCAM_region_ID', 'year')
      ) %>%
      mutate(diff = value_hh - value)

    stopifnot(max(data_check$diff) == 0)
    ...
    data_check <- L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR  %>%
      ...
      left_join_error_no_match(
        L144.in_EJ_R_bld_serv_tech_F_Yh_hh_EUR %>%
          ...
          summarise(value = sum(value)) %>%
          ungroup(),
        by = c('service', 'fuel', 'GCAM_region_ID', 'year')
      ) %>%
      mutate(diff = value_ctry - value)
    ...
    L144.base_service_EJ_serv_hh_EUR %>%
      ...
      add_precursors("gcam-europe/A44.USA_TechChange_EUR", "gcam-europe/calibrated_techs_bld_det_EUR", "common/iso_GCAM_regID", "energy/A44.tech_eff_mult_RG3",
                     "energy/A_regions", "gcam-europe/A44.cost_efficiency_EUR", "common/GCAM_region_names", "L107.en_consumption_shares_EUR", "gcam-europe/A44.shares_hp_EUR") ->
      L144.base_service_EJ_serv_hh_EUR
    ...
    L144.base_service_EJ_serv_fuel_hh_EUR %>%
      ...
      add_precursors("energy/A44.USA_TechChange_EUR", "gcam-europe/calibrated_techs_bld_det_EUR", "common/iso_GCAM_regID", "energy/A44.tech_eff_mult_RG3",
                     "energy/A_regions", "gcam-europe/A44.cost_efficiency_EUR", "gcam-europe/A44.cost_efficiency_EUR", "common/GCAM_region_names",
                     "L107.en_consumption_shares_EUR", "gcam-europe/A44.shares_hp_EUR") ->
      L144.base_service_EJ_serv_fuel_hh_EUR
