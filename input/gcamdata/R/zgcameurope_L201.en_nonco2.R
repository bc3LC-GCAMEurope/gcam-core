...
#' module_gcameurope_L201.en_nonco2
#'
...
module_gcameurope_L201.en_nonco2 <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    A_regions.en <- get_data(all_data, "energy/A_regions") %>% filter_regions_europe()
    ind_subsector_revised <- get_data(all_data,"emissions/mappings/ind_subsector_revised")

    L111.nonghg_tg_R_en_S_F_Yh_EUR <- get_data(all_data, "L111.nonghg_tg_R_en_S_F_Yh_EUR", strip_attributes = TRUE)
    ...
    L112.ghg_tg_R_en_S_F_Yh_EUR <- get_data(all_data, "L112.ghg_tg_R_en_S_F_Yh_EUR", strip_attributes = TRUE)
    ...
    bind_rows(
      ...
      get_data(all_data, "gcam-europe/calibrated_techs_bld_det_EUR") %>% select(supplysector, subsector, fuel, technology, minicam.energy.input),
    ...
    # L201.en_bcoc_emissions_EUR: BC/OC emissions factors for energy technologies in all regions
    L114.bcoc_tgej_R_en_S_F_2000_EUR %>%
      filter(supplysector != "out_resources") %>%
    ...
    # L201.OutputEmissions_elec_EUR: processing is straightforward here, just move electricity rows out of the InputEmissions
    # table into the OutputEmissions
    bind_rows(filter(L201.en_pol_emissions_EUR, supplysector == "electricity"),
              filter(L201.en_ghg_emissions_EUR, supplysector == "electricity")) %>%
      select(LEVEL2_DATA_NAMES[["OutputEmissions"]]) ->
      L201.OutputEmissions_elec_EUR
    L201.en_pol_emissions_EUR <- filter(L201.en_pol_emissions_EUR, supplysector != "electricity")
    L201.en_ghg_emissions_EUR <- filter(L201.en_ghg_emissions_EUR, supplysector != "electricity")
    L201.en_bcoc_emissions_EUR <- filter(L201.en_bcoc_emissions_EUR, supplysector != "electricity")


    # L201.nonghg_max_reduction_EUR: maximum reduction for energy technologies in all regions
    L151.nonghg_ctrl_R_en_S_T_EUR %>%
      filter(supplysector != "out_resources") %>%
      # add region name
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    # L201.nonghg_steepness_EUR: steepness of reduction for energy technologies in all regions
    A51.steepness_EUR %>%
    ...
    L201.nonghg_max_reduction_EUR %>%
      full_join(L201.nonghg_steepness_EUR,
    ...
    L201.nonghg_max_reduction_EUR <- select(L201.nonghg_gdp_control, -steepness)
    L201.nonghg_steepness_EUR <- select(L201.nonghg_gdp_control, -max_reduction)

    # L201.nonghg_res_EUR: Pollutant emissions for energy resources in all regions
    L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR %>%
      filter(supplysector == "out_resources",
    ...
    # L201.ghg_res_EUR: GHG emissions from resource production in all regions
    L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR %>%
      filter(supplysector == "out_resources",
             year %in% MODEL_BASE_YEARS) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    # L201.ResReadInControl_nonghg_res_EUR: Vintage adjustments for Pollutant emissions for energy resources in all regions
    L201.nonghg_res_EUR %>%
      # copy the final historical year value to the future model periods
      bind_rows(L201.nonghg_res_EUR %>%
    ...
    L201.ResReadInControl_nonghg_res_EUR %>%
      mutate(year = MODEL_FUTURE_YEARS[1],
             emiss.coef = 0.0) %>%
      bind_rows(L201.ResReadInControl_nonghg_res_EUR) ->
      L201.ResReadInControl_nonghg_res_EUR

    # L201.ResReadInControl_ghg_res_EUR: Vintage adjustments for GHG emissions from resource production in all regions
    L201.ghg_res_EUR %>%
      # copy the final historical year value to the future model periods
      bind_rows(L201.ghg_res_EUR %>%
                  filter(year == MODEL_FINAL_BASE_YEAR) %>%
                  select(-year) %>%
                  repeat_add_columns(tibble(year = MODEL_FUTURE_YEARS))) %>%
      rename(future.emiss.coeff.year = year) %>%
    ...
    L201.ResReadInControl_ghg_res_EUR %>%
      mutate(year = MODEL_FUTURE_YEARS[1],
             emiss.coef = 0.0) %>%
      bind_rows(L201.ResReadInControl_ghg_res_EUR) ->
      L201.ResReadInControl_ghg_res_EUR


    # L201.nonghg_max_reduction_res_EUR: maximum reduction for resources in all regions
    L151.nonghg_ctrl_R_en_S_T_EUR %>%
      filter(supplysector == "out_resources") %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    # L201.nonghg_steepness_res_EUR: steepness of reduction for resources in all regions
    A51.steepness_EUR %>%
    ...
    L201.nonghg_max_reduction_res_EUR %>%
      full_join(L201.nonghg_steepness_res_EUR, by = c("region", "resource", "subresource", "technology", "year", "Non.CO2", "ctrl.name")) %>%
      na.omit %>%
      # No need to include a GDP control when the max.reduction is zero
      filter(max.reduction > 0) ->
      L201.nonghg_gdp_control_res

    L201.nonghg_max_reduction_res_EUR <- select(L201.nonghg_gdp_control_res, -steepness)
    L201.nonghg_steepness_res_EUR <- select(L201.nonghg_gdp_control_res, -max.reduction)

    # Rename to regional SO2
    L201.en_pol_emissions_EUR <- rename_SO2(L201.en_pol_emissions_EUR, A_regions, FALSE)
    L201.OutputEmissions_elec_EUR <- rename_SO2(L201.OutputEmissions_elec_EUR, A_regions, FALSE)
    L201.nonghg_max_reduction_EUR <- rename_SO2(L201.nonghg_max_reduction_EUR, A_regions, FALSE)
    L201.nonghg_steepness_EUR <- rename_SO2(L201.nonghg_steepness_EUR, A_regions, FALSE)
    L201.nonghg_res_EUR <- rename_SO2(L201.nonghg_res_EUR, A_regions, FALSE)
    ...
    A_regions.en %>%
      ...
      pull(region) ->
      distheat.regions
    L201.en_pol_emissions_EUR <- filter(L201.en_pol_emissions_EUR, supplysector != "district heat" | region %in% distheat.regions)
    L201.en_ghg_emissions_EUR <- filter(L201.en_ghg_emissions_EUR, supplysector != "district heat" | region %in% distheat.regions)
    L201.en_bcoc_emissions_EUR <- filter(L201.en_bcoc_emissions_EUR, supplysector != "district heat" | region %in% distheat.regions)
    L201.nonghg_max_reduction_EUR <- filter(L201.nonghg_max_reduction_EUR, supplysector != "district heat" | region %in% distheat.regions)
    L201.nonghg_steepness_EUR <- filter(L201.nonghg_steepness_EUR, supplysector != "district heat" | region %in% distheat.regions)
    ...
    L201.nonghg_max_reduction_EUR <- delete_nonexistent_sectors(L201.nonghg_max_reduction_EUR, L201.delete.sectors)
    L201.nonghg_steepness_EUR <- delete_nonexistent_sectors(L201.nonghg_steepness_EUR, L201.delete.sectors)
    ...
    L201.en_pol_emissions_EUR %>%
      ...
      add_legacy_name("L201.en_pol_emissions_EUR") %>%
    ...
    L201.en_ghg_emissions_EUR %>%
      ...
      add_comments("in model base years, rename to regional SO2, filter to district heat, ") %>%
      add_comments("and delete sectors with zero heating and zero cooling degree days.") %>%
      add_legacy_name("L201.en_ghg_emissions_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "energy/A_regions",
                     "energy/calibrated_techs",
                     "gcam-europe/calibrated_techs_bld_det_EUR",
    ...
    L201.en_bcoc_emissions_EUR %>%
      ...
      add_comments("in model base years, rename to regional SO2, filter to district heat, ") %>%
      add_comments("and delete sectors with zero heating and zero cooling degree days.") %>%
      add_legacy_name("L201.en_bcoc_emissions_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "energy/A_regions",
                     "energy/calibrated_techs",
                     "gcam-europe/calibrated_techs_bld_det_EUR",
    ...
    L201.nonghg_max_reduction_EUR %>%
      ...
      add_legacy_name("L201.nonghg_max_reduction_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "emissions/A_regions", "energy/A_regions",
                     "L151.nonghg_ctrl_R_en_S_T_EUR",
                     "L244.DeleteThermalService_EUR") ->
    ...
    L201.nonghg_steepness_EUR %>%
      ...
      add_legacy_name("L201.nonghg_steepness_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "emissions/A_regions", "energy/A_regions",
                     "L151.nonghg_ctrl_R_en_S_T_EUR",
                     "gcam-europe/A51.steepness_EUR",
                     "L244.DeleteThermalService_EUR") ->
      L201.nonghg_steepness_EUR
    ...
    L201.nonghg_max_reduction_res_EUR %>%
      ...
      add_comments("The maximum reduction is calculated in L151 to match the maximum emissions controls assumed in GCAM3.") %>%
      add_comments("Controls are removed when the maximum reduction is zero.") %>%
      add_legacy_name("L201.nonghg_max_reduction_res_EUR") %>%
      same_precursors_as(L201.nonghg_max_reduction_EUR) ->
      L201.nonghg_max_reduction_res_EUR
    ...
    L201.nonghg_steepness_res_EUR %>%
      ...
      add_comments("The steepness is from an assumptions file (A51_steepness). It was chosen to replicate the pollutant reduction rates for SO2 in GCAM3.") %>%
      add_comments("Steepness is removed for technologies with maximum reduction rates of 0.") %>%
      add_legacy_name("L201.nonghg_steepness_res_EUR") %>%
      same_precursors_as(L201.nonghg_steepness_EUR) ->
      L201.nonghg_steepness_res_EUR
    ...
    L201.nonghg_res_EUR %>%
      ...
      add_legacy_name("L201.nonghg_res_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "emissions/A_regions",
                     "L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR") ->
      L201.nonghg_res_EUR
    ...
    L201.ghg_res_EUR %>%
      ...
      add_legacy_name("L201.ghg_res_EUR") %>%
      add_precursors("common/GCAM_region_names",
                     "L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR") ->
      L201.ghg_res_EUR
    ...
    L201.ResReadInControl_nonghg_res_EUR %>%
      ...
      add_comments("factors match the ones in L201.nonghg_res_EUR in historical years") %>%
      same_precursors_as(L201.nonghg_res_EUR) ->
      L201.ResReadInControl_nonghg_res_EUR
    ...
    L201.ResReadInControl_ghg_res_EUR %>%
      ...
      add_units("Tg/EJ") %>%
      add_comments("Used to make per vintage adjustments to ensure overall emissions") %>%
      add_comments("factors match the ones in L201.ghg_res_EUR in historical years") %>%
