...
#' module_gcameurope_L2326.aluminum_Inc_Elas_scenarios
#'
...
#' the generated outputs: \code{L2326.IncomeElasticity_aluminum_Scen_EUR}.
...
#' @author CR 2026 Jan
module_gcameurope_L2326.aluminum_Inc_Elas_scenarios <- function(command, ...) {
  ...
  MODULE_OUTPUTS <-
    c("L2326.IncomeElasticity_aluminum_Scen_EUR")
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L102.pcgdp_thous90USD_Scen_R_Y_EUR <- L102.pcgdp_thous90USD_Scen_R_Y_EUR %>%
      ungroup() %>%
      rename(pcgdp_90thousUSD = value) %>%
      mutate(year = as.integer(year)) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID")

    L101.Pop_thous_R_Yh_EUR <- L101.Pop_thous_R_Yh_EUR %>%
    ...
    aluminum_pro_Yfut <- L1326.out_Mt_R_aluminum_Yh_EUR %>%
    ...
    # L101.Pop_thous_R_Yh_EUR because it doesn't have scenarios
    ...
    L101_Pop_hist_and_fut <- L101.Pop_thous_R_Yh_EUR %>%
    ...
    L2326.pcgdp_thous90USD_Scen_R_Y  <- L102.pcgdp_thous90USD_Scen_R_Y_EUR %>%
    ...
    L2326.IncomeElasticity_aluminum_Scen_EUR %>%
      ...
      add_legacy_name("L2326.IncomeElasticity_aluminum_Scen_EUR") %>%
      add_precursors("common/GCAM_region_names", "socioeconomics/A326.inc_elas_parameter",
                     "L102.pcgdp_thous90USD_Scen_R_Y_EUR", "L1326.out_Mt_R_aluminum_Yh_EUR") ->
      L2326.IncomeElasticity_aluminum_Scen_EUR
