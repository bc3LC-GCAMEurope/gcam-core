...
#' module_gcameurope_L1328.food_processing
#'
...
#' the generated outputs: \code{L1328.in_EJ_R_food_F_Yh_EUR}, \code{L1328.out_Pcal_R_food_Yh_EUR},
#' \code{L1328.IO_EJPcal_R_food_F_Yh_EUR},\code{L1328.in_EJ_R_indenergy_F_Yh_EUR}, \code{L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR},
#' \code{L1328.en_bal_frac_industry_food_inonspec_R_Yh_EUR}.
...
#' @author SAS Dec 2022
module_gcameurope_L1328.food_processing <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    iso_GCAM_regID <- iso_GCAM_regID %>% filter_regions_europe()
    GCAM_region_names <- GCAM_region_names %>% filter_regions_europe()
    L101.CropMeat_Food_Pcal_R_C_Y <- L101.CropMeat_Food_Pcal_R_C_Y %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L100.IEA_en_bal_ctry_hist <- L100.IEA_en_bal_ctry_hist %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    A328.energy_infill_model_intercepts_R <- A328.energy_infill_model_intercepts_R %>% filter_regions_europe(region_ID_mapping = GCAM_region_names) %>%
      bind_rows(A328.energy_infill_model_intercepts_R %>% filter(region == "default"))
    L102.gdp_mil90usd_Scen_R_Y <- L102.gdp_mil90usd_Scen_R_Y %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    ...
    L1327.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      left_join(L1328.in_EJ_R_food_F_Yh_EUR %>% select(-sector), by = c("GCAM_region_ID", "year", "fuel")) %>%
      ungroup() %>%
    ...
    if(is.null(L100.IEA_en_bal_ctry_hist)) {
      ...
      L1328.en_bal_frac_industry_food_inonspec_R_Yh_EUR <- extract_prebuilt_data("L1328.en_bal_frac_industry_food_inonspec_R_Yh_EUR")
    } else {
      ...
      verify_identical_prebuilt(L1328.en_bal_frac_industry_food_inonspec_R_Yh_EUR)
    }
    ...
    L1328.in_EJ_R_food_F_Yh_recal %>%
      ...
      filter(year %in% unique(L1328.out_Pcal_R_food_Yh_EUR$year)) %>%
      left_join_error_no_match(L1328.out_Pcal_R_food_Yh_EUR %>%
                                 rename(Pcal = value),
    ...
    L1328.in_EJ_R_indenergy_F_Yh_tmp_2_shares %>%
      ...
      # # complete for all regions setting 0
      # complete(nesting(sector, fuel),
      #          year = seq(min(R_Yh_to_infill$year),MODEL_FINAL_BASE_YEAR,1),
      #          GCAM_region_ID = unique(GCAM_region_names$GCAM_region_ID),
      #          fill = list(value = 0, share = 0, infill_total = 0, infill_fuel = 0)) ->
    ...
    L1328.in_EJ_R_food_F_Yh_recal %>%
      ...
      mutate(value = replace_na(value, 0),
             sector = replace_na(sector, "food processing"),
    ...
    L1328.in_EJ_R_food_F_Yh_EUR %>%
      ...
      left_join(L1328.out_Pcal_R_food_Yh_EUR %>% rename(output = value), by = c("GCAM_region_ID", "year")) %>%
    ...
    L1328.out_Pcal_R_food_Yh_EUR %>%
      ...
      add_legacy_name("L1328.out_Pcal_R_food_Yh_EUR") %>%
    ...
    L1328.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      add_units("EJ") %>%
      add_comments("Subtracted food processing energy use from industrial energy use values in L1327.in_EJ_R_indenergy_F_Yh_EUR") %>%
    ...
    L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR %>%
      ...
      add_units("EJ") %>%
      add_comments("From the initial adjusted industrial energy use (subtracting food processing energy use from L1327.in_EJ_R_indenergy_F_Yh_EUR)") %>%
