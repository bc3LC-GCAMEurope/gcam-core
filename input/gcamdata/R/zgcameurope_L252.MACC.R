...
#' the generated outputs: \code{L252.ResMAC_fos_EUR}, \code{L252.AgMAC_EUR}, \code{L252.MAC_an_EUR}, \code{L252.MAC_prc_EUR},
#' \code{L252.MAC_higwp_EUR}, \code{L252.ResMAC_fos_tc_average_EUR}, \code{L252.AgMAC_tc_average_EUR}, \code{L252.MAC_an_tc_average_EUR},
#' \code{L252.MAC_prc_tc_average_EUR}, \code{L252.MAC_higwp_tc_average_EUR}. The corresponding file in the
...
module_gcameurope_L252.MACC <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "emissions/A_regions",
             ...
             "L201.ghg_res_EUR",
             "L211.AGREmissions_EUR",
             "L211.AnEmissions_EUR",
             "L211.AGRBio_EUR",
             "L232.nonco2_prc_EUR",
             "L241.hfc_all_EUR",
             "L241.pfc_all_EUR"))
    ...
    return(c("L252.ResMAC_fos_EUR",
             ...
             "L252.AgMAC_tc_average_EUR",
             "L252.MAC_an_tc_average_EUR",
    ...
    L152.MAC_pct_R_S_Proc_EPA <- get_data(all_data, "L152.MAC_pct_R_S_Proc_EPA") %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L201.ghg_res_EUR <- get_data(all_data, "L201.ghg_res_EUR", strip_attributes = TRUE)
    L211.AGREmissions_EUR <- get_data(all_data, "L211.AGREmissions_EUR", strip_attributes = TRUE)
    L211.AnEmissions_EUR <- get_data(all_data, "L211.AnEmissions_EUR", strip_attributes = TRUE)
    L211.AGRBio_EUR <- get_data(all_data, "L211.AGRBio_EUR", strip_attributes = TRUE)
    L232.nonco2_prc_EUR <- get_data(all_data, "L232.nonco2_prc_EUR", strip_attributes = TRUE)
    ...
    # --------------------------------------------------------------------------------------------------------
    # L252.ResMAC_fos_EUR: Fossil resource MAC curves
    ...
    L252.ResMAC_fos_full <- L201.ghg_res_EUR %>%
    ...
    # L252.AgMAC_EUR: Agricultural abatement (including bioenergy)
    # L252.AgMAC_full contains year-specific MACs for all future modeling periods till 2050
    L252.AgMAC_full <- L211.AGREmissions_EUR %>%
      select(-input.emissions, -year) %>%
      bind_rows(L211.AGRBio_EUR %>%
    ...
    # L252.MAC_prc_EUR: Abatement from industrial and urban processes
    # L252.MAC_prc_full contains year-specific MACs for all future modeling periods till 2050
    L252.MAC_prc_full <- L232.nonco2_prc_EUR %>%
      select(-input.emissions, -year) %>%
      distinct() %>%
    ...
    # L252.MAC_higwp_EUR: Abatement from HFCs, PFCs, and SF6
    # L252.MAC_higwp_full contains year-specific MACs for all future modeling periods till 2050
    L252.MAC_higwp_full <- bind_rows(L241.hfc_all_EUR, L241.pfc_all_EUR) %>%
      select(-input.emissions, -year) %>%
      distinct() %>%
      # Add in mac.control
      # Using left_join b/c mac.control for "other industrial processes" is NA
    ...
    # base-year MAC curve
    L252.AgMAC_EUR <- L252.AgMAC_full %>%
    ...
    L252.AgMAC_tc_average_EUR <- L252.MAC_summary_TC_average %>%
      ...
      left_join_error_no_match(L252.AgMAC_EUR %>% select(-tax, -mac.reduction) %>% distinct(),
    ...
    # 2) L252.MAC_an_EUR for livestock
    # base-year MAC curve
    L252.MAC_an_EUR <- L252.MAC_an_full %>%
    ...
    # combination of sector/subsector/gas used to isolate TC rows from the combined TC file (L252.MAC_summary_TC_average)
    An_key <- L252.MAC_an_EUR %>%
      select(supplysector, subsector, Non.CO2) %>%
      distinct() %>%
      mutate(key = paste(supplysector, subsector, Non.CO2, sep = "-"))

    L252.MAC_an_tc_average_EUR <- L252.MAC_summary_TC_average %>%
      filter(key %in% An_key$key) %>%
      rename(tech.change.year = year) %>%
      left_join_error_no_match(L252.MAC_an_EUR %>% select(-tax, -mac.reduction) %>% distinct(),
    ...
    # 3) L252.MAC_prc_EUR for process emissions
    # base-year MAC curve
    L252.MAC_prc_EUR <- L252.MAC_prc_full %>%
      mutate(key = paste(region, supplysector, subsector, stub.technology, Non.CO2, year, sep = "-")) %>%
      left_join(L252.MAC_base_year, by = "key") %>%
      na.omit() %>%
      select(-key, -base.year)
    ...
    # combination of sector/subsector/gas used to isolate TC rows from the combined TC file (L252.MAC_summary_TC_average)
    Proc_key <- L252.MAC_prc_EUR %>%
      select(supplysector, subsector, Non.CO2) %>%
      distinct() %>%
      mutate(key = paste(supplysector, subsector, Non.CO2, sep = "-"))

    L252.MAC_prc_tc_average_EUR <- L252.MAC_summary_TC_average %>%
      filter(key %in% Proc_key$key) %>%
      rename(tech.change.year = year) %>%
      left_join_error_no_match(L252.MAC_prc_EUR %>% select(-tax, -mac.reduction) %>% distinct(),
                               by = c("region", "supplysector", "subsector", "stub.technology", "Non.CO2", "mac.control")) %>%
      select(region, supplysector, subsector, stub.technology, year, Non.CO2, mac.control, tech.change.year, tech.change)

    # 4) L252.MAC_higwp_EUR for F-gases
    # base-year MAC curve
    L252.MAC_higwp_EUR <- L252.MAC_higwp_full %>%
      mutate(key = paste(region, supplysector, subsector, stub.technology, Non.CO2, year, sep = "-")) %>%
      left_join(L252.MAC_base_year, by = "key") %>%
      na.omit() %>%
      select(-key, -base.year)
    ...
    # combination of sector/subsector/gas used to isolate TC rows from the combined TC file (L252.MAC_summary_TC_average)
    HighGwp_key <- L252.MAC_higwp_EUR %>%
      select(supplysector, subsector, Non.CO2) %>%
      distinct() %>%
      mutate(key = paste(supplysector, subsector, Non.CO2, sep = "-"))

    L252.MAC_higwp_tc_average_EUR <- L252.MAC_summary_TC_average %>%
      filter(key %in% HighGwp_key$key) %>%
      rename(tech.change.year = year) %>%
      left_join_error_no_match(L252.MAC_higwp_EUR %>% select(-tax, -mac.reduction) %>% distinct(),
                               by = c("region", "supplysector", "subsector", "stub.technology", "Non.CO2", "mac.control")) %>%
      select(region, supplysector, subsector, stub.technology, year, Non.CO2, mac.control, tech.change.year, tech.change)

    # 5) L252.ResMAC_fos_EUR for resource production
    # base-year MAC curve
    L252.ResMAC_fos_EUR <- L252.ResMAC_fos_full %>%
    ...
    # combination of sector/subsector/gas used to isolate TC rows from the combined TC file (L252.MAC_summary_TC_average)
    Res_key <- L252.ResMAC_fos_EUR %>%
      select(resource, subresource, Non.CO2) %>%
      distinct() %>%
      mutate(key = paste(resource, subresource, Non.CO2, sep = "-"))

    L252.ResMAC_fos_tc_average_EUR <- L252.MAC_summary_TC_average %>%
    ...
    L252.MAC_Ag_TC <- L252.AgMAC_tc_average_EUR %>%
      ...
      lapply(function(df) {
        df %>%
          ...
          add_comments("adjust L252.AgMAC_tc_average_EUR to reflect variations in SSPs") %>%
          add_comments("Technology change data added in from A_MACC_TechChange_SSP_Mult") %>%
          same_precursors_as("L252.AgMAC_tc_average_EUR") %>%
          add_precursors("emissions/A_MACC_TechChange_SSP_Mult") %>%
          select(-scenario)
      })


    L252.MAC_An_TC <- L252.MAC_an_tc_average_EUR %>%
      adjust_tech_change_ssp() %>%
      split(.$scenario) %>%
      lapply(function(df) {
        df %>%
          ...
          add_units("tax: 1990 USD; mac.reduction: % reduction; tech_change: Unitless") %>%
          add_comments("adjust L252.MAC_an_tc_average_EUR to reflect variations in SSPs") %>%
          add_comments("Technology change data added in from A_MACC_TechChange_SSP_Mult") %>%
          same_precursors_as("L252.MAC_an_tc_average_EUR") %>%
          add_precursors("emissions/A_MACC_TechChange_SSP_Mult") %>%
          select(-scenario)
      })

    L252.MAC_prc_TC <- L252.MAC_prc_tc_average_EUR %>%
      adjust_tech_change_ssp() %>%
      split(.$scenario) %>%
      lapply(function(df) {
        df %>%
          ...
          add_units("tax: 1990 USD; mac.reduction: % reduction; tech_change: Unitless") %>%
          add_comments("adjust L252.MAC_prc_tc_average_EUR to reflect variations in SSPs") %>%
          add_comments("Technology change data added in from A_MACC_TechChange_SSP_Mult") %>%
          same_precursors_as("L252.MAC_prc_tc_average_EUR") %>%
          add_precursors("emissions/A_MACC_TechChange_SSP_Mult") %>%
          select(-scenario)
      })

    L252.MAC_res_TC <- L252.ResMAC_fos_tc_average_EUR %>%
      adjust_tech_change_ssp() %>%
      split(.$scenario) %>%
      lapply(function(df) {
        df %>%
          ...
          add_units("tax: 1990 USD; mac.reduction: % reduction; tech_change: Unitless") %>%
          add_comments("adjust L252.ResMAC_fos_tc_average_EUR to reflect variations in SSPs") %>%
          add_comments("Technology change data added in from A_MACC_TechChange") %>%
          same_precursors_as("L252.ResMAC_fos_tc_average_EUR") %>%
          add_precursors("emissions/A_MACC_TechChange_SSP_Mult") %>%
          select(-scenario)
      })

    L252.MAC_higwp_TC <- L252.MAC_higwp_tc_average_EUR %>%
      adjust_tech_change_ssp() %>%
      split(.$scenario) %>%
      lapply(function(df) {
        df %>%
          ...
          add_units("tax: 1990 USD; mac.reduction: % reduction; tech_change: Unitless") %>%
          add_comments("adjust L252.MAC_higwp_tc_average_EUR to reflect variations in SSPs") %>%
          add_comments("Technology change data added in from A_MACC_TechChange") %>%
          same_precursors_as("L252.MAC_higwp_tc_average_EUR") %>%
          add_precursors("emissions/A_MACC_TechChange_SSP_Mult") %>%
          select(-scenario)
      })
    ...
    L252.ResMAC_fos_phaseInTime_EUR %>%
      ...
      add_legacy_name("L252.ResMAC_fos_phaseInTime_EUR") %>%
    ...
    L252.MAC_prc_phaseInTime_EUR %>%
      ...
      add_units("years of fully phasing in MAC reductions") %>%
      add_comments("smooth phase in for early modeling periods") %>%
      add_legacy_name("L252.MAC_prc_phaseInTime_EUR") %>%
    ...
    L252.MAC_higwp_phaseInTime_EUR %>%
      ...
      add_units("years of fully phasing in MAC reductions") %>%
      add_comments("smooth phase in for early modeling periods") %>%
      add_legacy_name("L252.MAC_higwp_phaseInTime_EUR") %>%
    ...
    return_data(L252.ResMAC_fos_EUR, L252.AgMAC_EUR, L252.MAC_an_EUR, L252.MAC_prc_EUR, L252.MAC_higwp_EUR,
                L252.ResMAC_fos_tc_average_EUR, L252.AgMAC_tc_average_EUR, L252.MAC_an_tc_average_EUR,
                L252.MAC_prc_tc_average_EUR, L252.MAC_higwp_tc_average_EUR,
                L252.ResMAC_fos_phaseInTime_EUR, L252.MAC_prc_phaseInTime_EUR, L252.MAC_higwp_phaseInTime_EUR)
  } else {
