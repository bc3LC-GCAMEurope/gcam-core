...
#' the generated outputs: \code{L154.in_EJ_R_trn_m_sz_tech_F_Yh_Eurostat}, \code{L154.in_EJ_ctry_trn_m_sz_tech_F_Eurostat},
#'  \code{L154.intensity_MJvkm_R_trn_m_sz_tech_F_Y_Eurostat}, \code{L154.loadfactor_R_trn_m_sz_tech_F_Y_Eurostat},
#'  \code{L154.cost_usdvkm_R_trn_m_sz_tech_F_Y_Eurostat}, \code{L154.speed_kmhr_R_trn_m_sz_tech_F_Y_Eurostat},
#'  The corresponding file in the original data system was \code{LA154.transportation_UCD.R} (energy level1).
#' @details L154.in_EJ_R_trn_m_sz_tech_F_Yh and L154.in_EJ_ctry_trn_m_sz_tech_F generated by aggregating Eurostat data UCD
...
module_gcameurope_L154.transportation_UCD <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L154.in_EJ_R_trn_m_sz_tech_F_Yh_Eurostat",
             "L154.in_EJ_ctry_trn_m_sz_tech_F_Eurostat",
             ...
             "L154.EUR_histfut_data_times_UCD_shares_Eurostat"))
    ...
    L101.in_EJ_ctry_trn_Fi_Yh_Eurostat <- get_data(all_data, "L101.in_EJ_ctry_trn_Fi_Yh_Eurostat")
    ...
    # NOTE: We are currently aggregating Eurostat's data (EUR) on rail and road following GCAM general procedure
    # First, replace the international shipping data (swapping in EIA for EUR)
    # Only perform this swap for international shipping / refined liquids, and in countries in the EIA database

    EUR_data_EIA_intlship <- L101.in_EJ_ctry_trn_Fi_Yh_Eurostat %>%
    ...
    # Need to map EUR sector to UCD_category, calibrated_techs_trn_agg data is too busy
    ...
    EUR_data_aggregated_by_UCD_cat <- EUR_data_EIA_intlship %>%
    ...
    ctry_iso_region <- tibble(iso = unique(L101.in_EJ_ctry_trn_Fi_Yh_Eurostat$iso)) %>%
    ...
    UCD_cat_fuel <- UCD_fuel_share_in_cat %>%
      ...
      distinct
    iso_year <- EUR_data_aggregated_by_UCD_cat %>%
      ungroup %>%
      select(iso, year) %>%
      distinct
    ...
    EUR_hist_data_times_UCD_shares <- UCD_cat_fuel %>%
      ...
      left_join(UCD_fuel_share_in_cat, by = c("UCD_category", "fuel", "iso"), multiple = "all") %>%
      fast_left_join(EUR_data_aggregated_by_UCD_cat, by = c("UCD_category", "fuel", "iso", "year")) %>%
    ...
    EUR_hist_data_times_UCD_shares_region <- EUR_hist_data_times_UCD_shares %>%
    ...
    EUR_UCD_shared_region_fuel_sum <- EUR_hist_data_times_UCD_shares_region %>%
      #kbn 2020-01-29 Adding sce below(to enable flexible use of SSPs). Changes described in detail in comment with search string,kbn 2020-03-26.
    ...
    EUR_UCD_shared_scaled_to_enduse_data <- EUR_UCD_shared_region_fuel_sum %>%
    ...
    EUR_hist_data_times_UCD_shares_region_scaled_to_enduse_data <- EUR_hist_data_times_UCD_shares_region %>%
      #kbn 2020-29-01 Add sce below. Changes described in detail in comment with search string,kbn 2020-03-26.
      left_join(EUR_UCD_shared_scaled_to_enduse_data, by = c("GCAM_region_ID", "fuel", "year")) %>%
    ...
    colnames(EUR_hist_data_times_UCD_shares_region_scaled_to_enduse_data)[colnames(EUR_hist_data_times_UCD_shares_region_scaled_to_enduse_data)=='rev.mode']<-"mode"
    colnames(EUR_hist_data_times_UCD_shares_region_scaled_to_enduse_data)[colnames(EUR_hist_data_times_UCD_shares_region_scaled_to_enduse_data)=='rev_size.class']<-"size.class"
    ...
    EUR_histfut_data_times_UCD_shares <- EUR_histfut_data_times_UCD_shares[, value := if_else(is.na(value), approx_fun(year, value, rule = 2), value),by= c("iso", "UCD_sector", "mode", "size.class", "UCD_technology", "UCD_fuel", "UCD_category", "fuel")]
    EUR_histfut_data_times_UCD_shares <- EUR_histfut_data_times_UCD_shares[,c("iso", "UCD_sector", paste(energy.TRAN_UCD_MODE), paste(energy.TRAN_UCD_SIZE_CLASS), "UCD_technology", "UCD_fuel", "UCD_category", "fuel","value","year","GCAM_region_ID"),with=FALSE]
    ...
    # Produce outputs
    EUR_hist_data_times_UCD_shares_region_scaled_to_enduse_data %>%
      ...
      add_precursors("common/iso_GCAM_regID", "L101.in_EJ_ctry_trn_Fi_Yh_Eurostat",
    ...
    EUR_hist_data_times_UCD_shares %>%
    ...
    out_var_df[["intensity_MJvkm"]] %>%
      ...
      add_precursors("energy/UCD_trn_data_SSP1","energy/UCD_trn_data_SSP3","energy/UCD_trn_data_SSP5","energy/UCD_trn_data_CORE",
                     ...
                     "L101.in_EJ_ctry_trn_Fi_Yh_Eurostat", "L1011.in_EJ_ctry_intlship_TOT_Yh",
                     "L131.in_EJ_R_Senduse_F_Yh") ->
      L154.intensity_MJvkm_R_trn_m_sz_tech_F_Y_Eurostat
    ...
    out_var_df[["cost_usdvkm"]] %>%
      ...
      same_precursors_as(L154.cost_usdvkm_R_trn_m_sz_tech_F_Y_Eurostat) ->
      L154.capcoef_usdvkm_R_trn_m_sz_tech_F_Y_Eurostat
    ...
    return_data(L154.in_EJ_R_trn_m_sz_tech_F_Yh_Eurostat, L154.in_EJ_ctry_trn_m_sz_tech_F_Eurostat,
                L154.intensity_MJvkm_R_trn_m_sz_tech_F_Y_Eurostat, L154.loadfactor_R_trn_m_sz_tech_F_Y_Eurostat,
                L154.cost_usdvkm_R_trn_m_sz_tech_F_Y_Eurostat, L154.speed_kmhr_R_trn_m_sz_tech_F_Y_Eurostat,
                L154.EUR_histfut_data_times_UCD_shares_Eurostat, L154.capcoef_usdvkm_R_trn_m_sz_tech_F_Y_Eurostat)
  } else {
