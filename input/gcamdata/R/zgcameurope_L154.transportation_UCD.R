...
module_gcameurope_L154.transportation_UCD <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    # ############## zero in countries in which no sea  ##############
    # JRC_trn_data_gather_ships <- JRC_trn_data_gather %>%
    #   filter(mode %in% c('Ship Domestic',"Ship International" )) %>%
    #   mutate(across(everything(), ~ replace_na(.x, 0)))
    # JRC_trn_data_gather_Nonships <- JRC_trn_data_gather %>%
    #   filter(!mode %in% c('Ship Domestic',"Ship International" ))
    # JRC_trn_data_gather <- bind_rows(
    #   JRC_trn_data_gather_ships,
    #   JRC_trn_data_gather_Nonships
    # )

    ################ Rail ################
    #Eliminate Metros and Urban rails
    ...
    ################ Join Diesel, gasoline and LPG into Liquids ################
    ### Rail sector
    ...
    ## 2. Load factor (We assume it is the same for all liquids)
    ...
    ## 3. Preparation for Intensity: Extract original df
    ...
    ## 4. Calculate the Service (Service = Energy / Intensity)
    service_km <- energy %>%
      select(-variable) %>%
      rename(energy = value) %>%
      left_join(
    ...
    ## 5.Add the Total Service
    total_service_km <- service_km %>%
    ...
    ##################################### ships EEA and Air EEA as domestic  #####################################
    ...
    ##################################### Road Vehicles types #####################################
    # Get different type of cars, motorcycle and trucks
    ...
    ############ Load factor from technologies that doesn't has JRC ############
    JRC_trn_data_gather_rest <-  JRC_trn_data_gather %>%
      filter(variable != 'load factor')
    ...
    ############### Liquids and Hybrid liquids intensities prop ###############
    ...
    ############ Covid #####################
    #
    # JRC_trn_data_gather_2021 <- JRC_trn_data_gather %>%
    #   filter(year == 2019) %>%
    #   mutate(year = 2021)
    # JRC_trn_data_gather_2020 <- JRC_trn_data_gather %>%
    #   filter(year == 2019) %>%
    #   mutate(year = 2020)
    # JRC_trn_data_gather <- bind_rows(JRC_trn_data_gather %>%
    #                                    filter(year != 2021, year != 2020),
    #                                  JRC_trn_data_gather_2021,
    #                                  JRC_trn_data_gather_2020)
    ...
    ## Substituting zeros in the middle of the series by the older value
    JRC_trn_data_fillout <- JRC_trn_data_fillout %>%
