...
#' module_gcameurope_L172.EFW_irrigation
#'
...
#'   \code{command} is "MAKE") all the generated outputs: \code{L172.Coef_GJm3_IrrEnergy_R_EUR},
#'   \code{L172.in_EJ_R_irr_F_Yh_EUR}.
...
module_gcameurope_L172.EFW_irrigation <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L172.Coef_GJm3_IrrEnergy_R_EUR",
    ...
    # 1. Irrigation energy coefficients ===================================================
    ...
    L172.Coef_GJm3_IrrEnergy_R_EUR <- group_by(L172.Liu_EFW_inventory, GCAM_region_ID) %>%
      summarise(ag = sum(ag),
    ...
    # 2. Electricity use in irrigation ------------------------------------
    ...
    L172.in_EJ_R_irr_F_Yh_EUR <- L165.IrrWithd_km3_R_Y %>%
      semi_join(L101.en_bal_EJ_R_Si_Fi_Yh_EUR, by = "GCAM_region_ID") %>%
      ...
      left_join_error_no_match(select(L172.Coef_GJm3_IrrEnergy_R_EUR, GCAM_region_ID, coefficient),
                               by = "GCAM_region_ID") %>%
    ...
    L172.in_EJavail_ag <- subset(L101.en_bal_EJ_R_Si_Fi_Yh_EUR,
    ...
    L172.in_EJ_R_irr_F_Yh_EUR <- left_join_error_no_match(L172.in_EJ_R_irr_F_Yh_EUR, L172.in_EJavail_ag,
    ...
    L172.Coef_GJm3_IrrEnergy_R_EUR <- select(L172.in_EJ_R_irr_F_Yh_EUR, GCAM_region_ID, sector, fuel, year, coefficient)
    L172.in_EJ_R_irr_F_Yh_EUR <- select(L172.in_EJ_R_irr_F_Yh_EUR, GCAM_region_ID, sector, fuel, year, energy_EJ)
    ...
    L172.in_EJ_R_irr_F_Yh_EUR %>%
      ...
      same_precursors_as(L172.Coef_GJm3_IrrEnergy_R_EUR) ->
      L172.in_EJ_R_irr_F_Yh_EUR

    return_data(L172.Coef_GJm3_IrrEnergy_R_EUR,
                L172.in_EJ_R_irr_F_Yh_EUR)
  } else {
