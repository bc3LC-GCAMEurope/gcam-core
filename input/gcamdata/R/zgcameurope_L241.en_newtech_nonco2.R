...
#' module_gcameurope_L241.en_newtech_nonco2
#'
...
#' the generated outputs: \code{L241.nonco2_tech_coeff_EUR}, \code{L241.nonco2_max_reduction_EUR}, \code{L241.nonco2_steepness_EUR}. The corresponding file in the
...
module_gcameurope_L241.en_newtech_nonco2 <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/GCAM_region_names",
             FILE = "emissions/A_regions",
             FILE = "energy/A_regions",
             FILE = "emissions/A41.tech_coeff",
             FILE = "gcam-europe/A51.max_reduction_EUR",
    ...
    return(c("L241.nonco2_tech_coeff_EUR",
    ...
    Emiss_A_regions <- get_data(all_data, "emissions/A_regions") %>% filter_regions_europe()
    En_A_regions <- get_data(all_data, "energy/A_regions") %>% filter_regions_europe()
    A41.tech_coeff <- get_data(all_data, "emissions/A41.tech_coeff")
    A51.max_reduction_EUR <- get_data(all_data, "gcam-europe/A51.max_reduction_EUR")
    A51.steepness_EUR <- get_data(all_data, "gcam-europe/A51.steepness_EUR")

    L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR <- get_data(all_data, "L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR")
    L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR <- get_data(all_data, "L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR")
    ...
    L241.steepness %>%
      ...
      left_join(L241.nonco2_steepness_EUR, by = c("region", "supplysector", "subsector",  "stub.technology", "Non.CO2")) %>%
      na.omit ->
      L241.nonco2_steepness_EUR

    # Rename SO2 to regional SO2.
    L241.nonco2_tech_coeff_EUR <- rename_SO2(L241.nonco2_tech_coeff_EUR, Emiss_A_regions, FALSE)
    L241.nonco2_max_reduction_EUR <- rename_SO2(L241.nonco2_max_reduction_EUR, Emiss_A_regions, FALSE)
    L241.nonco2_steepness_EUR <- rename_SO2(L241.nonco2_steepness_EUR, Emiss_A_regions, FALSE)
    ...
    # L241.OutputEmissCoeff_elec_EUR: we need to be careful with the processing here as we need to adjust the input coef
    ...
    L241.nonco2_tech_coeff_EUR %>%
      ...
      add_legacy_name("L241.nonco2_tech_coeff_EUR") %>%
    ...
    L241.nonco2_max_reduction_EUR %>%
      ...
      add_precursors("common/GCAM_region_names", "emissions/A_regions",
                     "energy/A_regions",
                     "emissions/A41.tech_coeff",
                     "gcam-europe/A51.max_reduction_EUR",
    ...
    L241.nonco2_steepness_EUR %>%
      ...
      add_legacy_name("L241.nonco2_steepness_EUR") %>%
      add_precursors("common/GCAM_region_names", "emissions/A_regions",
                     "energy/A_regions",
                     "emissions/A41.tech_coeff",
                     "gcam-europe/A51.max_reduction_EUR",
    ...
    return_data(L241.nonco2_tech_coeff_EUR, L241.OutputEmissCoeff_elec_EUR,
                L241.nonco2_max_reduction_EUR, L241.nonco2_steepness_EUR)
  } else {
