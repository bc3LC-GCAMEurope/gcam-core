...
module_gcameurope_L221.en_supply <- function(command, ...) {
  ...
  MODULE_OUTPUTS <-
    c("L221.Supplysector_en_EUR",
      ...
      "L221.PrimaryConsKeyword_en_EUR",
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    A21.sector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), has_traded = TRUE,
                           GCAM_region_names = GCAM_region_names) -> L221.Supplysector_en_EUR
    ...
    A21.sector %>%
      ...
      filter(traded == TRUE) %>%
      rename(use.trial.market = traded) -> L221.SectorUseTrialMarket_en_EUR
    ...
    A21.subsector_logit %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorLogit"]], LOGIT_TYPE_COLNAME), has_traded = TRUE,
                           GCAM_region_names = GCAM_region_names) -> L221.SubsectorLogit_en_EUR
    ...
    if(any(!is.na(A21.subsector_shrwt$year))) {
      A21.subsector_shrwt %>%
        write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorShrwt"]]), has_traded = TRUE,
                             GCAM_region_names = GCAM_region_names) %>%
        filter(!is.na(year)) -> L221.SubsectorShrwt_en_EUR
    }
    ...
    if(any(!is.na(A21.subsector_shrwt$year.fillout))) {
      A21.subsector_shrwt %>%
        write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorShrwtFllt"]]), has_traded = TRUE,
                             GCAM_region_names = GCAM_region_names) %>%
        filter(!is.na(year.fillout)) -> L221.SubsectorShrwtFllt_en_EUR
    }
    ...
    if(any(is.na(A21.subsector_interp$to.value))) {
      A21.subsector_interp %>%
        ...
        select(-to.value) -> L221.SubsectorInterp_en_EUR
    }
    ...
    if(any(!is.na(A21.subsector_interp$to.value))) {
      A21.subsector_interp %>%
        write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorInterpTo"]]), has_traded = TRUE,
                             GCAM_region_names = GCAM_region_names) %>%
        filter(!is.na(to.value)) -> L221.SubsectorInterpTo_en_EUR
    }
    ...
    A21.globaltech_shrwt %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["Tech"]]), has_traded = FALSE,
                           GCAM_region_names = GCAM_region_names) -> L221.StubTech_en_EUR
    ...
    L221.StubTech_en_EUR %>%
      ...
      rename(stub.technology = technology) -> L221.StubTech_en_EUR
    ...
    A21.globaltech_coef %>%
      ...
      select(sector.name = supplysector, subsector.name = subsector, technology, minicam.energy.input, year, coefficient) -> L221.GlobalTechCoef_en_EUR

    L221.GlobalTechCoef_en_EUR %>%
      mutate(coefficient= if_else(minicam.energy.input =="crude oil", if_else(year <= MODEL_FINAL_BASE_YEAR,1,coefficient),coefficient))->L221.GlobalTechCoef_en_EUR
    ...
    L221.StubTechCoef_bioOil_EUR <- inner_join(L221.GlobalTechCoef_en_EUR, L121.BiomassOilRatios_kgGJ_R_C_EUR, by = c(technology = "GCAM_commodity")) %>%
      ...
      mutate(coefficient = if_else(is.na(IOcoef), coefficient, IOcoef),
             market.name = if_else(!is.na(ag_region), ag_region, region)) %>%
    ...
    A21.globaltech_keyword %>%
      ...
      filter(year %in% MODEL_YEARS) -> L221.PrimaryConsKeyword_en_EUR
    ...
    L108.ag_Feed_Mt_R_C_Y %>%
      filter(GCAM_commodity %in% A_an_input_subsector$subsector,
             GCAM_commodity %in% L202.ag_consP_R_C_75USDkg_EUR$GCAM_commodity) -> L221.ag_Feed_Mt_R_C_Y
    ...
    L221.ag_Feed_Mt_R_C_Y %>%
      ...
      left_join(rename(L202.ag_consP_R_C_75USDkg_EUR, price = value),
    ...
    L221.StubTechFractSecOut_en_EUR %>%
      ...
      select(-variable, -feed_price, -GCAM_region_ID) -> L221.StubTechFractProd_en_EUR
    ...
    L221.StubTechFractSecOut_en_EUR %>%
      ...
      select(LEVEL2_DATA_NAMES[["StubTechFractCalPrice"]]) -> L221.StubTechFractCalPrice_en_EUR
    ...
    A21.rsrc_info %>%
      ...
      mutate(market = region) -> L221.Rsrc_en_EUR
    ...
    L221.SubsectorLogit_en_EUR %>%
      filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.SubsectorLogit_en_EUR

    L221.Supplysector_en_EUR %>%
      filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.Supplysector_en_EUR

    if(exists("L221.SubsectorShrwt_en_EUR")) {
      L221.SubsectorShrwt_en_EUR %>%
        filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.SubsectorShrwt_en_EUR
    }

    if(exists("L221.SubsectorShrwtFllt_en_EUR")) {
      L221.SubsectorShrwtFllt_en_EUR %>%
        filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.SubsectorShrwtFllt_en_EUR
    }

    if(exists("L221.SubsectorInterp_en_EUR")) {
      L221.SubsectorInterp_en_EUR %>%
        filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.SubsectorInterp_en_EUR
    }
    if(exists("L221.SubsectorInterpTo_en_EUR")) {
      L221.SubsectorInterpTo_en_EUR %>%
        filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.SubsectorInterpTo_en_EUR
    }

    L221.StubTech_en_EUR %>%
      filter(!(region %in% aglu.NO_AGLU_REGIONS & supplysector %in% ag_en)) -> L221.StubTech_en_EUR


    # Market name
    non_EUR_data <- L221.StubTech_en %>%
      left_join_error_no_match(L221.GlobalTechCoef_en_EUR %>%
                  distinct(sector.name, subsector.name, technology, minicam.energy.input),
                by = c("supplysector" = "sector.name",
                       "subsector" = "subsector.name",
                       "stub.technology" = "technology")) %>%
      bind_rows(L221.StubTechCalInput_bioOil %>%
                  distinct(region, supplysector, subsector, stub.technology, minicam.energy.input)) %>%
      anti_join(L221.StubTech_en_EUR, by = c("region", "supplysector", "subsector", "stub.technology")) %>%
      filter(region %in% ag_regions$region,
             !region %in% gcameurope.EUROSTAT_COUNTRIES) %>%
      repeat_add_columns(tibble(year = MODEL_YEARS))
    L221.StubTechMkt_EUR <- bind_rows(L221.StubTech_en_EUR, L221.StubTechCalInput_bioOil_EUR,
                                      ) %>%
      ...
      select(-coefficient, -trade_region, -ag_region)
    ...
    L221.Supplysector_en_EUR %>%
      ...
      add_legacy_name("L221.Supplysector_en_EUR") %>%
    ...
    L221.SectorUseTrialMarket_en_EUR %>%
      ...
      add_legacy_name("L221.SectorUseTrialMarket_en_EUR") %>%
      add_precursors("energy/A21.sector", "common/GCAM_region_names")  ->
      L221.SectorUseTrialMarket_en_EUR
    ...
    L221.SubsectorLogit_en_EUR %>%
      ...
      add_comments("removed regions with no agricultural and land use sector") %>%
      add_legacy_name("L221.SubsectorLogit_en_EUR") %>%
    ...
    if(exists("L221.SubsectorShrwt_en_EUR")) {
      L221.SubsectorShrwt_en_EUR %>%
        ...
        add_comments("removed regions with no agricultural and land use sector") %>%
        add_legacy_name("L221.SubsectorShrwt_en_EUR") %>%
    ...
    if(exists("L221.SubsectorShrwtFllt_en_EUR")) {
      L221.SubsectorShrwtFllt_en_EUR %>%
        ...
        add_comments("A21.subsector_shrwt written to all regions") %>%
        add_comments("removed regions with no agricultural and land use sector") %>%
        add_legacy_name("L221.SubsectorShrwtFllt_en_EUR") %>%
        add_precursors("energy/A21.subsector_shrwt", "common/GCAM_region_names") ->
        L221.SubsectorShrwtFllt_en_EUR
    } else {
    ...
    if(exists("L221.SubsectorInterp_en_EUR")) {
      L221.SubsectorInterp_en_EUR %>%
        ...
        add_comments("removed regions with no agricultural and land use sector") %>%
        add_legacy_name("L221.SubsectorInterp_en_EUR") %>%
    ...
    if(exists("L221.SubsectorInterpTo_en_EUR")) {
      L221.SubsectorInterpTo_en_EUR %>%
        ...
        add_comments("A21.subsector_interp written to all regions") %>%
        add_comments("removed regions with no agricultural and land use sector") %>%
        add_legacy_name("L221.SubsectorInterpTo_en_EUR") %>%
        add_precursors("energy/A21.subsector_interp", "common/GCAM_region_names") ->
        L221.SubsectorInterpTo_en_EUR
    } else {
    ...
    L221.StubTech_en_EUR %>%
      ...
      add_legacy_name("L221.StubTech_en_EUR") %>%
    ...
    L221.StubTechCoef_bioOil_EUR %>%
      ...
      add_precursors("energy/A21.globaltech_coef", "L121.BiomassOilRatios_kgGJ_R_C_EUR") ->
      L221.StubTechCoef_bioOil_EUR
    ...
    L221.PrimaryConsKeyword_en_EUR %>%
      ...
      add_legacy_name("L221.PrimaryConsKeyword_en_EUR") %>%
    ...
    L221.StubTechFractProd_en_EUR %>%
      ...
      add_comments("Prices, production of feed output from L108.ag_Feed_Mt_R_C_Y and L202.ag_consP_R_C_75USDkg_EUR") %>%
      add_legacy_name("L221.StubTechFractProd_en_EUR") %>%
      add_precursors("L108.ag_Feed_Mt_R_C_Y", "L202.ag_consP_R_C_75USDkg_EUR", "aglu/A_an_input_subsector") ->
      L221.StubTechFractProd_en_EUR
    ...
    L221.Rsrc_en_EUR %>%
      ...
      add_legacy_name("L221.Rsrc_en_EUR") %>%
    ...
    L221.RsrcPrice_en_EUR %>%
      ...
      add_legacy_name("L221.RsrcPrice_en_EUR") %>%
      same_precursors_as(L221.StubTechFractCalPrice_en_EUR) ->
      L221.RsrcPrice_en_EUR
    ...
    L221.StubTechCalInput_bioOil_EUR %>%
      ...
      add_precursors("L122.in_Mt_R_C_Yh_EUR", "energy/A21.globaltech_coef", "common/GCAM_region_names") ->
      L221.StubTechCalInput_bioOil_EUR
