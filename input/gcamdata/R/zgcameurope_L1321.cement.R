...
module_gcameurope_L1321.cement <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L102.CO2_Mt_R_F_Yh_EUR <- get_data(all_data, "L102.CO2_Mt_R_F_Yh_EUR")
    L123.in_EJ_R_elec_F_Yh_EUR <- get_data(all_data, "L123.in_EJ_R_elec_F_Yh_EUR")
    ...
    # 1. Set constants used for this chunk ===================================================
    ...
    # 2a. Derivation of cement production and limestone consumption by region in 1994 ===================================================
    ...
    # 2b cement production over time ---------------------------------------------------------------------------------------------------------
    # Calculate cement production over time using ratio of production to emissions for L1321.out_Mt_R_cement_Yh_EUR
    ...
    # 3. Calculate limestone consumption by region and fuel--------------------------------------------------
    ...
    # 4. Derive energy inputs to cement production by region and historical year ---------------------------------------------------------------------------------------------------------------
    ...
    # 5. Calculate the average electricity generation efficiencies by region to be added to L1321.Cement_ALL_ctry_Yh -----------------------------------------------------------------------------------------------------------
    ...
    # 6. intensity, fuel shares, energy for heat and electricity  --------------------------------------------------------------------------------------------------------------------------------------
    ...
    # Compile electricity, heat, and limestone IO coefficients in L1321.IO_GJkg_R_cement_F_Yh_EUR
    L1321.IO_Cement_GJkg_R_elec_Yh %>%
      bind_rows(L1321.IO_Cement_GJkg_R_heat_Yh) %>%
      bind_rows(L1321.IO_Cement_R_limestone_Yh) ->
      L1321.IO_GJkg_R_cement_F_Yh_EUR

    # 7. Calculate input energy for cement production by region, fuel, and year for L1321.in_EJ_R_cement_F_Y_EUR ---------------------------------------------------------------------------------------------------
    ...
    # 8a. Calculate remaining industrial energy use (input), subtracting cement production energy from energy balances  ------------------------------------------------------------------------------------------------------------
    ...
    # Replace NA values in sectors with no match in cement with original values from L132.in_EJ_R_indenergy_F_Yh_EUR
    L1321.in_EJ_R_indenergy_F_Yh_NAs %>%
      filter(is.na(value)) %>%
      select(-value) %>%
    ...
    # 8b. Negative values in non-bio industrial energy use are problematic and have to be zeroed out   ----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    L1321.out_Mt_R_cement_Yh_EUR %>%
      ...
      add_precursors("emissions/A_PrimaryFuelCCoef", "energy/Worrell_1994_cement", "energy/mappings/cement_regions", "L100.CDIAC_CO2_ctry_hist", "L102.CO2_Mt_R_F_Yh_EUR") ->
      L1321.out_Mt_R_cement_Yh_EUR
    ...
    L1321.IO_GJkg_R_cement_F_Yh_EUR %>%
      ...
      add_precursors("emissions/A_PrimaryFuelCCoef", "energy/Worrell_1994_cement", "energy/mappings/cement_regions", "L100.CDIAC_CO2_ctry_hist", "L102.CO2_Mt_R_F_Yh_EUR", "L123.in_EJ_R_elec_F_Yh_EUR", "L123.out_EJ_R_elec_F_Yh_EUR", "energy/IEA_cement_elec_kwht",
    ...
    L1321.in_EJ_R_cement_F_Y_EUR %>%
      ...
      add_precursors("L100.CDIAC_CO2_ctry_hist", "L102.CO2_Mt_R_F_Yh_EUR", "L123.in_EJ_R_elec_F_Yh_EUR", "L123.out_EJ_R_elec_F_Yh_EUR", "energy/IEA_cement_elec_kwht",
                     "energy/IEA_cement_TPE_GJt", "energy/IEA_cement_fuelshares", "common/iso_GCAM_regID", "L1012.en_bal_EJ_R_Si_Fi_Yh_EUR") ->
      L1321.in_EJ_R_cement_F_Y_EUR

    L1321.in_EJ_R_indenergy_F_Yh_EUR %>%
      add_title("Adjusted historical input energy balances for industrial energy use") %>%
      add_units("Exajoules") %>%
      add_comments("Subtracted cement energy use from industrial energy use values in L132.in_EJ_R_indenergy_F_Yh_EUR") %>%
      ...
      add_precursors("L100.CDIAC_CO2_ctry_hist", "L102.CO2_Mt_R_F_Yh_EUR", "L123.in_EJ_R_elec_F_Yh_EUR", "L123.out_EJ_R_elec_F_Yh_EUR", "energy/IEA_cement_elec_kwht",
                     "energy/IEA_cement_TPE_GJt", "energy/IEA_cement_fuelshares", "L132.in_EJ_R_indenergy_F_Yh_EUR", "common/iso_GCAM_regID") ->
      L1321.in_EJ_R_indenergy_F_Yh_EUR
