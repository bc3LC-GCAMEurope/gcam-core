...
#' module_gcameurope_L1321.cement
#'
...
#' the generated outputs: \code{L1321.out_Mt_R_cement_Yh_EUR}, \code{L1321.IO_GJkg_R_cement_F_Yh_EUR}, \code{L1321.in_EJ_R_cement_F_Y_EUR}, \code{L1321.in_EJ_R_indenergy_F_Yh_EUR}. The corresponding file in the
...
module_gcameurope_L1321.cement <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L1321.out_Mt_R_cement_Yh_EUR",
    ...
    L102.CO2_Mt_R_F_Yh_EUR <- get_data(all_data, "L102.CO2_Mt_R_F_Yh_EUR", strip_attributes = TRUE)
    ...
    L132.in_EJ_R_indenergy_F_Yh_EUR <- get_data(all_data, "L132.in_EJ_R_indenergy_F_Yh_EUR", strip_attributes = TRUE)
    ...
    L1321.out_Mt_C_cement_Yh_corrected %>%
      ...
      ungroup() -> L1321.out_Mt_R_cement_Yh_EUR
    ...
    L1321.in_Cement_Mt_R_limestone_Yh %>%
      left_join_error_no_match(L1321.out_Mt_R_cement_Yh_EUR, by = c("GCAM_region_ID", "sector", "year")) %>%
      mutate(value = if_else(in.value == 0, 0, in.value / value)) %>%
      select(-in.value)  %>%
      ...
      ungroup() ->
      L1321.IO_Cement_R_limestone_Yh_EUR
    ...
    elec_change_average_1971_to_2018 <-  IEA_cement_elec_kwht %>%
      ...
      left_join( L1321.out_Mt_C_cement_Yh_corrected %>%
                   filter(year == 2018) %>%
    ...
    L101.en_bal_EJ_R_Si_Fi_Yh_EUR %>%
      ...
      left_join(iso_GCAM_regID %>%
                ...
                by = "GCAM_region_ID",
                relationship = "many-to-many") -> L1321.IEA_cement_fuel_share_EUR #This is missing some countries from the other data - need to fill in later
    ...
    L123.out_EJ_R_elec_F_Yh_EUR %>%
      ...
      semi_join(L123.in_EJ_R_elec_F_Yh_EUR, by = "fuel")  %>%
      group_by(GCAM_region_ID, year) %>%
      summarise(out.value = sum(value)) %>%
      ungroup() %>%
      left_join_error_no_match(L1321.in_EJ_R_elec_Yh_EUR, by = c("GCAM_region_ID", "year")) %>%
      ...
      filter(year %in% HISTORICAL_YEARS) %>%
      # Fix Albania (make it general for any potential NAs)
      group_by(GCAM_region_ID) %>%
      mutate(
        value = if_else(
          row_number() > max(which(!is.na(value))),
          last(na.omit(value)),
          value
        )
      ) %>%
    ...
    L1321.IO_R_elec_Yh_EUR$value[L1321.IO_R_elec_Yh_EUR$value > energy.MAX_IOELEC] <- energy.MAX_IOELEC
    ...
    L1321.out_Mt_C_cement_Yh_corrected %>%
      ...
      left_join_error_no_match(L1321.IO_R_elec_Yh_EUR, by = c("GCAM_region_ID", "year")) %>%
      rename(IOelec = value) %>%
      # Match in fuelshares of (by default) coal, gas, oil, and biomass
      left_join(L1321.IEA_cement_fuel_share_EUR , by = c('iso', 'year', 'GCAM_region_ID')) %>% #Fuel shares are incomplete - so there will be NAs - correct below
    ...
    L1321.Cement_ALL_ctry_Yh %>%
      ...
      ungroup() %>%
      mutate(
        heat_GJkg = ifelse(prod_Mt > 0, heat_EJ / prod_Mt, NA_real_),
        elec_GJkg = ifelse(prod_Mt > 0, elec_EJ / prod_Mt, NA_real_)
      ) %>%
      group_by(GCAM_region_ID) %>%
      arrange(year) %>%
      fill(heat_GJkg, elec_GJkg, .direction = "downup") %>%
      ungroup() %>%
      # If country had zero production in all years → still NA → set to 0
      replace_na(list(
        heat_GJkg = 0,
        elec_GJkg = 0
      )) -> L1321.Cement_ALL_R_Yh_EUR
    ...
    L1321.Cement_ALL_R_Yh_EUR %>%
      ...
      L1321.Cement_ALL_R_Yh_base_EUR
    ...
    if(! length(ADDITIONAL_YEARS) ){
      ...
      L1321.Cement_ALL_R_Yh_base_EUR  ->
        L1321.IO_Cement_GJkg_R_ALL_Yh_EUR
      ...
      L1321.Cement_ALL_R_Yh_base_EUR %>%
        ...
        select(-old.year) %>%
        bind_rows(L1321.Cement_ALL_R_Yh_base_EUR) ->
        L1321.IO_Cement_GJkg_R_ALL_Yh_EUR }
    ...
    L1321.IO_Cement_GJkg_R_ALL_Yh_EUR %>%
      ...
      unique ->
      L1321.IO_Cement_GJkg_R_elec_Yh_EUR
    ...
    L1321.IO_Cement_GJkg_R_ALL_Yh_EUR %>%
      ...
      unique ->
      L1321.IO_Cement_GJkg_R_heat_Yh_EUR

    # Compile electricity, heat, and limestone IO coefficients in L1321.IO_GJkg_R_cement_F_Yh
    L1321.IO_Cement_GJkg_R_elec_Yh_EUR %>%
      bind_rows(L1321.IO_Cement_GJkg_R_heat_Yh_EUR) %>%
      bind_rows(L1321.IO_Cement_R_limestone_Yh_EUR)  ->
      L1321.IO_GJkg_R_cement_F_Yh_EUR
    ...
    L1321.Cement_ALL_R_Yh_EUR %>%
      ...
      L1321.in_EJ_R_cement_F_Y_base_EUR

    # Copy final year value to any historical period values not contained in the data set
    if(! length(ADDITIONAL_YEARS) ){
      L1321.in_EJ_R_cement_F_Y_base_EUR ->
        L1321.in_EJ_R_cement_F_Y_EUR  } else{
          L1321.in_EJ_R_cement_F_Y_base_EUR %>%
            filter(year == FINAL_CO2_YEAR) %>%
            rename(old.year = year) %>%
            repeat_add_columns(tibble(year = ADDITIONAL_YEARS)) %>%
            select(-old.year) %>%
            bind_rows(L1321.in_EJ_R_cement_F_Y_base_EUR) %>%
            unique ->
            L1321.in_EJ_R_cement_F_Y_EUR }
    ...
    L1321.in_EJ_R_cement_F_Y_EUR %>%
      left_join( L101.en_bal_EJ_R_Si_Fi_Yh_EUR %>%
                   filter(sector == 'cement') %>%
                   group_by(GCAM_region_ID, sector, fuel, year) %>%
                   summarize(value = sum(value) ) ,
    ...
    L1321.in_EJ_R_cement_F_Y_EUR %>%
      ...
      select(-value.new, -value.iea) %>%
      unique  -> L1321.in_EJ_R_cement_F_Y_adj_EUR

    # rename adjustment
    L1321.in_EJ_R_cement_F_Y_adj_EUR <- L1321.in_EJ_R_cement_F_Y_EUR
    ...
    L132.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      left_join(select(L1321.in_EJ_R_cement_F_Y_EUR, -sector), by = c("GCAM_region_ID", "fuel", "year")) %>%
      mutate(value = ind.value - value) %>%
      select(-ind.value) ->
      L1321.in_EJ_R_indenergy_F_Yh_NAs_EUR

    # Replace NA values in sectors with no match in cement with original values from L132.in_EJ_R_indenergy_F_Yh
    L1321.in_EJ_R_indenergy_F_Yh_NAs_EUR %>%
      filter(is.na(value)) %>%
      select(-value) %>%
      left_join(select(L132.in_EJ_R_indenergy_F_Yh_EUR, -sector), by = c("GCAM_region_ID", "fuel", "year")) %>%
      bind_rows(filter(L1321.in_EJ_R_indenergy_F_Yh_NAs_EUR, !is.na(value))) ->
      L1321.in_EJ_R_indenergy_F_Yh_negbio_EUR
    ...
    L1321.in_EJ_R_indenergy_F_Yh_negbio_EUR %>%
      ...
      bind_rows(filter(L1321.in_EJ_R_indenergy_F_Yh_negbio_EUR, value >= 0 | fuel != "biomass")) %>%
      unique ->
      L1321.in_EJ_R_indenergy_F_Yh_EUR
    ...
    L1321.cement_adj_neg_EUR %>%
      ...
      ungroup() ->
      L1321.cement_adj_pos_EUR

    # Reset the negative values to 0 in the industrial energy table
    L1321.in_EJ_R_indenergy_F_Yh_EUR[, "value"][L1321.in_EJ_R_indenergy_F_Yh_EUR[, "value"] < 0] <- 0
    L1321.in_EJ_R_indenergy_F_Yh_EUR <- L1321.in_EJ_R_indenergy_F_Yh_EUR %>% unique

    # Add in the adjustments to the table of cement energy consumption.
    L1321.in_EJ_R_cement_F_Y_EUR %>%
      bind_rows(L1321.cement_adj_neg_EUR) %>%
      bind_rows(L1321.cement_adj_pos_EUR) %>%
      group_by(GCAM_region_ID, sector, fuel, year) %>%
      summarise(value = sum(value)) %>%
      ungroup() -> L1321.in_EJ_R_cement_F_Y_EUR
    ...
    L1321.in_EJ_R_cement_F_Y_EUR <- L1321.in_EJ_R_cement_F_Y_EUR %>%
    ...
    L1321.out_Mt_R_cement_Yh_EUR %>%
      ...
      add_legacy_name("L1321.out_Mt_R_cement_Yh_EUR") %>%
      add_precursors("emissions/A_PrimaryFuelCCoef", "energy/mappings/Andrew_iso_correction",
                     "energy/Andrew_cement_production", "emissions/Andrew_cement_emissions", "L102.CO2_Mt_R_F_Yh_EUR") ->
      L1321.out_Mt_R_cement_Yh_EUR
    ...
    L1321.IO_GJkg_R_cement_F_Yh_EUR %>%
      ...
      add_legacy_name("L1321.IO_GJkg_R_cement_F_Yh_EUR") %>%
      add_precursors("emissions/A_PrimaryFuelCCoef", "L102.CO2_Mt_R_F_Yh_EUR", "L123.in_EJ_R_elec_F_Yh_EUR", "L123.out_EJ_R_elec_F_Yh_EUR",
    ...
    L1321.in_EJ_R_cement_F_Y_EUR %>%
      ...
      add_legacy_name("L1321.in_EJ_R_cement_F_Y_EUR") %>%
      add_precursors("L102.CO2_Mt_R_F_Yh_EUR", "L123.in_EJ_R_elec_F_Yh_EUR",
                     "L123.out_EJ_R_elec_F_Yh_EUR", "energy/IEA_cement_elec_kwht",
                     "energy/IEA_cement_thermal_energy_GJt_clinker_global_trend", "energy/mappings/cement_regions",
                     "energy/IEA_cement_TPE_GJt",  "common/iso_GCAM_regID", "L1012.en_bal_EJ_R_Si_Fi_Yh_EUR") ->
      L1321.in_EJ_R_cement_F_Y_EUR
    ...
    L1321.in_EJ_R_indenergy_F_Yh_EUR %>%
      ...
      add_legacy_name("L1321.in_EJ_R_indenergy_F_Yh_EUR") %>%
      add_precursors("L102.CO2_Mt_R_F_Yh_EUR", "L123.in_EJ_R_elec_F_Yh_EUR",
                     "L123.out_EJ_R_elec_F_Yh_EUR", "energy/IEA_cement_elec_kwht",
                     "energy/IEA_cement_thermal_energy_GJt_clinker_global_trend", "energy/mappings/cement_regions",
    ...
    return_data(L1321.out_Mt_R_cement_Yh_EUR, L1321.IO_GJkg_R_cement_F_Yh_EUR, L1321.in_EJ_R_cement_F_Y_EUR, L1321.in_EJ_R_indenergy_F_Yh_EUR)
  } else {
