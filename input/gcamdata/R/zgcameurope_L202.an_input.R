...
#' module_gcameurope_L202.an_input
#'
...
#' the generated outputs: \code{L202.RenewRsrc_EUR}, \code{L202.RenewRsrcPrice_EUR}, \code{L202.maxSubResource_EUR},
#' \code{L202.RenewRsrcCurves_EUR}, \code{L202.ResTechShrwt_EUR}, \code{L202.UnlimitedRenewRsrcCurves_EUR}, \code{L202.UnlimitedRenewRsrcPrice_EUR},
#' \code{L202.Supplysector_in_EUR}, \code{L202.SubsectorAll_in_EUR}, \code{L202.StubTech_in_EUR}, \code{L202.StubTechInterp_in_EUR},
#' \code{L202.StubTechProd_in_EUR}, \code{L202.Supplysector_an_EUR}, \code{L202.SubsectorAll_an_EUR}, \code{L202.StubTechInterp_an_EUR}
#' \code{L202.StubTechProd_an_EUR}, \code{L202.StubTechCoef_an_EUR}, \code{L202.StubTechCost_an_EUR},
...
#' @author CRB July 2024
module_gcameurope_L202.an_input <- function(command, ...) {
  MODULE_INPUTS <-
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L107.an_Prod_Mt_R_C_Sys_Fd_Y <- L107.an_Prod_Mt_R_C_Sys_Fd_Y %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L108.an_Feed_Mt_R_C_Sys_Fd_Y_adj <- L108.an_Feed_Mt_R_C_Sys_Fd_Y_adj %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L108.ag_Feed_Mt_R_C_Y <- L108.ag_Feed_Mt_R_C_Y %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L109.ag_ALL_Mt_R_C_Y <- L109.ag_ALL_Mt_R_C_Y %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L1321.ag_prP_R_C_75USDkg <- L1321.ag_prP_R_C_75USDkg %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L1321.an_prP_R_C_75USDkg <- L1321.an_prP_R_C_75USDkg %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L110.For_ALL_bm3_R_Y <- L110.For_ALL_bm3_R_Y %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L233.TechCoef <- L233.TechCoef %>% filter(region %in% gcameurope.EUROSTAT_COUNTRIES)
    L110.IO_Coefs_pulp <- L110.IO_Coefs_pulp %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L1321.For_Cost <- L1321.For_Cost %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)

    L110.For_ALL_bm3_R_Y <- L110.For_ALL_bm3_R_Y %>%
      filter(GCAM_commodity %in% aglu.FOREST_COMMODITIES) %>%
      group_by(GCAM_commodity, GCAM_region_ID, year) %>%
      mutate(value=sum(Prod_bm3)) %>% ungroup() %>%
      select(GCAM_commodity, GCAM_region_ID,year,value) %>%
      distinct() %>%
      mutate(GCAM_commodity= paste0(GCAM_commodity,"_processing"))
    ...
    # L202.RenewRsrc_EUR: generic resource attributes
    ...
    # L202.RenewRsrcPrice_EUR: resource prices
    L202.RenewRsrc_EUR %>%
      select(region, renewresource) %>%
      mutate(year = min(MODEL_BASE_YEARS), price = gcam.DEFAULT_PRICE) ->
      L202.RenewRsrcPrice_EUR

    # L202.maxSubResource_EUR: maximum amount of resource production allowed in any period (72-97)
    ...
    # L202.UnlimitedRenewRsrcPrice_EUR (105-112)
    ...
    # L202.Supplysector_in_EUR: generic supplysector info for inputs to animal production (114-122)
    A_an_input_supplysector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), GCAM_region_names) ->
      L202.Supplysector_in_EUR

    # L202.SubsectorAll_in_EUR: generic subsector info for inputs to animal production technologies
    A_an_input_subsector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorAll"]], LOGIT_TYPE_COLNAME), GCAM_region_names) ->
      L202.SubsectorAll_in_EUR

    # If any subsectors have a to-value provided, generate another table: L202.SubsectorInterpTo_in_EUR
    ...
    # L202.StubTech_in_EUR: identification of stub technologies for inputs to animal production (124-140)
    ...
    # L202.StubTechProd_in_EUR: base year output of the inputs (feed types) to animal production (142-149)
    A_an_input_technology %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["Tech"]], GCAM_region_names) %>%
    ...
    A_an_input_technology %>%
      ...
      mutate(share.weight.year = year,
             subs.share.weight = if_else(calOutputValue > 0, 1, 0),
             tech.share.weight = if_else(calOutputValue > 0, 1, 0)) %>%
      select(LEVEL2_DATA_NAMES[["StubTechProd"]]) ->L202.StubTechProd_in_Forest_EUR
    #Take out forest supply sectors from this
    ...
    A_an_input_technology %>%
      ...
      repeat_add_columns(tibble(year = MODEL_BASE_YEARS)) %>%
      # not every region/technology/year has a match, so need to use left_join
      left_join(L1327.IO_woodpulp_energy_EUR %>% left_join_error_no_match(GCAM_region_names, by = c("GCAM_region_ID")),
      ...
      mutate(share.weight.year = year,
             subs.share.weight = if_else(calOutputValue > 0, 1, 0),
             tech.share.weight = if_else(calOutputValue > 0, 1, 0)) %>%
      select(LEVEL2_DATA_NAMES[["StubTechProd"]]) -> L202.StubTechProd_in_pulp_energy_EUR


    # L202.Supplysector_an_EUR: generic animal production supplysector info (159-162)
    A_an_supplysector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), GCAM_region_names) ->
      L202.Supplysector_an_EUR

    # L202.SubsectorAll_an_EUR: generic animal production subsector info (164-167)
    A_an_subsector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorAll"]], LOGIT_TYPE_COLNAME), GCAM_region_names) ->
      L202.SubsectorAll_an_EUR

    # L202.StubTechInterp_an_EUR: shareweight interpolation for animal production technologies (173-175)
    A_an_technology %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["TechInterp"]], GCAM_region_names) %>%
      rename(stub.technology = technology) ->
      L202.StubTechInterp_an_EUR

    # L202.StubTechProd_an_EUR: animal production by technology and region (177-199)
    A_an_technology %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["Tech"]], GCAM_region_names) %>%
      rename(stub.technology = technology) %>%
      repeat_add_columns(tibble(year = MODEL_BASE_YEARS)) %>%
    ...
    # L202.StubTechCoef_an_EUR: animal production input-output coefficients by technology and region (201-214)
    A_an_technology %>%
    ...
    A_an_input_technology %>%
      ...
      ungroup() %>%
      select(colnames(L202.StubTechCoef_an_EUR))->L202.StubTechCoef_an_Forest
    ...
    pulp_coef_adj_final_year <- L1327.IO_woodpulp_energy_EUR %>%
    ...
    A_an_input_technology %>%
      ...
      filter(stub.technology == "woodpulp_energy") %>%
      repeat_add_columns(tibble(year = c(MODEL_BASE_YEARS, MODEL_FUTURE_YEARS))) %>%
      left_join(L1327.IO_woodpulp_energy_EUR %>%
    ...
    stub_tech_temp %>%
      ...
      ungroup() %>%
      select(colnames(L202.StubTechCoef_an_EUR)) ->
      L202.StubTechCoef_an_pulp_energy

    L202.StubTechCoef_an_EUR %>% bind_rows(L202.StubTechCoef_an_Forest) ->L202.StubTechCoef_an_EUR
    ...
    final_coef_year_data <- filter(L202.StubTechCoef_an_EUR, year == final_coef_year) %>% select(-year)
    L202.StubTechCoef_an_EUR %>%
      ...
      bind_rows(filter(L202.StubTechCoef_an_EUR, ! year > final_coef_year)) %>%
    ...
    L202.ag_consP_R_C_75USDkg_EUR <- L1321.ag_prP_R_C_75USDkg %>%
      rename(PrP = value) %>%
    ...
    L202.prP_R_C_75USDkg <- L202.ag_consP_R_C_75USDkg_EUR %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
    ...
    L202.RenewRsrc_EUR <- filter(L202.RenewRsrc_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.RenewRsrcPrice_EUR <- filter(L202.RenewRsrcPrice_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.maxSubResource_EUR <- filter(L202.maxSubResource_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.RenewRsrcCurves_EUR <- filter(L202.RenewRsrcCurves_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.ResTechShrwt_EUR <- filter(L202.ResTechShrwt_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.UnlimitedRenewRsrcCurves_EUR <- filter(L202.UnlimitedRenewRsrcCurves_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.UnlimitedRenewRsrcPrice_EUR <- filter(L202.UnlimitedRenewRsrcPrice_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.Supplysector_in_EUR <- filter(L202.Supplysector_in_EUR, !region %in% aglu.NO_AGLU_REGIONS)

    L202.SubsectorAll_in_EUR %>%
      filter(!region %in% aglu.NO_AGLU_REGIONS) %>%
      anti_join(L202.no_ddgs_regions_subs, by = c("region", "supplysector", "subsector")) ->
      L202.SubsectorAll_in_EUR
    if(exists("L202.SubsectorInterpTo_in_EUR")){
      L202.SubsectorInterpTo_in_EUR %>%
        filter(!region %in% aglu.NO_AGLU_REGIONS) %>%
        anti_join(L202.no_ddgs_regions_subs, by = c("region", "supplysector", "subsector")) ->
        L202.SubsectorInterpTo_in_EUR
    ...
    L202.Supplysector_an_EUR <- filter(L202.Supplysector_an_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.SubsectorAll_an_EUR <- filter(L202.SubsectorAll_an_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.StubTechInterp_an_EUR <- filter(L202.StubTechInterp_an_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.StubTechProd_an_EUR <- filter(L202.StubTechProd_an_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    L202.StubTechCoef_an_EUR <- filter(L202.StubTechCoef_an_EUR, !region %in% aglu.NO_AGLU_REGIONS)
    ...
    L202.RenewRsrc_EUR %>%
      ...
      add_legacy_name("L202.RenewRsrc_EUR") %>%
    ...
    L202.RenewRsrcPrice_EUR %>%
      ...
      add_legacy_name("L202.RenewRsrcPrice_EUR") %>%
      add_precursors("aglu/A_agRsrc", "common/GCAM_region_names") ->
      L202.RenewRsrcPrice_EUR
    ...
    L202.maxSubResource_EUR %>%
      ...
      add_legacy_name("L202.maxSubResource_EUR") %>%
    ...
    L202.RenewRsrcCurves_EUR %>%
      ...
      add_legacy_name("L202.RenewRsrcCurves_EUR") %>%
    ...
    L202.UnlimitedRenewRsrcCurves_EUR %>%
      ...
      add_legacy_name("L202.UnlimitedRenewRsrcCurves_EUR") %>%
    ...
    L202.UnlimitedRenewRsrcPrice_EUR %>%
      ...
      add_units("1975$/kg") %>%
      add_comments("A_agUnlimitedRsrcCurves written to all regions") %>%
      add_legacy_name("L202.UnlimitedRenewRsrcPrice_EUR") %>%
      add_precursors("aglu/A_agUnlimitedRsrcCurves", "common/GCAM_region_names") ->
      L202.UnlimitedRenewRsrcPrice_EUR
    ...
    L202.Supplysector_in_EUR %>%
      ...
      add_legacy_name("L202.Supplysector_in_EUR") %>%
    ...
    L202.SubsectorAll_in_EUR %>%
      ...
      add_legacy_name("L202.SubsectorAll_in_EUR") %>%
    ...
    if(exists("L202.SubsectorInterpTo_in_EUR")) {
      L202.SubsectorInterpTo_in_EUR %>%
        ...
        same_precursors_as(L202.SubsectorAll_in_EUR) ->
        L202.SubsectorInterpTo_in_EUR
    } else {
      missing_data()  %>%
    ...
    L202.StubTech_in_EUR %>%
      ...
      add_legacy_name("L202.StubTech_in_EUR") %>%
    ...
    L202.StubTechInterp_in_EUR %>%
      ...
      add_units("NA") %>%
      add_comments("A_an_input_technology written to all regions") %>%
      add_legacy_name("L202.StubTechInterp_in_EUR") %>%
      add_precursors("aglu/A_an_input_technology", "energy/A_regions", "common/GCAM_region_names") ->
      L202.StubTechInterp_in_EUR
    ...
    L202.StubTechProd_in_EUR %>%
      ...
      add_legacy_name("L202.StubTechProd_in_EUR") %>%
    ...
    L202.StubTechProd_in_Forest_EUR %>%
      ...
      add_units("Mt/yr") %>%
      add_comments("Calibrated primary sources of animal feed commodities, specific to each region and time period.") %>%
      add_legacy_name("L202.StubTechProd_in_Forest_EUR") %>%
      add_precursors("aglu/A_an_input_technology",
                     "energy/A_regions", "common/GCAM_region_names",
                     "L202.StubTechProd_in_EUR") ->
      L202.StubTechProd_in_Forest_EUR
    ...
    L202.Supplysector_an_EUR %>%
      ...
      add_legacy_name("L202.Supplysector_an_EUR") %>%
    ...
    L202.SubsectorAll_an_EUR %>%
      ...
      add_legacy_name("L202.SubsectorAll_an_EUR") %>%
    ...
    L202.StubTechInterp_an_EUR %>%
      ...
      add_legacy_name("L202.StubTechInterp_an_EUR") %>%
    ...
    L202.StubTechProd_an_EUR %>%
      ...
      add_legacy_name("L202.StubTechProd_an_EUR") %>%
    ...
    L202.StubTechCoef_an_EUR %>%
      ...
      add_legacy_name("L202.StubTechCoef_an_EUR") %>%
      add_precursors("aglu/A_an_technology", "L107.an_Prod_Mt_R_C_Sys_Fd_Y", "common/GCAM_region_names") ->
      L202.StubTechCoef_an_EUR
    ...
    L202.StubTechCost_an_EUR %>%
      ...
      add_legacy_name("L202.StubTechCost_an_EUR") %>%
      same_precursors_as(L202.StubTechCoef_an_EUR) %>%
    ...
    L202.StubTechCost_For_proc_EUR %>%
      ...
      add_comments("Animal feed cost, prices, and technology") %>%
      add_comments("This is the non-feed cost; i.e., all costs of producing animal commodities except for the feed.") %>%
      add_legacy_name("L202.StubTechCost_For_proc_EUR") %>%
      same_precursors_as(L202.StubTechCoef_an_EUR) %>%
    ...
    L202.StubTechProd_in_pulp_energy_EUR %>%
      ...
      add_legacy_name("L202.StubTechProd_in_pulp_energy_EUR") %>%
