...
#' module_gcameurope_L1012.en_bal_EFW
#'
#' Adjustments to the Eurostat energy balances to account for energy-for-water processes explicitly modeled.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L1012.en_bal_EJ_R_Si_Fi_Yh_EUR}.
#' @details This chunk deducts bottom-up energy-for-water (EFW) estimates from the Eurostat Energy Balances estimates of
...
module_gcameurope_L1012.en_bal_EFW <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/iso_GCAM_regID",
             ...
             "L172.in_EJ_R_irr_F_Yh_EUR",
             "L173.in_EJ_R_indEFWtot_F_Yh_EUR",
             "L174.in_EJ_R_muniEFWtot_F_Yh_EUR"))
    ...
    L171.in_EJ_R_desal_F_Yh_EUR <- get_data(all_data, "L171.in_EJ_R_desal_F_Yh_EUR", strip_attributes = TRUE)
    L172.in_EJ_R_irr_F_Yh_EUR <- get_data(all_data, "L172.in_EJ_R_irr_F_Yh_EUR", strip_attributes = TRUE)
    L173.in_EJ_R_indEFWtot_F_Yh_EUR <- get_data(all_data, "L173.in_EJ_R_indEFWtot_F_Yh_EUR", strip_attributes = TRUE)
    L174.in_EJ_R_muniEFWtot_F_Yh_EUR <- get_data(all_data, "L174.in_EJ_R_muniEFWtot_F_Yh_EUR", strip_attributes = TRUE)
    ...
    # Part 1: Desalination ----------------
    # Notes: The attribution of desal-related energy to sectors ("flows") in the Eurostat, as in IEA Energy Balances is not known.
    # The important part for incorporation into GCAM's energy system is to minimize distortions to energy balances.

    # 1.1 Compute the share of each desal-related (i.e., commercial and industrial) sector's energy by region, fuel, and year
    L1012.desal_shares <- filter(L101.en_bal_EJ_R_Si_Fi_Yh_EUR, sector %in% efw.DESAL_ENERGY_SECTORS) %>%
      filter(fuel %in% L171.in_EJ_R_desal_F_Yh_EUR$fuel) %>%
    ...
    L1012.deduction_EJ_R_desal_F_Yh_EUR <- repeat_add_columns(select(L171.in_EJ_R_desal_F_Yh_EUR, -sector),
    ...
    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR_desal <- right_join(L101.en_bal_EJ_R_Si_Fi_Yh_EUR,
                                                   L1012.deduction_EJ_R_desal_F_Yh_EUR,
                                                   by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
    ...
    stopifnot(with(filter(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR_desal,
                          initial_energy_EJ > 0,
    ...
    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR <- L101.en_bal_EJ_R_Si_Fi_Yh_EUR %>%
      left_join(select(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR_desal, GCAM_region_ID, sector, fuel, year, deduction_EJ),
                by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
      replace_na(list(deduction_EJ = 0)) %>%
      mutate(value = value - deduction_EJ) %>%
      select(-deduction_EJ)

    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR <- bind_rows(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR,
                                            select(rename(L171.in_EJ_R_desal_F_Yh_EUR, value = energy_EJ),
                                                   GCAM_region_ID, sector, fuel, year, value))

    # Part 2: Irrigation --------------
    # Irrigation-related energy consumption also does not correspond to a single sector in the Eurostat Energy Balances.
    # Using same methodology as for the IEA Energy Balance for now, described below:
    # The data include an "agriculture" line item, but most nations either have zero or very little electricity
    ...
    L1012.deduction_EJ_R_irr_F_Yh_EUR <- rename(L172.in_EJ_R_irr_F_Yh_EUR, deduction_EJ = energy_EJ) %>%
    ...
    L1012.deduction_EJ_R_mfg_F_Yh_EUR <- rename(L173.in_EJ_R_indEFWtot_F_Yh_EUR, deduction_EJ = energy_EJ) %>%
    ...
    L1012.in_EJ_R_revmfg_F_Yh_EUR <- select(L1012.deduction_EJ_R_mfg_F_Yh_EUR, GCAM_region_ID, fuel, year, from.sector, energy_EJrev) %>%
    ...
    if(!is.na(unique(L1012.deduction_EJ_R_mfg_F_Yh_EUR$from.sector.2)) &
       min(L1012.deduction_EJ_R_mfg_F_Yh_EUR$energy_EJrev / L1012.deduction_EJ_R_mfg_F_Yh_EUR$value) <
      ...
      L1012.in_EJ_R_revmfg2_F_Yh_EUR <- left_join_error_no_match(L1012.deduction_EJ_R_mfg_F_Yh_EUR,
      ...
      if(any(L1012.in_EJ_R_revmfg2_F_Yh_EUR$energy_EJ_revised < 0)){
        stop(paste0("Negative industrial energy demand from EFW incorporation in region ",
                    unique(L1012.in_EJ_R_revmfg2_F_Yh_EUR$GCAM_region_ID[L1012.in_EJ_R_revmfg2_F_Yh_EUR$energy_EJ_revised < 0]),
      ...
      L1012.in_EJ_R_revmfg_F_Yh_EUR <- bind_rows(L1012.in_EJ_R_revmfg_F_Yh_EUR, L1012.in_EJ_R_revmfg2_F_Yh_EUR)
    }

    # Append industrial EFW, and join revisions to industrial energy back into the energy balances table
    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR <- bind_rows(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR,
                                            rename(L173.in_EJ_R_indEFWtot_F_Yh_EUR, value = energy_EJ)) %>%
      left_join(L1012.in_EJ_R_revmfg_F_Yh_EUR,
                by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
      mutate(value = if_else(is.na(energy_EJ_revised), value, energy_EJ_revised)) %>%
      select(-energy_EJ_revised)
    ...
    # sector in the IEA (and Eurostat) Energy Balances: "Commercial and public services", or "COMMPUB". However, in practice the
    ...
    L1012.deduction_EJ_R_muni_F_Yh_EUR <- rename(L174.in_EJ_R_muniEFWtot_F_Yh_EUR, deduction_EJ = energy_EJ) %>%
      left_join_error_no_match(EFW_mapping_agg, by = c("sector" = "agg_sector", "fuel")) %>%
      left_join_error_no_match(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR, by = c("GCAM_region_ID", from.sector = "sector", "fuel", "year")) %>%
      left_join_error_no_match(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR, by = c("GCAM_region_ID", from.sector.2 = "sector", "fuel", "year"),
                               suffix = c(".1", ".2")) %>%
    ...
    L1012.in_EJ_R_revmuni1_F_Yh_EUR <- select(L1012.deduction_EJ_R_muni_F_Yh_EUR, GCAM_region_ID, fuel, year, from.sector, energy_EJ.1rev) %>%
      rename(sector = from.sector, energy_EJ_revised = energy_EJ.1rev)
    L1012.in_EJ_R_revmuni_F_Yh_EUR <- select(L1012.deduction_EJ_R_muni_F_Yh_EUR, GCAM_region_ID, fuel, year, from.sector.2, energy_EJ.2rev) %>%
      rename(sector = from.sector.2, energy_EJ_revised = energy_EJ.2rev) %>%
      bind_rows(L1012.in_EJ_R_revmuni1_F_Yh_EUR)
    ...
    L1012.en_bal_EJ_R_Si_Fi_Yh_EUR <- bind_rows(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR,
                                            rename(L174.in_EJ_R_muniEFWtot_F_Yh_EUR, value = energy_EJ)) %>%
      left_join(L1012.in_EJ_R_revmuni_F_Yh_EUR, by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
      mutate(value = if_else(is.na(energy_EJ_revised), value, energy_EJ_revised)) %>%
      select(-energy_EJ_revised)
    ...
    return_data(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR)
  } else {
