...
#' the generated outputs: \code{L110.water_demand_primary_R_S_W_m3_GJ_EUR}. The corresponding file in the
...
module_gcameurope_L110.water_demand_primary <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L110.water_demand_primary_R_S_W_m3_GJ_EUR",
             "L110.in_km3_water_primary_EUR",
             "L110.in_km3_water_primary_basin_EUR"))
    ...
    L121.in_EJ_R_TPES_crude_Yh_EUR <- get_data(all_data, "L121.in_EJ_R_TPES_crude_Yh_EUR")
    ...
    # Get water consumption and withdrawal ratios
    GCAM_region_names %>%
      mutate(rsrc_region = "Western Europe") %>%
      filter_regions_europe() %>%
    ...
    L110.energy_flow_coal <- subset(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR, sector == "TPES" & fuel == "coal") %>%
      mutate(supplysector = "regional coal") %>%
      select(GCAM_region_ID, supplysector, year, value)
    L110.energy_flow_oil <- L121.in_EJ_R_TPES_crude_Yh_EUR %>%
      mutate(supplysector = "regional oil") %>%
      select(GCAM_region_ID, supplysector, year, value)
    L110.energy_flow_unoilprod <- subset(L121.in_EJ_R_TPES_unoil_Yh_EUR, fuel == "unconventional oil") %>%
      mutate(supplysector = "regional oil") %>%
      select(GCAM_region_ID, supplysector, year, value)
    L110.energy_flow_gas <- subset(L122.in_EJ_R_gasproc_F_Yh_EUR, fuel == "gas") %>%
    ...
    L110.energy_flow_nuc <- subset(L1012.en_bal_EJ_R_Si_Fi_Yh_EUR, fuel == "elec_nuclear" & sector == "out_electricity generation") %>%
    ...
    L110.in_km3_water_primary_EUR <- bind_rows(L110.energy_flow_coal, L110.energy_flow_oil, L110.energy_flow_unoilprod,
    ...
    L110.in_km3_water_primary_basin_EUR <- L110.in_km3_water_primary_EUR %>%
    ...
    L110.water_demand_primary %>%
      ...
      add_legacy_name("L110.water_demand_primary_R_S_W_m3_GJ_EUR") %>%
    ...
    L110.in_km3_water_primary_EUR %>%
      ...
      same_precursors_as(L110.water_demand_primary_R_S_W_m3_GJ_EUR) %>%
    ...
    L110.in_km3_water_primary_basin_EUR %>%
      ...
      same_precursors_as(L110.in_km3_water_primary_EUR) %>%
      add_precursors("L103.water_mapping_R_B_W_Ws_share") ->
      L110.in_km3_water_primary_basin_EUR

    return_data(L110.water_demand_primary_R_S_W_m3_GJ_EUR, L110.in_km3_water_primary_EUR, L110.in_km3_water_primary_basin_EUR)
