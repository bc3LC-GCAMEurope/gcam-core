...
#' module_gcameurope_L2392.gas_trade
#'
...
#'   \code{command} is "MAKE") all the generated outputs:\code{L2392.Delete_Supplysector_tra_NG_EUR},
#'   \code{L2392.Delete_Supplysector_reg_NG_EUR}, \code{L2392.PrimaryConsKeyword_en_NG_EUR}, \code{L2392.CarbonCoef_NG_EUR},
#'   \code{L2392.Supplysector_tra_NG_EUR},\code{L2392.SectorUseTrialMarket_tra_NG_EUR}, \code{L2392.SubsectorAll_tra_NG_EUR},
#'   \code{L2392.TechShrwt_tra_NG_EUR}, \code{L2392.TechCost_tra_NG_EUR},\code{L2392.TechLifetime_tra_NG_EUR},\code{L2392.TechSCurve_tra_NG_EUR},
#'   \code{L2392.TechCoef_tra_NG_EUR}, \code{L2392.ProfitShutdown_tra_NG_EUR},
#'   \code{L2392.Supplysector_reg_NG_EUR}, \code{2392.NestingSubsectorAll_reg_NG}, \code{L2392.SubsectorAll_reg_NG_EUR},
#'   \code{L2392.TechShrwt_reg_NG_EUR},\code{L2392.TechCoef_reg_NG_EUR},\code{L2392.TechCost_reg_NG_EUR}, \code{L2392.TechLifetime_reg_NG_EUR},
#'   \code{L2392.TechSCurve_reg_NG_EUR},\code{L2392.ProfitShutdown_reg_NG_EUR}, \code{L2392.TechInterp_reg_NG_EUR},
#'   \code{L2392.Production_tra_NG_EUR}, \code{L2392.Production_reg_imp_NG_EUR}, \code{L2392.Production_reg_dom_NG_EUR}
...
#' @author RH Feb 2024
#'
module_gcameurope_L2392.gas_trade <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2392.PrimaryConsKeyword_en_NG_EUR <- bind_rows(L2392.PrimaryConsKeyword_en_domestic_LNG,
    ...
    # L2392.SectorUseTrialMarket_tra_NG_EUR: Create solved markets for the traded sectors
    ...
    L2392.SubsectorAll_tra_stat_diff_exports <- A_ff_TradedSubsector_NG %>%
      ...
      right_join(L2391.NG_export_calOutput_statdiff_EUR, by = c("region", "supplysector" = "sector", "subsector" = "technology",
    ...
    NG_import_calOutput_statdiff_pipelines <- L2391.NG_import_calOutput_statdiff_EUR %>%
    ...
    NG_export_calOutput_statdiff_regions <- L2391.NG_export_calOutput_statdiff_EUR %>%
    ...
    # L2392.TechShrwt_tra_NG_EUR: Share-weights of traded technologies
    L2392.TechShrwt_tra_NG_EUR <- select(A_ff_TradedTechnology_R_Y_NG, LEVEL2_DATA_NAMES[["TechShrwt"]])

    # L2392.TechCost_tra_NG_EUR: Costs of traded technologies
    ...
    L2392.TechCost_tra_NG_EUR <- A_ff_TradedTechnology_R_Y_NG %>%
    ...
    L2392.Production_tra_pipeline <- filter(A_ff_TradedTechnology_R_Y_pipeline, year %in% MODEL_BASE_YEARS) %>%
      left_join_error_no_match(L2391.NG_export_calOutput_pipeline_EUR,
                               by = c(market.name = "region", minicam.energy.input = "GCAM_Commodity", "year")) %>%
      mutate(calOutputValue = round(calOutputValue, energy.DIGITS_CALOUTPUT),
             share.weight.year = year,
             subs.share.weight = if_else(calOutputValue > 0, 1, 0),
    ...
    L2392.Production_tra_stat_diff_exports <- filter(A_ff_TradedTechnology_R_Y_stat_diff_exports, year %in% MODEL_BASE_YEARS) %>%
      ...
      left_join(select(L2391.NG_export_calOutput_statdiff_EUR, -subsector),
    ...
    L2392.Production_tra_stat_diff_imports <- filter(A_ff_TradedTechnology_R_Y_stat_diff_imports, year %in% MODEL_BASE_YEARS) %>%
      left_join_error_no_match(L2391.NG_import_calOutput_statdiff_EUR,
    ...
    L2392.Production_tra_NG_EUR <- bind_rows(L2392.Production_tra_LNG,
    ...
    # L2392.SubsectorAll_reg_NG_EUR: generic subsector info for regional natural gas  commodities (competing imported LNG vs. pipeline)
    L2392.SubsectorAll_reg_NG_EUR <- write_to_all_regions(A_ff_RegionalSubsector_NG,
    ...
    # L2392.TechShrwt_reg_NG_EUR: Share-weights of regional technologies
    L2392.TechShrwt_reg_NG_EUR <- select(A_ff_RegionalTechnology_R_Y_NG, c(LEVEL2_DATA_NAMES[["TechShrwt"]], "subsector0"))

    # L2392.TechCoef_reg_NG_EUR: Coefficient and market name of regional technologies
    L2392.TechCoef_reg_NG_EUR <- select(A_ff_RegionalTechnology_R_Y_NG, c(LEVEL2_DATA_NAMES[["TechCoef"]],"subsector0"))

    # L2392.TechInterp_reg_NG_EUR : Interpolation rules for regional technologies
    L2392.TechInterp_reg_NG_EUR <- select(A_ff_RegionalTechnology_R_Y_NG, c(LEVEL2_DATA_NAMES[["TechInterpTo"]], "subsector0"))

    # L2392.TechCost_reg_NG_EUR: Costs of regional technologies
    ...
    L2392.TechCost_reg_NG_EUR <- A_ff_RegionalTechnology_R_Y_NG %>%
    ...
    L2392.Production_reg_imp_pipeline <- A_ff_RegionalTechnology_R_Y_pipeline %>%
      filter(year %in% MODEL_BASE_YEARS) %>%
      left_join_error_no_match(L2391.NG_import_calOutput_pipeline_EUR,
    ...
    L2392.Production_reg_dom_NG_EUR <- L239.Production_reg_dom %>%
      filter(supplysector == "regional natural gas") %>%
