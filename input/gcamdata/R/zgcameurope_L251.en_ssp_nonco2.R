...
#' module_gcameurope_L251.en_ssp_nonco2
#'
...
#' the generated outputs: \code{L251.ctrl.delete_EUR}, \code{L251.ssp15_ef_EUR}, \code{L251.ssp2_ef_EUR}, \code{L251.ssp2_ef_residTradBio_EUR}, \code{L251.ssp34_ef_EUR}, \code{L251.ssp15_ef_vin_EUR}, \code{L251.ssp2_ef_vin_EUR}, \code{L251.ssp34_ef_vin_EUR}. The corresponding file in the
...
#' @author CRB June 2024
module_gcameurope_L251.en_ssp_nonco2 <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L251.ctrl.delete_EUR",
             ...
             "L251.ssp34_ef_elec_EUR",
             "L251.ssp15_ef_vin_EUR",
             "L251.ssp2_ef_vin_EUR",
             "L251.ssp34_ef_vin_EUR",
    ...
    get_data(all_data, "emissions/A_regions") %>% filter_regions_europe() ->
      A_regions
    get_data(all_data, "L161.SSP2_EF_EUR") ->
      L161.SSP2_EF_EUR
    get_data(all_data, "L161.SSP15_EF_EUR") ->
      L161.SSP15_EF_EUR
    get_data(all_data, "L161.SSP34_EF_EUR") ->
      L161.SSP34_EF_EUR
    ...
    bind_rows(
      ...
      get_data(all_data, "gcam-europe/calibrated_techs_bld_det_EUR") %>% select(supplysector, subsector, technology, minicam.energy.input),
    ...
    GAINS_years <- unique(L161.SSP15_EF_EUR$year)
    ...
    L251.ssp15_ef_EUR %>%
      ...
      L251.ssp15_ef_elec_EUR
    L251.ssp15_ef_EUR <- filter(L251.ssp15_ef_EUR, supplysector != "electricity")
    L251.ssp2_ef_EUR %>%
      filter(supplysector == "electricity") %>%
      left_join_error_no_match(L223.GlobalTechEff_elec, by = c("supplysector" = "sector.name",
                                                               "subsector" = "subsector.name",
      ...
      select(LEVEL2_DATA_NAMES[["OutputEmissCoeff"]]) ->
      L251.ssp2_ef_elec_EUR
    L251.ssp2_ef_EUR <- filter(L251.ssp2_ef_EUR, supplysector != "electricity")
    L251.ssp34_ef_EUR %>%
      filter(supplysector == "electricity") %>%
      left_join_error_no_match(L223.GlobalTechEff_elec, by = c("supplysector" = "sector.name",
                                                               "subsector" = "subsector.name",
      ...
      select(LEVEL2_DATA_NAMES[["OutputEmissCoeff"]]) ->
      L251.ssp34_ef_elec_EUR
    L251.ssp34_ef_EUR <- filter(L251.ssp34_ef_EUR, supplysector != "electricity")
    ...
    L251.ctrl.delete_EUR <- rename_SO2(L251.ctrl.delete_EUR, A_regions, FALSE)
    L251.ssp15_ef_EUR <- rename_SO2(L251.ssp15_ef_EUR, A_regions, FALSE)
    L251.ssp2_ef_EUR <- rename_SO2(L251.ssp2_ef_EUR, A_regions, FALSE)
    L251.ssp34_ef_EUR <- rename_SO2(L251.ssp34_ef_EUR, A_regions, FALSE)
    L251.ssp15_ef_elec_EUR <- rename_SO2(L251.ssp15_ef_elec_EUR, A_regions, FALSE)
    L251.ssp2_ef_elec_EUR <- rename_SO2(L251.ssp2_ef_elec_EUR, A_regions, FALSE)
    L251.ssp34_ef_elec_EUR <- rename_SO2(L251.ssp34_ef_elec_EUR, A_regions, FALSE)
    L251.ssp15_ef_vin_EUR <- rename_SO2(L251.ssp15_ef_vin_EUR, A_regions, FALSE)
    L251.ssp2_ef_vin_EUR <- rename_SO2(L251.ssp2_ef_vin_EUR, A_regions, FALSE)
    L251.ssp34_ef_vin_EUR <- rename_SO2(L251.ssp34_ef_vin_EUR, A_regions, FALSE)

    # This section removes historical years (e.g., 2010) to ensure consistency with the CORE.
    L251.ssp15_ef_EUR <- L251.ssp15_ef_EUR %>% filter(year %in% MODEL_FUTURE_YEARS)
    L251.ssp2_ef_EUR <- L251.ssp2_ef_EUR %>% filter(year %in% MODEL_FUTURE_YEARS)
    L251.ssp34_ef_EUR <- L251.ssp34_ef_EUR %>% filter(year %in% MODEL_FUTURE_YEARS)
    L251.ssp15_ef_elec_EUR <- L251.ssp15_ef_elec_EUR %>% filter(year %in% MODEL_FUTURE_YEARS)
    L251.ssp2_ef_elec_EUR <- L251.ssp2_ef_elec_EUR %>% filter(year %in% MODEL_FUTURE_YEARS)
    L251.ssp34_ef_elec_EUR <- L251.ssp34_ef_elec_EUR %>% filter(year %in% MODEL_FUTURE_YEARS)
    L251.ssp15_ef_vin_EUR <- L251.ssp15_ef_vin_EUR %>% filter(future.emiss.coeff.year %in% MODEL_FUTURE_YEARS)
    L251.ssp2_ef_vin_EUR <- L251.ssp2_ef_vin_EUR %>% filter(future.emiss.coeff.year %in% MODEL_FUTURE_YEARS)
    L251.ssp34_ef_vin_EUR <- L251.ssp34_ef_vin_EUR %>% filter(future.emiss.coeff.year %in% MODEL_FUTURE_YEARS)

    # This section performs a filtering join that discards rows that do not have a SSP emission GDP control steepness value.
    L251.ctrl.delete_EUR %>%
      semi_join(L201.nonghg_steepness_EUR,
                by = c("region", "supplysector", "subsector", "stub.technology", "Non.CO2")) ->
      L251.ctrl.delete_EUR

    # Write SSP2 EFs for TradBio
    L251.ssp2_ef_residTradBio_EUR <- L251.ssp2_ef_EUR %>%
    ...
    L251.ctrl.delete_EUR %>%
      ...
      add_legacy_name("L251.ctrl.delete_EUR") %>%
      add_precursors("L161.SSP2_EF_EUR",
                     "L201.nonghg_steepness_EUR",
                     "emissions/A_regions") ->
      L251.ctrl.delete_EUR
    L251.ssp15_ef_EUR %>%
      ...
      add_legacy_name("L251.ssp15_ef_EUR") %>%
    ...
    L251.ssp2_ef_EUR %>%
      ...
      add_comments("Then, regional non-CO2 emission species information is added.") %>%
      add_legacy_name("L251.ssp2_ef_EUR") %>%
      add_precursors("L161.SSP2_EF_EUR",
                     "emissions/A_regions",
                     "energy/calibrated_techs",
                     "gcam-europe/calibrated_techs_bld_det_EUR",
    ...
    L251.ssp34_ef_EUR %>%
      ...
      add_comments("Then, regional non-CO2 emission species information is added.") %>%
      add_legacy_name("L251.ssp34_ef_EUR") %>%
      add_precursors("L161.SSP34_EF_EUR",
                     "emissions/A_regions",
                     "energy/calibrated_techs",
                     "gcam-europe/calibrated_techs_bld_det_EUR",
                     UCD_tech_map_name) ->
    ...
    L251.ssp15_ef_vin_EUR %>%
      ...
      add_legacy_name("L251.ssp15_ef_vin_EUR") %>%
      add_precursors("L161.SSP15_EF_EUR",
                     "emissions/A_regions") ->
      L251.ssp15_ef_vin_EUR
    L251.ssp2_ef_vin_EUR %>%
      ...
      add_comments("Then, emissions controls are added for future years of vintaged technologies for SSP emission factors.") %>%
      add_comments("Finally, regional non-CO2 emission species information is added.") %>%
      add_legacy_name("L251.ssp2_ef_vin_EUR") %>%
      add_precursors("L161.SSP2_EF_EUR",
                     "emissions/A_regions") ->
      L251.ssp2_ef_vin_EUR
    L251.ssp34_ef_vin_EUR %>%
      ...
      add_comments("Then, emissions controls are added for future years of vintaged technologies for SSP emission factors.") %>%
      add_comments("Finally, regional non-CO2 emission species information is added.") %>%
      add_legacy_name("L251.ssp34_ef_vin_EUR") %>%
      add_precursors("L161.SSP34_EF_EUR",
                     "emissions/A_regions") ->
      L251.ssp34_ef_vin_EUR

    return_data(L251.ctrl.delete_EUR, L251.ssp15_ef_EUR, L251.ssp2_ef_EUR, L251.ssp2_ef_residTradBio_EUR,
                L251.ssp34_ef_EUR, L251.ssp15_ef_elec_EUR, L251.ssp2_ef_elec_EUR, L251.ssp34_ef_elec_EUR,
                L251.ssp15_ef_vin_EUR, L251.ssp2_ef_vin_EUR, L251.ssp34_ef_vin_EUR)
  } else {
