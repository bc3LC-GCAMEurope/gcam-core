...
#' module_gcameurope_L1231.elec_tech
#'
...
#' the generated outputs: \code{L1231.in_EJ_R_elec_F_tech_Yh_EUR}, \code{L1231.out_EJ_R_elec_F_tech_Yh_EUR}, \code{L1231.eff_R_elec_F_tech_Yh_EUR}. The corresponding file in the
...
module_gcameurope_L1231.elec_tech <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L1231.in_EJ_R_elec_F_tech_Yh_EUR",
    ...
    L123.eff_R_elec_F_Yh_EUR <- get_data(all_data, "L123.eff_R_elec_F_Yh_EUR", strip_attributes = TRUE)
    ...
    L123.eff_R_elec_F_Yh_EUR %>%
      filter(fuel == "gas") %>%
      rename(efficiency = value) -> L1231.eff_R_elec_gas_Yh_EUR_gathered
    ...
    L1231.eff_R_elec_gas_Yh_EUR_gathered %>%
      ...
      mutate(efficiency_tech1 = if_else(efficiency < efficiency_tech1, efficiency, efficiency_tech1),
             efficiency_tech2 = if_else(efficiency > efficiency_tech2, efficiency + ELEC_ADJ, efficiency_tech2)) -> L1231.eff_R_elec_gas_Yh_EUR_gathered
    ...
    L1231.eff_R_elec_gas_Yh_EUR_gathered %>%
      ...
      rename(efficiency = efficiency_tech1) -> L1231.eff_R_elec_gas_tech1_Yh_EUR_gathered
    ...
    L1231.eff_R_elec_gas_Yh_EUR_gathered %>%
      ...
      bind_rows(L1231.eff_R_elec_gas_tech1_Yh_EUR_gathered) %>%
      select(GCAM_region_ID, sector, fuel, technology, year, efficiency) -> L1231.eff_R_elec_gas_tech_Yh_EUR

    # Solve for share of first technology to be used to estimate gas technology inputs
    L1231.eff_R_elec_gas_Yh_EUR_gathered %>%
      mutate(share_tech1 = (efficiency - efficiency_tech2) / (efficiency_tech1 - efficiency_tech2)) -> L1231.eff_R_elec_gas_Yh_EUR_gathered

    # Use share obtained above to get inputs of gas by technology
    L1231.eff_R_elec_gas_Yh_EUR_gathered %>%
      select(GCAM_region_ID, sector, fuel, year, share_tech1) %>%
      full_join(filter(L123.in_EJ_R_elec_F_Yh_EUR, fuel == "gas"), by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
      rename(in_EJ = value) %>%
      mutate(in_EJ_tech1 = in_EJ*share_tech1,
             in_EJ_tech2 = in_EJ - in_EJ_tech1) -> L1231.in_EJ_R_elec_gas_Yh_EUR_gathered

    # Combined gas technologies (inputs) in long format
    L1231.in_EJ_R_elec_gas_Yh_EUR_gathered %>%
      select(-in_EJ, -share_tech1, -in_EJ_tech2) %>%
      mutate(technology = GAS_TECH1) %>%
      rename(value = in_EJ_tech1) -> L1231.eff_R_elec_gas_tech1_Yh_EUR_gathered
    ...
    L1231.in_EJ_R_elec_gas_Yh_EUR_gathered %>%
      ...
      bind_rows(L1231.eff_R_elec_gas_tech1_Yh_EUR_gathered) %>%
      select(GCAM_region_ID, sector, fuel, technology, year, value) -> L1231.in_EJ_R_elec_gas_tech_Yh_EUR
    ...
    L123.eff_R_elec_F_Yh_EUR %>%
      ...
      bind_rows(L1231.eff_R_elec_gas_tech_Yh_EUR) %>%
      select(GCAM_region_ID, sector, fuel, technology, year, efficiency) -> L1231.eff_R_elec_F_tech_Yh_EUR
    ...
    L123.in_EJ_R_elec_F_Yh_EUR %>%
      filter(fuel != "gas") %>%
      left_join(select(calibrated_techs, sector, fuel, technology), by = c("sector", "fuel")) %>%
      bind_rows(L1231.in_EJ_R_elec_gas_tech_Yh_EUR) %>%
      select(GCAM_region_ID, sector, fuel, technology, year, value) -> L1231.in_EJ_R_elec_F_tech_Yh_EUR

    # Calculate output as input * efficiency
    # This tibble only includes technologies with modeled efficiencies
    L1231.eff_R_elec_F_tech_Yh_EUR %>%
      left_join(L1231.in_EJ_R_elec_F_tech_Yh_EUR, by = c("GCAM_region_ID", "sector", "fuel", "technology", "year")) %>%
      mutate(output = efficiency*value) %>%
      select(-value, -efficiency) -> L1231.out_EJ_R_elec_Fin_tech_Yh_EUR

    # Rename efficiecy as value for final output in long format so that data will be reshaped correctly by testing code
    L1231.eff_R_elec_F_tech_Yh_EUR %>%
      rename(value = efficiency) -> L1231.eff_R_elec_F_tech_Yh_EUR
