...
#' module_gcameurope_L231.proc_sector
#'
...
#' the generated outputs: \code{L231.UnlimitRsrc_EUR}, \code{L231.UnlimitRsrcPrice_EUR}, \code{L231.FinalDemand_urb_EUR}, \code{L231.SectorLogitTables[[ curr_table ]]$data}, \code{L231.Supplysector_urb_ind_EUR}, \code{L231.SubsectorLogitTables[[ curr_table ]]$data}, \code{L231.SubsectorLogit_urb_ind_EUR}, \code{L231.SubsectorShrwt_urb_ind_EUR}, \code{L231.SubsectorShrwtFllt_urb_ind_EUR}, \code{L231.SubsectorInterp_urb_ind_EUR}, \code{L231.SubsectorInterpTo_urb_ind_EUR}, \code{L231.StubTech_urb_ind_EUR}, \code{L231.GlobalTechShrwt_urb_ind_EUR}, \code{L231.GlobalTechEff_urb_ind_EUR}, \code{L231.GlobalTechCoef_urb_ind_EUR}, \code{L231.GlobalTechCost_urb_ind_EUR}, \code{L231.RegionalTechCalValue_urb_ind_EUR}, \code{L231.IndCoef_EUR}. The corresponding file in the
...
module_gcameurope_L231.proc_sector <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    A_regions <- get_data(all_data, "emissions/A_regions") %>% filter_regions_europe()
    ...
    # ===================================================
    # L231.FinalDemand_urb_EUR: Final demand information for urban processes sector
    L231.FinalDemand_urb_EUR <- tibble(region = A_regions$region,
    ...
    L231.Supplysector_urb_ind_EUR <- A31.sector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), GCAM_region_names = GCAM_region_names )

    # 2b. Subsector information
    # L231.SubsectorLogit_urb_ind_EUR: Subsector logit exponents of urban & industrial processes sectors
    L231.SubsectorLogit_urb_ind_EUR <- A31.subsector_logit %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorLogit"]], LOGIT_TYPE_COLNAME), GCAM_region_names = GCAM_region_names)

    # L231.SubsectorShrwt_urb_ind_EUR and L231.SubsectorShrwtFllt_urb_ind_EUR: Subsector shareweights of urban & industrial processes sectors
    if(any(!is.na(A31.subsector_shrwt$year))) {
      L231.SubsectorShrwt_urb_ind_EUR <- A31.subsector_shrwt %>%
        filter(!is.na(year)) %>%
    ...
    if(any(!is.na(A31.subsector_shrwt$year.fillout))) {
      L231.SubsectorShrwtFllt_urb_ind_EUR <- A31.subsector_shrwt %>%
        filter(!is.na(year.fillout)) %>%
        write_to_all_regions(LEVEL2_DATA_NAMES[["SubsectorShrwtFllt"]], GCAM_region_names = GCAM_region_names)
    }

    # L231.SubsectorInterp_urb_ind_EUR and L231.SubsectorInterpTo_urb_ind_EUR: Subsector shareweight interpolation of urban & industrial processes sector
    if(any(is.na(A31.subsector_interp$to.value))) {
      L231.SubsectorInterp_urb_ind_EUR <- A31.subsector_interp %>%
        filter(is.na(to.value)) %>%
        write_to_all_regions(LEVEL2_DATA_NAMES[["SubsectorInterp"]], GCAM_region_names = GCAM_region_names)
    }
    if(any(!is.na(A31.subsector_interp$to.value))) {
      L231.SubsectorInterpTo_urb_ind_EUR <- A31.subsector_interp %>%
        filter(!is.na(to.value)) %>%
        write_to_all_regions(LEVEL2_DATA_NAMES[["SubsectorInterpTo"]], GCAM_region_names = GCAM_region_names)
    }

    # Technology information
    # L231.StubTech_urb_ind_EUR: Identification of stub technologies of urban & industrial processes sectors
    # Note: assuming that technology list in the shareweight table includes the full set (any others would default to a 0 shareweight)
    L231.StubTech_urb_ind_EUR <- A31.globaltech_shrwt %>%
      select(supplysector, subsector, technology) %>%
      distinct %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["Tech"]] , GCAM_region_names = GCAM_region_names) %>%
      rename(stub.technology = technology)

    # L231.GlobalTechShrwt_urb_ind_EUR: Shareweights of global urban & industrial processes sector technologies
    L231.GlobalTechShrwt_urb_ind_EUR <- A31.globaltech_shrwt %>%
      select(supplysector, subsector, technology) %>%
      distinct %>%
    ...
    # L231.GlobalTechEff_urb_ind_EUR: Energy inputs and coefficients of global urban & industrial processes technologies
    L231.GlobalTechEff_urb_ind_EUR <- A31.globaltech_eff %>%
    ...
    # L231.GlobalTechCoef_urb_ind_EUR: Energy inputs and coefficients of global urban & industrial processes technologies
    L231.GlobalTechCoef_urb_ind_EUR <- A31.globaltech_coef %>%
    ...
    # L231.GlobalTechCost_urb_ind_EUR: Capital costs of global urban & industrial processes technologies
    L231.GlobalTechCost_urb_ind_EUR <- A31.globaltech_cost %>%
    ...
    L231.RegionalTechCalValue_urb_ind_EUR <- L231.GlobalTechCost_urb_ind_EUR %>%
    ...
    # L231.UnlimitRsrc_EUR: output unit, price unit, and market for unlimited resources
    L231.UnlimitRsrc_EUR <- L231.rsrc_info %>%
    ...
    # L231.UnlimitRsrcPrice_EUR: prices for unlimited resources
    L231.UnlimitRsrcPrice_EUR <- L231.rsrc_info %>%
      filter(resource_type == "unlimited-resource") %>%
      select(region, unlimited.resource = resource, year, price = value)

    # L231.IndCoef_EUR: coefficient on industrial processes as an input to the industry sector
    ...
    L231.Ind_globaltech_eff_EUR <- A32.globaltech_eff %>%
      select(-year, -value) %>%
      repeat_add_columns(tibble(year = c(HISTORICAL_YEARS, MODEL_FUTURE_YEARS))) %>%
    ...
    sum.calvalue <- L231.RegionalTechCalValue_urb_ind_EUR %>%
    ...
    L231.IndCoef_EUR <- bind_rows(L1322.in_EJ_R_indenergy_F_Yh_EUR %>%
                                mutate(sector = "other industrial energy use"),
                              L1322.in_EJ_R_indfeed_F_Yh_EUR %>%
                                mutate(sector = "other industrial feedstocks")) %>%
      left_join_keep_first_only(L231.Ind_globaltech_eff_EUR, by = c("sector", "fuel", "year")) %>%
      ...
      mutate(coefficient = if_else(ind_output == 0, 0, ind_proc_input / ind_output),
    ...
    L231.IndCoef_EUR <- L231.IndCoef_EUR %>%
      select(-year, -coefficient, -ind_output, -ind_proc_input) %>%
      distinct %>%
      repeat_add_columns(tibble(year = MODEL_YEARS)) %>%
      left_join(L231.IndCoef_EUR, by = c("GCAM_region_ID", "region", "supplysector", "subsector", "technology", "minicam.energy.input", "year")) %>%
    ...
    L231.UnlimitRsrcPrice_EUR %>%
      ...
      add_legacy_name("L231.UnlimitRsrcPrice_EUR") %>%
      add_precursors("emissions/A31.rsrc_info") ->
      L231.UnlimitRsrcPrice_EUR
    ...
    L231.FinalDemand_urb_EUR %>%
      ...
      add_legacy_name("L231.FinalDemand_urb_EUR") %>%
    ...
    L231.Supplysector_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.Supplysector_urb_ind_EUR") %>%
    ...
    L231.SubsectorLogit_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.SubsectorLogit_urb_ind_EUR") %>%
      add_precursors("emissions/A31.subsector_logit", "common/GCAM_region_names") ->
      L231.SubsectorLogit_urb_ind_EUR

    if(exists("L231.SubsectorShrwt_urb_ind_EUR")) {
      L231.SubsectorShrwt_urb_ind_EUR %>%
    ...
    if(exists("L231.SubsectorShrwtFllt_urb_ind_EUR")) {
      L231.SubsectorShrwtFllt_urb_ind_EUR %>%
        add_title("Urban and Industrial Processes Subsector Shareweights") %>%
        add_units("Unitless") %>%
        add_comments("A31.subsector_shrwt written to all regions") %>%
        add_legacy_name("L231.SubsectorShrwtFllt_urb_ind_EUR") %>%
        add_precursors("emissions/A31.subsector_shrwt", "common/GCAM_region_names") ->
        L231.SubsectorShrwtFllt_urb_ind_EUR
    } else {
      missing_data() %>%
    ...
    if(exists("L231.SubsectorInterp_urb_ind_EUR")) {
      L231.SubsectorInterp_urb_ind_EUR %>%
        ...
        add_legacy_name("L231.SubsectorInterp_urb_ind_EUR") %>%
    ...
    if(exists("L231.SubsectorInterpTo_urb_ind_EUR")) {
      L231.SubsectorInterpTo_urb_ind_EUR %>%
        add_title("Urban and Industrial Processes Subsector Shareweight Interpolation") %>%
        add_units("NA") %>%
        add_comments("A31.subsector_interp written to all regions") %>%
        add_legacy_name("L231.SubsectorInterpTo_urb_ind_EUR") %>%
        add_precursors("emissions/A31.subsector_interp", "common/GCAM_region_names") ->
        L231.SubsectorInterpTo_urb_ind_EUR
    } else {
      missing_data() %>%
    ...
    L231.StubTech_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.StubTech_urb_ind_EUR") %>%
    ...
    L231.GlobalTechShrwt_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.GlobalTechShrwt_urb_ind_EUR") %>%
    ...
    L231.GlobalTechEff_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.GlobalTechEff_urb_ind_EUR") %>%
    ...
    L231.GlobalTechCoef_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.GlobalTechCoef_urb_ind_EUR") %>%
    ...
    L231.GlobalTechCost_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.GlobalTechCost_urb_ind_EUR") %>%
    ...
    L231.RegionalTechCalValue_urb_ind_EUR %>%
      ...
      add_legacy_name("L231.RegionalTechCalValue_urb_ind_EUR") %>%
    ...
    L231.IndCoef_EUR %>%
      ...
      add_legacy_name("L231.IndCoef_EUR") %>%
    ...
    return_data(L231.UnlimitRsrc_EUR, L231.UnlimitRsrcPrice_EUR, L231.FinalDemand_urb_EUR, L231.Supplysector_urb_ind_EUR, L231.SubsectorLogit_urb_ind_EUR,
                ...
                L231.GlobalTechCost_urb_ind_EUR, L231.RegionalTechCalValue_urb_ind_EUR, L231.IndCoef_EUR, L231.Ind_globaltech_eff_EUR)
  } else {
