...
#' module_gcameurope_L2326.aluminum
#'
...
#' the generated outputs: \code{L2326.SectorLogitTables[[ curr_table ]]$data}, \code{L2326.Supplysector_aluminum_EUR}, \code{L2326.FinalEnergyKeyword_aluminum_EUR},
#' \code{L2326.SubsectorLogitTables[[ curr_table ]]$data}, \code{L2326.SubsectorLogit_aluminum_EUR}, \code{L2326.SubsectorShrwtFllt_aluminum_EUR},
#' \code{L2326.SubsectorInterp_aluminum_EUR}, \code{L2326.StubTech_aluminum_EUR}, \code{L2326.StubTechProd_aluminum_EUR}, \code{L2326.StubTechCalInput_aluminum_EUR},
#' \code{L2326.StubTechCoef_aluminum_EUR}, \code{L2326.PerCapitaBased_aluminum_EUR}, \code{L2326.BaseService_aluminum_EUR}, \code{L2326.PriceElasticity_aluminum_EUR},
...
module_gcameurope_L2326.aluminum <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1326.out_Mt_R_aluminum_Yh_EUR <- get_data(all_data, "L1326.out_Mt_R_aluminum_Yh_EUR")
    L1326.IO_GJkg_R_aluminum_F_Yh_EUR<- get_data(all_data, "L1326.IO_GJkg_R_aluminum_F_Yh_EUR")
    # ===================================================
    ...
    # L2326.Supplysector_aluminum_EUR: Supply sector information for aluminum sector
    A326.sector %>%
    ...
    # L2326.FinalEnergyKeyword_aluminum_EUR: Supply sector keywords for aluminum sector
    A326.sector %>%
    ...
    # L2326.SubsectorLogit_aluminum_EUR: Subsector logit exponents of aluminum sector
    ...
    # and L2326.SubsectorShrwtFllt_aluminum_EUR: Subsector shareweights of aluminum sector
    ...
    # L2326.SubsectorInterp_aluminum_EUR: Subsector shareweight interpolation of aluminum sector
    ...
    # L2326.StubTech_aluminum_EUR: Identification of stub technologies of aluminum
    ...
    # L2326.StubTechCalInput_aluminum_EUR: calibrated input of aluminum energy use technologies (including cogen)
    L2326.GlobalTechCoef_aluminum %>%
    ...
    # L2326.StubTechCalInput_aluminum_EUR: calibrated aluminum input
    calibrated_techs %>%
    ...
    L1326.in_EJ_R_aluminum_Yh_EUR %>%
      ...
      mutate(stub.technology = technology,
             technology = NULL)   ->
      L2326.StubTechCalInput_aluminum_EUR_tmp
    ...
    L1326.out_Mt_R_aluminum_Yh_EUR %>%
      ...
      left_join(L1326.out_Mt_R_aluminum_Yh_EUR %>%
      ...
      L1326.IO_GJkg_R_aluminum_F_Yh_EUR_alumina


    L1326.IO_GJkg_R_aluminum_F_Yh_EUR %>%
      bind_rows(L1326.IO_GJkg_R_aluminum_F_Yh_EUR_alumina) %>%
      filter(year %in% MODEL_BASE_YEARS) %>%
      complete(nesting(fuel,year,sector),GCAM_region_ID = GCAM_region_names$GCAM_region_ID) %>%
      filter(fuel != 'heat' | (fuel == 'heat' & value>0) ) %>%
      mutate(value = replace_na(value,0)) %>%
      ...
      L2326.StubTechCoef_aluminum_EUR_tmp

    L2326.aluminum_tmp %>%
      left_join(L2326.StubTechCoef_aluminum_EUR_tmp,
                by = c("region", "supplysector", "subsector", "stub.technology", "year", "minicam.energy.input")) %>%
    ...
    # L2326.StubTechProd_aluminum_EUR: calibrated aluminum output
    calibrated_techs %>%
    ...
    L1326.out_Mt_R_aluminum_Yh_EUR %>%
      ...
      complete(nesting(fuel,GCAM_region_ID,sector),year = MODEL_BASE_YEARS) %>%
      complete(nesting(fuel,year,sector),GCAM_region_ID = gcameurope.EUROSTAT_GCAMREGIONID) %>%
      ...
      L2326.StubTechProd_aluminum_EUR_tmp
    ...
    L2326.aluminum_out_tmp %>%
      filter(supplysector == "aluminum") %>%
      left_join(L2326.StubTechProd_aluminum_EUR_tmp,
    ...
    # L2326.PerCapitaBased_aluminum_EUR: per-capita based flag for aluminum exports final demand
    A326.demand %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["PerCapitaBased"]], GCAM_region_names)  ->
      L2326.PerCapitaBased_aluminum_EUR

    # L2326.BaseService_aluminum_EUR: base-year service output of aluminum
    L2326.StubTechProd_aluminum_EUR %>%
    ...
    # L2326.PriceElasticity_aluminum_EUR: price elasticity
    A326.demand %>%
    ...
    return_data(L2326.Supplysector_aluminum_EUR, L2326.FinalEnergyKeyword_aluminum_EUR, L2326.SubsectorLogit_aluminum_EUR,
                L2326.SubsectorShrwtFllt_aluminum_EUR, L2326.SubsectorInterp_aluminum_EUR,
                L2326.StubTech_aluminum_EUR, L2326.StubTechCalInput_aluminum_EUR,L2326.StubTechCoef_aluminum_EUR,
                L2326.PerCapitaBased_aluminum_EUR, L2326.BaseService_aluminum_EUR,L2326.StubTechProd_aluminum_EUR,
                L2326.PriceElasticity_aluminum_EUR)
