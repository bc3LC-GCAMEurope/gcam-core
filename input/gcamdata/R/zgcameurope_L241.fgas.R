...
#' @author KD July 2017
module_gcameurope_L241.fgas <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L142.pfc_R_S_T_Yh_EUR <- get_data(all_data, "L142.pfc_R_S_T_Yh_EUR", strip_attributes = T)
    L141.hfc_R_S_T_Yh_EUR <- get_data(all_data, "L141.hfc_R_S_T_Yh_EUR", strip_attributes = T)
    L141.hfc_ef_R_cooling_Yh_EUR <- get_data(all_data, "L141.hfc_ef_R_cooling_Yh_EUR", strip_attributes = T)
    ...
    # ===================================================
    gcam.Germany_REGION <- GCAM_region_names %>%
      filter(region == 'Germany') %>% pull(GCAM_region_ID)
    ...
    L241.pfc_all_EUR %>%
      bind_rows(L241.hfc_all_EUR) %>%
      bind_rows(L241.hfc_future_EUR) %>%
    ...
    L241.pfc_all_EUR %>%
      ...
      add_legacy_name("L241.pfc_all_EUR") %>%
      add_precursors("common/GCAM_region_names", "emissions/A_regions", "emissions/FUT_EMISS_GV",
                     "L141.hfc_R_S_T_Yh_EUR", "L142.pfc_R_S_T_Yh_EUR",
                     "L141.hfc_ef_R_cooling_Yh_EUR") ->
      L241.pfc_all_EUR
    ...
    L241.fgas_all_units_EUR %>%
      ...
      add_units("Gg") %>%
      add_comments("NA") %>%
      add_legacy_name("L241.fgas_all_units_EUR") %>%
      add_precursors("common/GCAM_region_names", "emissions/A_regions", "emissions/FUT_EMISS_GV",
                     "L141.hfc_R_S_T_Yh_EUR", "L142.pfc_R_S_T_Yh_EUR",
                     "L141.hfc_ef_R_cooling_Yh_EUR") ->
      L241.fgas_all_units_EUR

    return_data(L241.hfc_all_EUR, L241.pfc_all_EUR, L241.hfc_future_EUR, L241.fgas_all_units_EUR)
