...
#' the generated outputs: \code{L143.HDDCDD_scen_R_Y_EUR}, \code{L143.HDDCDD_scen_RG3_Y_EUR}, \code{L143.HDDCDD_scen_ctry_Y_EUR}. The corresponding file in the
...
module_gcameurope_L143.HDDCDD <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L143.HDDCDD_scen_R_Y_EUR",
             "L143.HDDCDD_scen_RG3_Y_EUR",
             "L143.HDDCDD_scen_ctry_Y_EUR"))
    ...
    # Add in country iso
    L143.HDDCDD_scen_ctry_Y_EUR <- HDDCDD_data %>%
      # Drop file name
      select(-file) %>%
    ...
    L143.HDDCDD_scen_ctry_Y_EUR_Malta <- L143.HDDCDD_scen_ctry_Y_EUR %>%
      filter(country == "Greece") %>%
      mutate(country = "Malta", iso = "mlt")

    L143.HDDCDD_scen_ctry_Y_EUR <- bind_rows(L143.HDDCDD_scen_ctry_Y_EUR, L143.HDDCDD_scen_ctry_Y_EUR_Malta)

    # Serbia and Montenegro are currently combined. Copy to separated countries, assigning the same HDD and CDD to each
    if("scg" %in% L143.HDDCDD_scen_ctry_Y_EUR$iso) {
      # Create Serbia tibble
      L143.HDDCDD_scen_srb_Y <- L143.HDDCDD_scen_ctry_Y_EUR %>%
        filter(iso == "scg") %>%
        mutate(iso = "srb")

      # Insert Serbia tibble, change 'scg' iso to 'mne' iso
      L143.HDDCDD_scen_ctry_Y_EUR <- L143.HDDCDD_scen_ctry_Y_EUR %>%
    ...
    L143.wtHDDCDD_scen_ctry_Y <- L143.HDDCDD_scen_ctry_Y_EUR %>%
    ...
    L143.HDDCDD_scen_R_Y_EUR <- L143.wtHDDCDD_scen_ctry_Y %>%
    ...
    L143.HDDCDD_scen_RG3_Y_EUR <- L143.wtHDDCDD_scen_ctry_Y %>%
      group_by(region_GCAM3, SRES, GCM, variable, year) %>%
      summarise(value = weighted.mean(value, population)) %>%
      ungroup()

    # Select only EUR regions
    L143.HDDCDD_scen_ctry_Y_EUR <- L143.HDDCDD_scen_ctry_Y_EUR %>%
      filter_regions_europe(region_ID_mapping = iso_GCAM_regID)
    ...
    L143.HDDCDD_scen_R_Y_EUR %>%
      ...
      add_legacy_name("L143.HDDCDD_scen_R_Y_EUR") %>%
    ...
    L143.HDDCDD_scen_RG3_Y_EUR %>%
      ...
      add_legacy_name("L143.HDDCDD_scen_RG3_Y_EUR") %>%
      same_precursors_as(L143.HDDCDD_scen_R_Y_EUR) ->
      L143.HDDCDD_scen_RG3_Y_EUR
    ...
    L143.HDDCDD_scen_ctry_Y_EUR %>%
      ...
      add_legacy_name("L143.HDDCDD_scen_ctry_Y_EUR") %>%
      same_precursors_as(L143.HDDCDD_scen_R_Y_EUR) ->
      L143.HDDCDD_scen_ctry_Y_EUR

    return_data(L143.HDDCDD_scen_R_Y_EUR, L143.HDDCDD_scen_RG3_Y_EUR, L143.HDDCDD_scen_ctry_Y_EUR)
  } else {
