...
#' module_gcameurope_L270.EFW_input_coefs
#'
...
#'   \code{command} is "MAKE") all the generated outputs: \code{L270.TechCoef_EFW_EUR}. This did not exist in the original
#'   data system.
...
module_gcameurope_L270.EFW_input_coefs <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L173.WWtrtfrac_R_ind_Yh_EUR <- get_data(all_data, "L173.WWtrtfrac_R_ind_Yh_EUR", strip_attributes = TRUE)
    L174.WWtrtfrac_R_muni_Yh_EUR <- get_data(all_data, "L174.WWtrtfrac_R_muni_Yh_EUR", strip_attributes = TRUE)
    L203.TechCoef_watertd_EUR <- get_data(all_data, "L203.TechCoef_watertd_EUR", strip_attributes = TRUE)
    ...
    # L270.TechCoef_EFW_EUR: input coefficients from EFW sectors to water T&D technologies
    ...
    L270.TechCoef_EFW_EUR_init <- L203.TechCoef_watertd_EUR %>%
    ...
    L270.WWtrt_coef <- bind_rows(L173.WWtrtfrac_R_ind_Yh_EUR, L174.WWtrtfrac_R_muni_Yh_EUR) %>%
    ...
    L270.TechCoef_EFW_EUR <- left_join(L270.TechCoef_EFW_EUR_init, L270.WWtrt_coef,
    ...
    L270.TechCoef_WWtrt_SSP_EUR <- inner_join(L270.TechCoef_EFW_EUR_init, L270.WWtrt_coef_SSP,
                                          by = c("region", "water.supplysector", "minicam.energy.input", "year")) %>%
      mutate(coefficient = if_else(is.na(coef_revised), coefficient, coef_revised),
             coefficient = round(coefficient, energy.DIGITS_COEFFICIENT)) %>%
    ...
    L270.TechCoef_EFW_EUR %>%
      add_title("GCAM-Europe Tech coefficients of energy-for-water processes") %>%
    ...
    L270.TechCoef_WWtrt_SSP_EUR %>%
      add_title("GCAM-Europe Wastewater treatment fractions by scenario, region, year") %>%
      add_units("Unitless (e.g., m^3 of wastewater treated / m^3 of water withdrawn)") %>%
      add_comments("Units of water flow from the EFW processes to the given technology, per unit output of the technology") %>%
      same_precursors_as(L270.TechCoef_EFW_EUR) ->
      L270.TechCoef_WWtrt_SSP_EUR

    return_data(L270.TechCoef_EFW_EUR, L270.TechCoef_WWtrt_SSP_EUR)
  } else {
