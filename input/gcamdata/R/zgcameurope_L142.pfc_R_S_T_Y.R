...
#' module_gcameurope_L142.pfc_R_S_T_Y
#'
...
#' the generated outputs: \code{L142.pfc_R_S_T_Yh_EUR}. The corresponding file in the
...
module_gcameurope_L142.pfc_R_S_T_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L142.pfc_R_S_T_Yh_EUR"))
    ...
    if(length(additional_years) == 0){
      L142.pfc_R_S_T_Yh_base <- L142.pfc_R_S_T_Yh_EUR
    } else{
      TEMP <- filter(L142.pfc_R_S_T_Yh_EUR, year == emissions.EDGAR_YEARS[length(emissions.EDGAR_YEARS)]) %>%
        select(-year) %>%
        repeat_add_columns(tibble(year = additional_years))

      L142.pfc_R_S_T_Yh_base <- bind_rows(L142.pfc_R_S_T_Yh_EUR, TEMP)
    }
    ...
    L142.pfc_R_S_T_Yh_EUR <- L142.EPA_PFC_R_S_T_Yh
    ...
    L244.GenericShares_EUR<- get_data(all_data, "L244.GenericShares_EUR",strip_attributes = TRUE) %>%
      ...
      complete(nesting(region,supplysector), year = c(year, unique(L142.pfc_R_S_T_Yh_EUR$year))) %>%
      # Interpolate
    ...
    L142.pfc_R_S_T_Yh_EUR<-L142.pfc_R_S_T_Yh_EUR %>%
      filter(!grepl("resid",supplysector)) %>%
    ...
    L142.pfc_R_S_T_Yh_EUR %>%
      ...
      add_legacy_name("L142.pfc_R_S_T_Yh_EUR") %>%
    ...
    return_data(L142.pfc_R_S_T_Yh_EUR)
  } else {
