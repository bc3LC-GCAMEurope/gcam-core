...
module_aglu_L2012.ag_For_Past_bio_input_irr_mgmt <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L181.ag_Prod_Mt_R_C_Y_GLU_irr_level %>%
      ...
      unique %>%
      bind_rows(
        bind_rows(L123.For_Prod_bm3_R_Y_GLU %>%
                    mutate(GCAM_subsector=Land_Type) %>%
                    rename(GCAM_subsector = GCAM_subsector),
                  L123.ag_Prod_Mt_R_Past_Y_GLU %>%
    ...
    L2012.AgSupplySubsector_EU <- L2012.AgSupplySector_EU %>%
      ...
      mutate(AgSupplySubsector  = AgSupplySector,
             logit.year.fillout = min(MODEL_BASE_YEARS),
             logit.exponent = -3,
             logit.type = NA)

    L2012.AgProduction_ag_irr_mgmt_EU <- L2012.AgSupplySubsector_EU %>%
