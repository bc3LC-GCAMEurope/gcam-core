...
#' the generated outputs: \code{L124.in_EJ_R_heat_F_Yh_EUR}, \code{L124.out_EJ_R_heat_F_Yh_EUR}, \code{L124.out_EJ_R_heatfromelec_F_Yh_EUR}, \code{L124.heatoutratio_R_elec_F_tech_Yh_EUR}.
...
module_gcameurope_L124.heat <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L124.globaltech_coef <- A24.globaltech_coef %>%
      gather_years %>%
    ...
    L124.heatoutratio_R_elec_F_tech_Yh_EUR %>%
      ...
      filter(sum != 0) %>%
      select(-sum) -> L124.heatoutratio_R_elec_F_tech_Yh_EUR
    ...
    # Eurostat's MAINCHP flow, which we model in the electric sector with a secondary output of heat to the district heat sector. In GCAM this will
    ...
    L124.mult_R_heat_Yh_EUR %>%
      ...
      replace_na(list(value = 0)) -> L124.mult_R_heat_Yh_EUR
    ...
    L124.in_EJ_R_heat_F_Yh_EUR %>%
      ...
      left_join_error_no_match(rename(L124.mult_R_heat_Yh_EUR, mult_val = value)
                               , by = c("sector", "GCAM_region_ID", "year")) %>%
      mutate(value = value + (norm_val * mult_val)) %>%
      select(fuel, sector, GCAM_region_ID, year, value) -> L124.in_EJ_R_heat_F_Yh_EUR
    ...
    L124.out_EJ_R_heat_F_Yh_EUR %>%
      ...
      left_join_error_no_match(L124.mult_R_heat_Yh_EUR %>%
