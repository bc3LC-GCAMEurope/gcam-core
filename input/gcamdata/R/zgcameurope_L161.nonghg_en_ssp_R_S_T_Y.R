...
#' module_gcameurope_L161.nonghg_en_ssp_R_S_T_Y
#'
...
#' the generated outputs: \code{L161.SSP15_EF_EUR}, \code{L161.SSP2_EF_EUR}, \code{L161.SSP34_EF_EUR}. The corresponding file in the
...
module_gcameurope_L161.nonghg_en_ssp_R_S_T_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L102.pcgdp_thous90USD_Scen_R_Y <- get_data(all_data, "L102.pcgdp_thous90USD_Scen_R_Y") %>% filter_regions_europe(region_ID_mapping = A_regions)
    L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR <- get_data(all_data, "L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR")
    L114.bcoc_tgej_R_en_S_F_2000_EUR <- get_data(all_data, "L114.bcoc_tgej_R_en_S_F_2000_EUR") %>%
    ...
    GCAM_sector_tech_resid<-GCAM_sector_tech_EUR %>%
    ...
    GCAM_sector_tech_EUR<-GCAM_sector_tech_EUR %>%
      filter(!grepl("resid",supplysector)) %>%
    ...
    emfact_scaled <- L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR %>%
      ...
      left_join(GCAM_sector_tech_EUR %>% select(supplysector, subsector, stub.technology, IIASA_sector) %>% distinct(),
    ...
    L244.DeleteService<-bind_rows(L244.DeleteGenericService_EUR %>% select(-building.service.input),
                                  L244.DeleteThermalService_EUR %>% select(-thermal.building.service.input)) %>%
    ...
    out_df[["1&5"]] %>%
      ...
      add_legacy_name("L161.SSP15_EF_EUR") %>%
    ...
    out_df[["2"]] %>%
      ...
      add_units("Tg/EJ") %>%
      add_comments("Scales future GAINS emissions factors to L111/L114 base year emissions factors, then applies to GCAM future years") %>%
      add_legacy_name("L161.SSP2_EF_EUR") %>%
      add_precursors("emissions/A_regions",
                     "gcam-europe/GCAM_sector_tech_EUR",
                     "gcam-europe/GCAM_sector_tech_Revised_EUR",
                     "emissions/mappings/gains_to_gcam_sector",
    ...
    out_df[["3&4"]] %>%
      ...
      add_units("Tg/EJ") %>%
      add_comments("Scales future GAINS emissions factors to L111/L114 base year emissions factors, then applies to GCAM future years") %>%
      add_legacy_name("L161.SSP34_EF_EUR") %>%
      add_precursors("emissions/A_regions",
                     "gcam-europe/GCAM_sector_tech_EUR",
                     "gcam-europe/GCAM_sector_tech_Revised_EUR",
                     "emissions/mappings/gains_to_gcam_sector",
    ...
    return_data(L161.SSP15_EF_EUR, L161.SSP2_EF_EUR, L161.SSP34_EF_EUR)
  } else {
