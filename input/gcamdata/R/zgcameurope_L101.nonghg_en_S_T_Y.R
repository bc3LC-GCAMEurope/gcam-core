...
#' module_gcameurope_L101.nonghg_en_S_T_Y
#'
...
#' the generated outputs: \code{L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR}, \code{L101.co_tgej_EUR_en_Sepa_F_Yh_EUR}, \code{L101.nox_tgej_EUR_en_Sepa_F_Yh_EUR}, \code{L101.voc_tgej_EUR_en_Sepa_F_Yh_EUR}, \code{L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR}, \code{L101.in_EJ_R_en_Si_F_Yh_EUR}. The corresponding file in the
#' original data system was \code{L101.nonghg_en_EUR_S_T_Y.R} (emissions level1).
...
#' @author CRB May 2024
module_gcameurope_L101.nonghg_en_S_T_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR",
             "L101.co_tgej_EUR_en_Sepa_F_Yh_EUR",
             "L101.nox_tgej_EUR_en_Sepa_F_Yh_EUR",
             "L101.voc_tgej_EUR_en_Sepa_F_Yh_EUR",
             "L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR",
    ...
    bind_rows(spread(L1322.in_EJ_R_indenergy_F_Yh_EUR, year, value),
              ...
              spread(L144.in_EJ_R_bld_serv_F_Yh_EUR, year, value) %>% select(-sector) %>% rename(sector = service)) %>%
    ...
    L101.in_EJ_EUR_en_Sepa_F_Yh.mlt %>%
      ...
      group_by(GCAM_region_ID, EPA_agg_sector, EPA_agg_fuel, year) %>%
    ...
    L101.so2_tg_EUR_en_Sepa_F_Yh <- EPA_convert_and_aggregate(EPA_SO2, EPA_tech)
    L101.co_tg_EUR_en_Sepa_F_Yh <- EPA_convert_and_aggregate(EPA_CO, EPA_tech)
    L101.nox_tg_EUR_en_Sepa_F_Yh <- EPA_convert_and_aggregate(EPA_NOx, EPA_tech)
    L101.voc_tg_EUR_en_Sepa_F_Yh <- EPA_convert_and_aggregate(EPA_VOC, EPA_tech)
    L101.nh3_tg_EUR_en_Sepa_F_Yh <- EPA_convert_and_aggregate(EPA_NH3, EPA_tech)

    # Compute SO2, CO, NOx, VOC, and NH3 emissions factors by dividing EPA inventory by IEA energy balances
    # We do this for each gas, so define a function with all the steps
    EPA_compute_emissions_factors <- function(x, L101.in_EJ_EUR_en_Sepa_F_Yh.mlt) {
      L101.in_EJ_EUR_en_Sepa_F_Yh.mlt %>%
    ...
    L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR <- EPA_compute_emissions_factors(L101.so2_tg_EUR_en_Sepa_F_Yh, L101.in_EJ_EUR_en_Sepa_F_Yh.mlt)
    L101.co_tgej_EUR_en_Sepa_F_Yh_EUR <- EPA_compute_emissions_factors(L101.co_tg_EUR_en_Sepa_F_Yh, L101.in_EJ_EUR_en_Sepa_F_Yh.mlt)
    L101.nox_tgej_EUR_en_Sepa_F_Yh_EUR <- EPA_compute_emissions_factors(L101.nox_tg_EUR_en_Sepa_F_Yh, L101.in_EJ_EUR_en_Sepa_F_Yh.mlt)
    L101.voc_tgej_EUR_en_Sepa_F_Yh_EUR <- EPA_compute_emissions_factors(L101.voc_tg_EUR_en_Sepa_F_Yh, L101.in_EJ_EUR_en_Sepa_F_Yh.mlt)
    L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR <- EPA_compute_emissions_factors(L101.nh3_tg_EUR_en_Sepa_F_Yh, L101.in_EJ_EUR_en_Sepa_F_Yh.mlt)
    ...
    L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR %>%
      ...
      left_join_error_no_match(select(filter(L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR, year == 1990), -year), by = c("GCAM_region_ID", "sector", "fuel")) %>%
      bind_rows(filter(L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR, !year %in% emissions.NH3_EXTRA_YEARS)) ->
      L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR
    ...
    L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR %>%
      rename(value = em_factor) %>%   # for testing system
      add_title("SO2 emissions factors for the EUR by EPA sector / fuel / historical year") %>%
      add_units("Tg/EJ") %>%
      add_comments("Matched to EPA categories, summed, converted to Tg") %>%
      add_legacy_name("L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR") %>%
      add_precursors("emissions/EPA_SO2") ->
      L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR

    L101.co_tgej_EUR_en_Sepa_F_Yh_EUR %>%
      rename(value = em_factor) %>%   # for testing system
      add_title("CO emissions factors for the EUR by EPA sector / fuel / historical year") %>%
      add_units("Tg/EJ") %>%
      add_comments("Matched to EPA categories, summed, converted to Tg") %>%
      add_legacy_name("L101.co_tgej_EUR_en_Sepa_F_Yh_EUR") %>%
      same_precursors_as(L101.in_EJ_R_en_Si_F_Yh_EUR) %>%
      add_precursors("emissions/EPA_CO") ->
      L101.co_tgej_EUR_en_Sepa_F_Yh_EUR

    L101.nox_tgej_EUR_en_Sepa_F_Yh_EUR %>%
      rename(value = em_factor) %>%   # for testing system
      add_title("NOx emissions factors for the EUR by EPA sector / fuel / historical year") %>%
      add_units("Tg/EJ") %>%
      add_comments("Matched to EPA categories, summed, converted to Tg") %>%
      add_legacy_name("L101.nox_tgej_EUR_en_Sepa_F_Yh_EUR") %>%
      same_precursors_as(L101.in_EJ_R_en_Si_F_Yh_EUR) %>%
    ...
    L101.voc_tgej_EUR_en_Sepa_F_Yh_EUR %>%
      rename(value = em_factor) %>%   # for testing system
      add_title("VOC emissions factors for the EUR by EPA sector / fuel / historical year") %>%
      add_units("Tg/EJ") %>%
      add_comments("Matched to EPA categories, summed, converted to Tg") %>%
      add_legacy_name("L101.voc_tgej_EUR_en_Sepa_F_Yh_EUR") %>%
      same_precursors_as(L101.in_EJ_R_en_Si_F_Yh_EUR) %>%
    ...
    L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR %>%
      rename(value = em_factor) %>%   # for testing system
      add_title("NH3 emissions factors for the EUR by EPA sector / fuel / historical year") %>%
      add_units("Tg/EJ") %>%
      add_comments("Matched to EPA categories, summed, converted to Tg") %>%
      add_legacy_name("L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR") %>%
      same_precursors_as(L101.in_EJ_R_en_Si_F_Yh_EUR) %>%
      add_precursors("emissions/EPA_NH3") ->
      L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR

    return_data(L101.so2_tgej_EUR_en_Sepa_F_Yh_EUR, L101.co_tgej_EUR_en_Sepa_F_Yh_EUR,
                L101.nox_tgej_EUR_en_Sepa_F_Yh_EUR, L101.voc_tgej_EUR_en_Sepa_F_Yh_EUR,
                L101.nh3_tgej_EUR_en_Sepa_F_Yh_EUR, L101.in_EJ_R_en_Si_F_Yh_EUR)
  } else {
