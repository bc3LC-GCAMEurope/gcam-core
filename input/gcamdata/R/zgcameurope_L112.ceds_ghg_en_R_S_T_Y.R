...
#' module_gcameurope_L112.ceds_ghg_en_R_S_T_Y
#'
...
module_gcameurope_L112.ceds_ghg_en_R_S_T_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/GCAM_region_names",
             ...
             FILE = "gcam-europe/CEDS_sector_tech_combustion_EUR",
             FILE = "gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
    ...
    return(c("L111.nonghg_tg_R_en_S_F_Yh_EUR",
             ...
             "L112.in_EJ_R_en_S_F_Yh_calib_all_baseenergy_EUR",
             "L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR",
             "L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR",
             "L121.nonco2_tg_R_awb_C_Y_GLU_EUR",
             "L121.AWBshare_R_C_Y_GLU_EUR",
             "L122.ghg_tg_R_agr_C_Y_GLU_EUR",
             "L122.EmissShare_R_C_Y_GLU_EUR",
             "L124.nonco2_tg_R_grass_Y_GLU_EUR",
             "L124.nonco2_tg_R_forest_Y_GLU_EUR",
             "L124.deforest_coefs_EUR",
             "L131.nonco2_tg_R_prc_S_S_Yh_EUR",
             "L125.bcoc_tgbkm2_R_grass_2000_EUR",
             "L125.bcoc_tgbkm2_R_forest_2000_EUR",
             "L125.deforest_coefs_bcoc_EUR"))
    ...
    L112.CEDS_intl_shipping <- get_data(all_data, "L102.ceds_int_shipping_nonco2_tg_S_F") %>%
      filter(year >= min(HISTORICAL_YEARS), year <= max(HISTORICAL_YEARS))
    Int_shipping_IEA_EIA <- get_data(all_data, "L154.IEA_histfut_data_times_UCD_shares") %>%
      filter(UCD_category=="trn_international ship") %>%
      filter_regions_europe()

    #Get NEI data for crude oil and natural gas, and BC OC fractions for this
    NEI_tg_oilgas_state_Yb <- get_data(all_data, "L270.nonghg_tg_state_refinery_F_Yb") %>%
      filter(sector == "NG_production_distribution" | sector == "petroleum_production")
    BC_OC_assumptions <- get_data(all_data, "gcam-usa/emissions/BC_OC_assumptions")
    BCOC_PM25_ratios <- get_data(all_data, "gcam-europe/BCOC_PM25_ratios_EUR") %>%
    ...
    GAINS_sector <- get_data(all_data,"emissions/CEDS/gains_iso_sector_emissions") %>% filter_regions_europe()
    #Separate out GAINS em factors for NG
    GAINS_fuel <- get_data(all_data,"emissions/CEDS/gains_iso_fuel_emissions") %>% select(-natural_gas) %>% filter_regions_europe()
    GAINS_fuel_NG <- get_data(all_data,"emissions/CEDS/gains_iso_fuel_emissions") %>%  select(-dieseloil,-lightoil) %>% filter_regions_europe()
    IEA_Ctry_data <- get_data(all_data,"L154.IEA_histfut_data_times_UCD_shares") %>% filter_regions_europe()
    ...
    L1328.in_EJ_R_indenergy_F_Yh_EUR <- get_data(all_data, "L1328.in_EJ_R_indenergy_F_Yh_EUR")
    ...
    L1326.in_EJ_R_aluminum_Yh_EUR <- get_data(all_data, "L1326.in_EJ_R_aluminum_Yh_EUR")
    L1327.in_EJ_R_paper_F_Yh_EUR <- get_data(all_data, "L1327.in_EJ_R_paper_F_Yh_EUR")
    L1328.in_EJ_R_food_F_Yh_EUR <- get_data(all_data, "L1328.in_EJ_R_food_F_Yh_EUR")
    L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR <- get_data(all_data, "L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR")
    ...
    if (energy.TRAN_UCD_MODE=="rev.mode"){
      CEDS_sector_tech <- get_data(all_data, "gcam-europe/CEDS_sector_tech_combustion_revised_EUR") %>% distinct()
    }else{

      CEDS_sector_tech <- get_data(all_data, "gcam-europe/CEDS_sector_tech_combustion_EUR")
    }

    calibrated_outresources <- get_data(all_data, "emissions/mappings/calibrated_outresources")

    L124.LC_bm2_R_Grass_Yh_GLU_adj <- get_data(all_data, "L124.LC_bm2_R_Grass_Yh_GLU_adj",strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L124.LC_bm2_R_UnMgdFor_Yh_GLU_adj <- get_data(all_data, "L124.LC_bm2_R_UnMgdFor_Yh_GLU_adj",strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L107.an_Prod_Mt_R_C_Sys_Fd_Y <- get_data(all_data, "L107.an_Prod_Mt_R_C_Sys_Fd_Y",strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L101.ag_Prod_Mt_R_C_Y_GLU <- get_data(all_data, "L101.ag_Prod_Mt_R_C_Y_GLU",strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L111.ag_resbio_R_C <- get_data(all_data, "L111.ag_resbio_R_C",strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    L122.LC_bm2_R_HarvCropLand_C_Yh_GLU <- get_data(all_data, "L122.LC_bm2_R_HarvCropLand_C_Yh_GLU",strip_attributes = TRUE) %>% filter_regions_europe(region_ID_mapping = GCAM_region_names)
    ...
    Clean_IEA_ctry_data %>%
      ...
      left_join(GAINS_sector %>%
                  tidyr::gather("UCD_sector","em_fact","Freight":"Passenger"),by=c("iso","year","Non.co2","UCD_sector")) %>%
      na.omit() %>%
    ...
    GAINS_NG_em_factors_adj<- GAINS_NG_em_factors %>%
      ...
      repeat_add_columns(tibble(group = unique(get_data(all_data, "L244.TrnShares_EUR")$group))) %>%
    ...
    # Replace industry_energy in L101.in_EJ_R_en_Si_F_Yh_EUR with energy in detailed industrial sectors and remaining industrial energy
    ...
    L1328.in_EJ_R_indenergy_F_Yh_EUR %>% mutate(technology = fuel) %>%
      bind_rows(L1323.in_EJ_R_iron_steel_F_Y_EUR,
                L1324.in_EJ_R_Off_road_F_Y_EUR,
                L1325.in_EJ_R_chemical_F_Y_EUR,
                L1326.in_EJ_R_aluminum_Yh_EUR,
                L1327.in_EJ_R_paper_F_Yh_EUR,
    ...
    L244.GenericShares_EUR<- get_data(all_data, "L244.GenericShares_EUR",strip_attributes = TRUE) %>%
    ...
    L244.ThermalShares_EUR<- get_data(all_data, "L244.ThermalShares_EUR",strip_attributes = TRUE) %>%
    ...
    L244.TrnShares <- get_data(all_data, "L244.TrnShares_EUR", strip_attributes = TRUE) %>%
    ...
    L244.Shares_resid<-bind_rows(L244.GenericShares_EUR,L244.ThermalShares_EUR) %>%
      left_join_error_no_match(GCAM_region_names, by = "region")


    cons.gr.adj<-get_data(all_data, "L244.GenericShares_EUR",strip_attributes = TRUE)  %>%
      select(gcam.consumer) %>%
    ...
    L112.CEDS_GCAM_emissions %>%
      ...
      left_join_error_no_match(L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR %>%
    ...
    L112.nonco2_tg_R_en_S_F_Yh %>%
      left_join_error_no_match(L112.in_EJ_R_en_S_F_Yh_calib_all_baseenergy_EUR,
    ...
    IPCC_unconventional_oil_fug_emfacts %>%
      ...
      left_join_error_no_match(get_data(all_data, "common/iso_GCAM_regID"), by = c("iso", "country_name")) %>%
    ...
    ## NO conventional_oil_new_emfacts FOR EUR REGIONS
    ...
    # since conventional_oil_new_emfacts not present for EUR regions,
    # simply select column of emissions factor dataset
    L112.nonco2_tgej_R_en_S_F_Yh_with_fug_uo_emfacts %>%
      select(GCAM_region_ID, year, Non.CO2, supplysector, subsector,
    ...
    L112.CEDS_GCAM %>% filter_regions_europe(region_ID_mapping = GCAM_region_names) %>%
    ...
    GCAM_sector_tech %>%
      ...
      ungroup()->L131.nonco2_tg_R_prc_S_S_Yh_EUR
    ...
    L107.an_Prod_Mt_R_C_Sys_Fd_Y %>%
      ...
      L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR.mlt
    ...
    L122.EmissShare_R_C_Y_GLU_EUR <- L122.ProdGLUshare_R_C_Y_GLU %>%
    ...
    L112.CEDS_GCAM_agr %>% filter_regions_europe(region_ID_mapping = GCAM_region_names) %>%
    ...
    L112.nonco2_tg_R_en_S_F_Yh %>%
      ...
      select(GCAM_region_ID, Non.CO2, supplysector, subsector, stub.technology, year, value = emissions) %>%
      bind_rows(GAINS_NG_em_factors %>% mutate(value=energy*value) %>%
                  filter(!(Non.CO2 %in% c("CH4", "N2O", "CO2"))) %>% select(-energy))->L111.nonghg_tg_R_en_S_F_Yh_EUR
    ...
    L112.nonco2_tgej_R_en_S_F_Yh %>%
      ...
      bind_rows(GAINS_NG_em_factors %>% filter(Non.CO2 %in% c("CH4", "N2O")) %>% select(-energy)) -> L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR
    ...
    L113.ghg_tg_R_an_C_Sys_Fd_Yh_full %>%
      filter(Non.CO2 == "NH3_AGR") -> L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR

    # Animal all other emissions
    L113.ghg_tg_R_an_C_Sys_Fd_Yh_full %>%
      filter(Non.CO2 != "NH3_AGR") -> L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR
    ...
    L122.EmissShare_R_C_Y_GLU_EUR -> L122.EmissShare_R_C_Y_GLU_EUR

    # AGR emissions: Filter for gases present in original data set
    L122.ghg_tg_R_agr_C_Y_GLU_full ->
      L122.ghg_tg_R_agr_C_Y_GLU_EUR

    L124.deforest_coefs_full->L124.deforest_coefs_EUR

    L124.nonco2_tg_R_grass_Y_GLU_full %>%
      rename(value = emissions) ->L124.nonco2_tg_R_grass_Y_GLU_EUR
    ...
    L125.bcoc_tgbkm2_R_grass_2000_EUR <- L124.LC_bm2_R_Grass_Yh_GLU_adj %>%
    ...
    L125.bcoc_tgbkm2_R_forest_2000_EUR<-bind_rows(BC_OC_Deforest,BC_OC_Forest) %>%
      mutate(em_factor=if_else(is.finite(em_factor),em_factor,0))


    # Filter out BC OC for its own output
    L124.deforest_coefs_full %>%
      filter(Non.CO2 %in% c("BC", "OC")) ->L125.deforest_coefs_bcoc_EUR
    ...
    if(emissions.NONCO2.EPA.SCALING){
      ...
      # 1) L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR (resource production emission factors)
      # 2) L131.nonco2_tg_R_prc_S_S_Yh_EUR (industrial processes and urban processes input emissions)
      # 3) L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR (agriculture livestock - input emissions)
      # 4) L121.nonco2_tg_R_awb_C_Y_GLU_EUR (agriculture waster burning - input emissions)
      # 5) L122.ghg_tg_R_agr_C_Y_GLU_EUR (agriculture crop - input emissions)
      # 6) L112.ghg_tg_R_en_S_F_Yh_EUR (energy combustion-related - input emission)
      ...
      # Part 1: L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR (resource production emission factors)
      #---------------------------------------------------------------------------------------------------------------
      ...
      # 4) update L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR for resource production
      L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR %>%
      ...
      L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR <- L112.ghg_tgej_R_en_S_F_Yh_update

      # Part 2: L131.nonco2_tg_R_prc_S_S_Yh_EUR (industrial processes and urban processes input emissions)
      ...
      L131.nonco2_tg_R_prc_S_S_Yh_EUR <- L131.nonco2_tg_R_prc_S_S_Yh_adj

      # Part 3: L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR (agriculture livestock input emissions)
      #---------------------------------------------------------------------------------------------------------------
      ...
      # update the origional dataset
      L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR <- L113.ghg_tg_R_an_C_Sys_Fd_Yh_adj

      # Part 4: L121.nonco2_tg_R_awb_C_Y_GLU_EUR (agriculture waster burning)
      #---------------------------------------------------------------------------------------------------------------
      ...
      # update the origional dataset
      L121.nonco2_tg_R_awb_C_Y_GLU_EUR <- L121.nonco2_tg_R_awb_C_Y_GLU_adj


      # Part 5: L122.ghg_tg_R_agr_C_Y_GLU_EUR (agriculture rice - input emissions)
      #---------------------------------------------------------------------------------------------------------------
      ...
      # update the origional dataset
      L122.ghg_tg_R_agr_C_Y_GLU_EUR <- L122.ghg_tg_R_agr_C_Y_GLU_adj

      # Part 6: L112.ghg_tg_R_en_S_F_Yh_EUR (energy combustion-related - input emission)
      #---------------------------------------------------------------------------------------------------------------
      ...
      L112.ghg_tg_R_en_S_F_Yh_EUR <- L112.ghg_tg_R_en_S_F_Yh_adj
    ...
    L111.nonghg_tg_R_en_S_F_Yh_EUR %>%
      ...
      add_legacy_name("L111.nonghg_tg_R_en_S_F_Yh_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F", "L102.ceds_int_shipping_nonco2_tg_S_F", "emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     ...
                     "L101.in_EJ_R_en_Si_F_Yh_EUR", "L1328.in_EJ_R_indenergy_F_Yh_EUR", "L1323.in_EJ_R_iron_steel_F_Y_EUR", "L1324.in_EJ_R_Off_road_F_Y_EUR",
                     "L1325.in_EJ_R_chemical_F_Y_EUR", "L1326.in_EJ_R_aluminum_Yh_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
                     "L1327.in_EJ_R_paper_F_Yh_EUR", "L1328.in_EJ_R_food_F_Yh_EUR", "L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR",
                     "emissions/mappings/UCD_techs_emissions_revised","L154.IEA_histfut_data_times_UCD_shares",
                     "emissions/CEDS/gains_iso_sector_emissions","emissions/CEDS/gains_iso_fuel_emissions","L244.TrnShares_EUR",
    ...
    L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR %>%
      ...
      same_precursors_as("L111.nonghg_tg_R_en_S_F_Yh_EUR") ->
      L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR
    ...
    L112.ghg_tg_R_en_S_F_Yh_EUR %>%
      ...
      add_legacy_name("L112.ghg_tg_R_en_S_F_Yh_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","L102.ceds_int_shipping_nonco2_tg_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","energy/mappings/UCD_techs","energy/calibrated_techs","gcam-europe/calibrated_techs_bld_det_EUR",
                     "emissions/mappings/Trn_subsector","gcam-europe/CEDS_sector_tech_combustion_EUR","emissions/mappings/calibrated_outresources",
                     "L101.in_EJ_R_en_Si_F_Yh_EUR", "L1328.in_EJ_R_indenergy_F_Yh_EUR", "L1323.in_EJ_R_iron_steel_F_Y_EUR", "L1324.in_EJ_R_Off_road_F_Y_EUR",
                     "L1325.in_EJ_R_chemical_F_Y_EUR", "L1326.in_EJ_R_aluminum_Yh_EUR","emissions/mappings/Trn_subsector_revised",
                     "L1327.in_EJ_R_paper_F_Yh_EUR", "L1328.in_EJ_R_food_F_Yh_EUR", "L1328.in_EJ_R_indenergy_infilled_for_food_F_Yh_EUR",
                     "L101.in_EJ_R_en_Si_F_Yh_EUR", "emissions/mappings/Trn_subsector_revised", "emissions/EPA/EPA_2019_raw", "emissions/EPA_CH4N2O_map",
                     "gcam-europe/CEDS_sector_tech_combustion_revised_EUR","emissions/mappings/UCD_techs_emissions_revised","L154.IEA_histfut_data_times_UCD_shares",
                     "emissions/CEDS/gains_iso_sector_emissions","emissions/CEDS/gains_iso_fuel_emissions",
                     "L244.GenericShares_EUR","L244.ThermalShares_EUR","L270.nonghg_tg_state_refinery_F_Yb", "gcam-usa/emissions/BC_OC_assumptions", "gcam-usa/emissions/BCOC_PM25_ratios") ->
      L112.ghg_tg_R_en_S_F_Yh_EUR
    ...
    L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR %>%
      ...
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F",
                     ...
                     "gcam-europe/CEDS_sector_tech_combustion_EUR",
    ...
    L112.in_EJ_R_en_S_F_Yh_calib_all_baseenergy_EUR %>%
      ...
      add_legacy_name("L112.in_EJ_R_en_S_F_Yh_calib_all_baseenergy_EUR") %>%
      add_precursors("L101.in_EJ_R_en_Si_F_Yh_EUR",
                     "energy/mappings/UCD_techs",
                     "gcam-europe/calibrated_techs_bld_det_EUR",
                     "emissions/mappings/Trn_subsector",
                     "gcam-europe/CEDS_sector_tech_combustion_EUR",
    ...
    L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR %>%
      ...
      add_legacy_name("L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","L102.ceds_int_shipping_nonco2_tg_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
    ...
    L121.AWBshare_R_C_GLU %>%
      ...
      add_legacy_name("L121.AWBshare_R_C_Y_GLU_EUR") %>%
    ...
    L121.nonco2_tg_R_awb_C_Y_GLU_EUR %>%
      ...
      add_comments("Agricultural waste burning emissions by region are assigned to crops and land use regions on the basis of excess dry biomass...") %>%
      add_comments("estimated from production, harvest index, and water content") %>%
      add_legacy_name("L121.nonco2_tg_R_awb_C_Y_GLU_EUR") %>%
      same_precursors_as("L121.AWBshare_R_C_Y_GLU_EUR") ->
      L121.nonco2_tg_R_awb_C_Y_GLU_EUR
    ...
    L122.EmissShare_R_C_Y_GLU_EUR %>%
      ...
      add_legacy_name("L122.EmissShare_R_C_Y_GLU_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
    ...
    L122.ghg_tg_R_agr_C_Y_GLU_EUR %>%
      ...
      add_legacy_name("L122.ghg_tg_R_agr_C_Y_GLU_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
    ...
    L124.nonco2_tg_R_grass_Y_GLU_EUR %>%
      ...
      add_legacy_name("L124.nonco2_tg_R_grass_Y_GLU_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR",
                     "L124.LC_bm2_R_Grass_Yh_GLU_adj") ->
      L124.nonco2_tg_R_grass_Y_GLU_EUR
    ...
    L125.bcoc_tgbkm2_R_grass_2000_EUR %>%
      ...
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR",
                     "L124.LC_bm2_R_Grass_Yh_GLU_adj")->L125.bcoc_tgbkm2_R_grass_2000_EUR
    ...
    L125.bcoc_tgbkm2_R_forest_2000_EUR %>%
      ...
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
                     "L124.LC_bm2_R_Grass_Yh_GLU_adj")->L125.bcoc_tgbkm2_R_forest_2000_EUR
    ...
    L125.deforest_coefs_bcoc_EUR %>%
      ...
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
                     "L124.LC_bm2_R_Grass_Yh_GLU_adj")->L125.deforest_coefs_bcoc_EUR
    ...
    L124.nonco2_tg_R_forest_Y_GLU_EUR %>%
      ...
      add_legacy_name("L124.nonco2_tg_R_forest_Y_GLU_EUR") %>%
      add_precursors("L102.ceds_GFED_nonco2_tg_R_S_F","emissions/CEDS/ceds_sector_map","emissions/CEDS/ceds_fuel_map", "common/GCAM_region_names",
                     "common/iso_GCAM_regID","gcam-europe/CEDS_sector_tech_combustion_EUR","gcam-europe/CEDS_sector_tech_combustion_revised_EUR",
                     "L124.LC_bm2_R_UnMgdFor_Yh_GLU_adj") ->
      L124.nonco2_tg_R_forest_Y_GLU_EUR
    ...
    L124.deforest_coefs_EUR %>%
      ...
      add_legacy_name("L124.deforest_coefs_EUR") %>%
      same_precursors_as("L124.nonco2_tg_R_forest_Y_GLU_EUR") ->
      L124.deforest_coefs_EUR
    ...
    L131.nonco2_tg_R_prc_S_S_Yh_EUR %>%
      ...
      add_legacy_name("L131.nonco2_tg_R_prc_S_S_Yh_EUR") %>%
    ...
    return_data(L111.nonghg_tg_R_en_S_F_Yh_EUR, L111.nonghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR, L112.ghg_tg_R_en_S_F_Yh_EUR,
                L112.ghg_tgej_R_en_S_F_Yh_infered_combEF_AP_EUR, L112.in_EJ_R_en_S_F_Yh_calib_all_baseenergy_EUR, L113.ghg_tg_R_an_C_Sys_Fd_Yh_EUR,
                L115.nh3_tg_R_an_C_Sys_Fd_Yh_EUR, L121.nonco2_tg_R_awb_C_Y_GLU_EUR, L121.AWBshare_R_C_Y_GLU_EUR, L122.ghg_tg_R_agr_C_Y_GLU_EUR,
                L122.EmissShare_R_C_Y_GLU_EUR, L124.nonco2_tg_R_grass_Y_GLU_EUR, L124.nonco2_tg_R_forest_Y_GLU_EUR, L124.deforest_coefs_EUR,
                L131.nonco2_tg_R_prc_S_S_Yh_EUR,L125.bcoc_tgbkm2_R_grass_2000_EUR,L125.bcoc_tgbkm2_R_forest_2000_EUR,L125.deforest_coefs_bcoc_EUR)
  } else {
