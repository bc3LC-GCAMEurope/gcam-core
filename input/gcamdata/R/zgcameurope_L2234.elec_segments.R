...
module_gcameurope_L2234.elec_segments <- function(command, ...) {
  ...
  MODULE_OUTPUTS <- c("L2234.Supplysector_elecS_EUR",
                      ...
                      "L2234.StubTechCapFactor_elecS_solar_EUR",
                      "L2234.StubTechCapFactor_elecS_wind_EUR",
                      ...
                      "L2234.StubTechMarket_elecS_EUR",
                      "L2234.StubTechMarket_backup_elecS_EUR",
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2234.SubsectorShrwt_elecS_EUR <- expand_to_segments(A23.elecS_subsector_shrwt, group_by_cols = "subsector") %>%
    ...
    L2234.SubsectorShrwtInterpTo_elecS_EUR <- expand_to_segments(A23.elecS_subsector_shrwt_interpto, group_by_cols = "subsector") %>%
    ...
    # 3b. Primary energy keywords ----------------
    L223.PrimaryRenewKeyword_elec %>%
      expand_to_segments(sector = "sector.name", group_by_cols = c("subsector.name", "technology")) %>%
      tech_name_expansion(sector = "sector.name") %>%
      arrange(year, subsector.name, sector.name, technology) -> L2234.PrimaryRenewKeyword_elecS

    L223.PrimaryRenewKeywordInt_elec %>%
      expand_to_segments(sector = "sector.name", group_by_cols = c("subsector.name", "intermittent.technology")) %>%
    ...
    # 3c. Capital Costs of detailed electric sector technologies ----------------------------
    L223.GlobalTechCapital_elec %>%
      expand_to_segments(sector = "sector.name", group_by_cols = c("subsector.name", "technology")) %>%
      tech_name_expansion(sector = "sector.name") %>%
      filter(!is.na(capital.overnight)) -> L2234.GlobalTechCapital_elecS
    ...
    # TURN INTO AUTOMATIC FUNTION W/O HARD-CODING !!!
    ...
    # 3e. O&M Costs of detailed electric sector technologies -----------------------
    A23.elecS_tech_mapping %>%
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
    ...
    A23.elecS_inttech_mapping %>%
      ...
      bind_rows(A23.elecS_tech_mapping) -> L2234.StubTechMarket_elecS_EUR_temp

    L223.StubTechMarket_elec_EUR %>%
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
      left_join(L2234.StubTechMarket_elecS_EUR_temp,
      ...
      select(region, supplysector = Electric.sector, subsector, stub.technology = Electric.sector.technology,
             year, minicam.energy.input, market.name) -> L2234.StubTechMarket_elecS_EUR

    # 3k. Backup markets -----------------------------------------
    L223.StubTechMarket_backup_EUR %>%
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
      ...
      select(region, supplysector = Electric.sector, subsector, stub.technology = Electric.sector.intermittent.technology,
             year, minicam.energy.input, market.name) -> L2234.StubTechMarket_backup_elecS_EUR
    ...
    # 4. Calibration Year Outputs. Note that all technologies in GCAM-USA are calibrated on the output. ---------------------------------
    A23.elecS_inttech_mapping %>%
      rename(Electric.sector.technology = Electric.sector.intermittent.technology, technology = intermittent.technology) %>%
      bind_rows(A23.elecS_tech_mapping) -> L2234.StubTechProd_elecS_EUR_temp

    L2234.StubTechProd_elecS_EUR_temp %>%
      repeat_add_columns(tibble(year = MODEL_BASE_YEARS)) %>%
      write_to_all_states(c("region","Electric.sector", "supplysector", "subsector",
                            "Electric.sector.technology", "technology","year")) -> L2234.StubTechProd_elecS_EUR_temp

    L2234.StubTechProd_elecS_EUR_temp %>%
      # L223.StubTechProd_elec_EUR does not contain technologies which do not exist historically, such as IGCC or CCS
      # because of this, the join produces NAs; left_join_error_no_match throws an error, so left_join is used
      left_join(L223.StubTechProd_elec_EUR,
      ...
      ungroup() -> L2234.StubTechProd_elecS_EUR

    L223.StubTechProd_elec_EUR %>%
      group_by(region,supplysector,subsector, year ) %>%
      summarise(subscalOutputValue = sum(calOutputValue)) %>%
      ungroup() -> L2234.SubsCalOutputVal_elecS_EUR

    L2234.StubTechProd_elecS_EUR %>%
      # join will produce NAs; left_join_error_no_match throws error, so left_join used
      left_join(L2234.SubsCalOutputVal_elecS_EUR, by = c("region", "year", "subsector")) %>%
      mutate(subscalOutputValue = if_else(is.na(subscalOutputValue), 0, subscalOutputValue)) %>%
      select(-supplysector.y) %>%
      rename(supplysector = supplysector.x) -> L2234.StubTechProd_elecS_EUR

    L1239.R_elec_supply %>%
      select(state, fuel, segment, year, fraction)%>%
      rename(region = state, supplysector = segment, subsector = fuel) %>%
      mutate(year = as.numeric(year)) -> L2234.fuelfractions_segment_EUR

    L2234.StubTechProd_elecS_EUR %>%
      # join will produce NAs; left_join_error_no_match throws error, so left_join used
      left_join(L2234.fuelfractions_segment_EUR, by = c("region", "supplysector", "subsector", "year")) %>%
      ...
      mutate(calOutputValue = if_else(calOutputValue < 0, 0, calOutputValue),
             ...
             calOutputValue = round(calOutputValue, energy.DIGITS_CALOUTPUT)) -> L2234.StubTechProd_elecS_EUR
    ...
    L2234.StubTechProd_elecS_EUR %>%
      ...
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
      left_join(L2234.StubTechProd_elecS_EUR, by = c("region", "supplysector", "subsector", "year")) %>%
      mutate(subs.share.weight = if_else(subsector.cal.value == 0, 0, 1),
             share.weight = if_else(calOutputValue == 0, 0, 1 )) -> L2234.StubTechProd_elecS_EUR

    # Removing offshore wind technologies for states that don't have any resource
    offshore_wind_states <- L223.StubTechMarket_elec_EUR %>%
    ...
    L2234.StubTechProd_elecS_EUR %>%
      ...
      bind_rows(L2234.StubTechProd_elecS_EUR %>%
                  filter(grepl("_offshore", stub.technology)) %>%
                  semi_join(offshore_wind_states, by = c("region"))) -> L2234.StubTechProd_elecS_EUR
    ...
    # to the L2234.SubsectorShrwtInterp_elecS_EUR table.
    ...
    # The fixed value in the above formulations are read in separately in L2234.SubsectorShrwt_elecS_EUR
    ...
    L2234.StubTechProd_elecS_EUR %>%
      filter(year == max(MODEL_BASE_YEARS)) -> L2234.StubTechProd_elecS_EUR_final_cal_year
    ...
    L2234.SubsectorShrwt_elecS_EUR %>%
      ...
      # and is missing from L2234.StubTechProd_elecS_EUR_final_cal_year, generating NAs for grid_storage subsector.cal.value
      # subsector.cal.value is only used to adjust nuclear shareweights, so NAs for grid_storage don't matter; left_join is used
      left_join(L2234.StubTechProd_elecS_EUR_final_cal_year %>%
      ...
      select(region, supplysector, subsector, year, share.weight) ->  L2234.SubsectorShrwt_elecS_EUR
    ...
    L2234.SubsectorShrwt_elecS_EUR %>%
      ...
      arrange(region, subsector, year, supplysector) -> L2234.SubsectorShrwt_elecS_EUR
    ...
    L2234.SubsectorShrwtInterp_elecS_EUR %>%
      ...
      arrange(region, subsector, from.year, supplysector) -> L2234.SubsectorShrwtInterp_elecS_EUR
    ...
    L2234.SubsectorShrwtInterpTo_elecS_EUR %>%
      ...
      arrange(region, subsector, from.year, supplysector) -> L2234.SubsectorShrwtInterpTo_elecS_EUR

    # Get the L2234.StubTechProd_elecS_EUR in the right form without subsector.cal.value
    L2234.StubTechProd_elecS_EUR %>%
      select(region, supplysector, subsector, stub.technology, year, calOutputValue,
             share.weight.year, subs.share.weight, share.weight) -> L2234.StubTechProd_elecS_EUR

    # Fixed Output calibration for hydro
    A23.elecS_tech_mapping %>%
      filter(subsector == "hydro")  -> L2234.StubTechFixOut_elecS_EUR_temp

    L223.StubTechFixOut_elec_EUR %>%
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
      left_join(L2234.StubTechFixOut_elecS_EUR_temp,
                by = c("supplysector", "subsector","stub.technology" = "technology")) %>%
      select(-supplysector, -subsector_1, -stub.technology) %>%
      rename(supplysector = Electric.sector, stub.technology = Electric.sector.technology) %>%
      left_join_error_no_match(L2234.fuelfractions_segment_EUR, by = c("region", "supplysector", "subsector", "year")) %>%
      mutate(fixedOutput = fixedOutput*fraction) %>%
      select(region, supplysector, subsector, stub.technology, year, fixedOutput,
             share.weight.year, subs.share.weight, tech.share.weight) -> L2234.StubTechFixOut_elecS_EUR

    # Fixed Output for hydro in future years.
    # We apply the same fule fractions in future years as in the final calibration year
    L2234.fuelfractions_segment_EUR %>%
      filter(year == max(MODEL_BASE_YEARS),
             subsector == "hydro") -> L2234.fuelfractions_segment_EUR_hydro_final_calibration_year
    ...
    L223.StubTechFixOut_hydro_EUR %>%
      ...
      select(region, supplysector, subsector, stub.technology, year = year.x, fixedOutput,
             share.weight.year, subs.share.weight, tech.share.weight) -> L2234.StubTechFixOut_hydro_elecS_EUR
    ...
    L2234.StubTechMarket_elecS_EUR %>%
      ...
      left_join(L223.StubTechEff_elec_EUR %>%
                  select(-supplysector),
      ...
      filter(!is.na(efficiency)) -> L2234.StubTechEff_elecS_EUR

    # Re-writing efficiencies for gas technologies since efficiencies in L223.StubTechEff_elec_EUR are based on
    ...
    # (in L223.StubTechEff_elec_EUR) are based on the actual historical efficiency in the L123.eff_R_elec_F_Yh.csv file.
    L123.eff_R_elec_F_Yh %>%
    ...
    L2234.StubTechEff_elecS_EUR %>%
      ...
      select(-count_tech, -eff_actual) -> L2234.StubTechEff_elecS_EUR

    # Capacity factors by state for wind and solar
    A23.elecS_inttech_mapping %>%
      rename(stub.technology = intermittent.technology) %>%
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
      left_join(L223.StubTechCapFactor_elec_wind_EUR, by = c("subsector", "stub.technology")) %>%
      select(region, supplysector = Electric.sector, subsector, stub.technology = Electric.sector.intermittent.technology,
             year, capacity.factor) %>%
      filter(subsector != "solar",
             !is.na(capacity.factor)) -> L2234.StubTechCapFactor_elecS_wind_EUR
    ...
    A23.elecS_tech_mapping %>%
      ...
      select(region, supplysector = Electric.sector, subsector, stub.technology = Electric.sector.technology,
             year, capacity.factor) %>%
      filter(subsector != "solar",
             !is.na(capacity.factor)) -> L2234.StubTechCapFactor_elecS_wind_storage_EUR

    L2234.StubTechCapFactor_elecS_wind_EUR %>%
      bind_rows(L2234.StubTechCapFactor_elecS_wind_storage_EUR) -> L2234.StubTechCapFactor_elecS_wind_EUR

    A23.elecS_inttech_mapping %>%
      rename(stub.technology = intermittent.technology) %>%
      # join is intended to duplicate rows; left_join_error_no_match throws error, so left_join used
      left_join(L223.StubTechCapFactor_elec_solar_EUR, by = c("subsector", "stub.technology")) %>%
      select(region, supplysector = Electric.sector, subsector, stub.technology = Electric.sector.intermittent.technology,
             year, capacity.factor) %>%
      filter(subsector != "wind",
             !is.na(capacity.factor)) -> L2234.StubTechCapFactor_elecS_solar_EUR
    ...
    A23.elecS_tech_mapping %>%
      ...
      select(region, supplysector = Electric.sector, subsector, stub.technology = Electric.sector.technology,
             year, capacity.factor) %>%
      filter(subsector != "wind",
             !is.na(capacity.factor)) -> L2234.StubTechCapFactor_elecS_solar_storage_EUR

    L2234.StubTechCapFactor_elecS_solar_EUR %>%
      bind_rows(L2234.StubTechCapFactor_elecS_solar_storage_EUR) -> L2234.StubTechCapFactor_elecS_solar_EUR
    ...
    L2234.geo.tables <- list(L2234.SubsectorLogit_elecS_EUR = L2234.SubsectorLogit_elecS_EUR,
                             L2234.SubsectorShrwt_elecS_EUR = L2234.SubsectorShrwt_elecS_EUR,
                             L2234.SubsectorShrwtInterp_elecS_EUR = L2234.SubsectorShrwtInterp_elecS_EUR,
                             L2234.SubsectorShrwtInterpTo_elecS_EUR = L2234.SubsectorShrwtInterpTo_elecS_EUR,
                             L2234.StubTechEff_elecS_EUR = L2234.StubTechEff_elecS_EUR,
                             L2234.StubTechProd_elecS_EUR = L2234.StubTechProd_elecS_EUR)
    ...
    L2234.SubsectorLogit_elecS_EUR <- L2234.geo.tables_rev[["L2234.SubsectorLogit_elecS_EUR"]]
    L2234.SubsectorShrwt_elecS_EUR <- L2234.geo.tables_rev[["L2234.SubsectorShrwt_elecS_EUR"]]
    L2234.SubsectorShrwtInterp_elecS_EUR <- L2234.geo.tables_rev[["L2234.SubsectorShrwtInterp_elecS_EUR"]]
    L2234.SubsectorShrwtInterpTo_elecS_EUR <- L2234.geo.tables_rev[["L2234.SubsectorShrwtInterpTo_elecS_EUR"]]
    L2234.StubTechEff_elecS_EUR <- L2234.geo.tables_rev[["L2234.StubTechEff_elecS_EUR"]]
    L2234.StubTechProd_elecS_EUR <- L2234.geo.tables_rev[["L2234.StubTechProd_elecS_EUR"]]

    # Remove CSP technologies from regions with no CSP resource
    # This is only a problem in L2234.StubTechProd_elecS_EUR
    ...
    L2234.StubTechProd_elecS_EUR %>%
      ...
      anti_join(A10.renewable_resource_delete,
                by = c("region", "subsector" = "resource_elec_subsector")) -> L2234.StubTechProd_elecS_EUR
    ...
    L2234.StubTech_energy_elecS_EUR <- write_to_all_states(A23.elecS_stubtech_energy_inputs,
                                                           c("region", "supplysector","subsector","stub.technology",
                                                             "period", "minicam.energy.input", "market.name", "efficiency") )

    L2234.StubTech_energy_elecS_EUR %>%
      left_join_error_no_match(states_subregions, by = c("region" = "state")) %>%
      ...
      select(region, supplysector, subsector, stub.technology, year = period,
             minicam.energy.input, efficiency, market.name) -> L2234.StubTechEff_elecS_EUR_additional

    L2234.StubTechEff_elecS_EUR %>%
      bind_rows(L2234.StubTechEff_elecS_EUR_additional) -> L2234.StubTechEff_elecS_EUR
    ...
    L2234.Supplysector_elecS_grid <- write_to_all_states(A23.elecS_sector,
                                                         ...
                                                         region_list = gcameurope.GRID_REGIONS)

    L2234.Supplysector_elecS_EUR %>%
      bind_rows(L2234.Supplysector_elecS_grid) -> L2234.Supplysector_elecS_EUR
    ...
    L2234.ElecReserve_elecS_EUR %>%
      bind_rows(L2234.ElecReserve_elecS_grid) -> L2234.ElecReserve_elecS_EUR
    ...
    L2234.Supplysector_elecS_EUR %>%
      ...
      mutate(logit.year.fillout = min(MODEL_BASE_YEARS),
             logit.exponent = gcameurope.GRID_REGION_LOGIT,
             logit.type = gcameurope.GRID_REGION_LOGIT_TYPE) -> L2234.SubsectorLogit_elecS_grid
    ...
    L2234.SubsectorLogit_elecS_EUR %>%
      ...
      bind_rows(L2234.SubsectorLogit_elecS_grid) -> L2234.SubsectorLogit_elecS_EUR
    ...
    L2234.SubsectorLogit_elecS_grid %>%
      ...
      mutate(year.fillout = min(MODEL_FUTURE_YEARS),
             share.weight = gcameurope.DEFAULT_SHAREWEIGHT) -> L2234.SubsectorShrwtFllt_elecS_grid
    ...
    L2234.SubsectorLogit_elecS_grid %>%
      ...
      mutate(technology = subsector,
             share.weight = gcameurope.DEFAULT_SHAREWEIGHT) %>%
    ...
    L2234.TechMarket_elecS_grid %>%
      mutate(coefficient = gcameurope.DEFAULT_COEFFICIENT) %>%
    ...
    L2234.TechProd_elecS_grid %>%
      ...
      mutate(subsector = paste(region, supplysector, sep = " "),
             ...
             subs.share.weight = gcameurope.DEFAULT_SHAREWEIGHT,
             share.weight = gcameurope.DEFAULT_SHAREWEIGHT) %>%
    ...
    # Create a L2234.PassThroughSector_elecS_EUR table.
    ...
    L2234.StubTechMarket_elecS_EUR %>%
      filter(grepl("_offshore", stub.technology)) %>%
      select(-minicam.energy.input, -market.name) %>%
      left_join_error_no_match(L223.StubTechCost_offshore_wind_EUR %>%
                                 select(-supplysector, -stub.technology),
                               by = c("region", "subsector", "year")) -> L2234.StubTechCost_offshore_wind_elecS_EUR
    ...
    L2234.StubTechCapFactor_elecS_solar_EUR %>%
      ...
      add_precursors("gcam-usa/A23.elecS_tech_mapping",
                     "gcam-usa/A23.elecS_inttech_mapping",
                     "gcam-usa/A23.elecS_tech_availability",
                     "L223.StubTechCapFactor_elec_solar_EUR") ->
      L2234.StubTechCapFactor_elecS_solar_EUR
    ...
    L2234.StubTechCapFactor_elecS_wind_EUR %>%
      ...
      add_precursors("gcam-usa/A23.elecS_tech_mapping",
                     "gcam-usa/A23.elecS_inttech_mapping",
                     "gcam-usa/A23.elecS_tech_availability",
                     "L223.StubTechCapFactor_elec_wind_EUR") ->
      L2234.StubTechCapFactor_elecS_wind_EUR
